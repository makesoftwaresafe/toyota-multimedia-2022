...
int main(int argc, char* argv[])
{
    ...
    dltdata.part_num = -1;
    dltdata.Bflag = DLT_MODE_NON_BLOCKING;

    /* Fetch command line arguments */
    opterr = 0;

    while ((c = getopt (argc, argv, "vashyxmf:o:e:b:c:B")) != -1)
        switch (c)
        {
        case 'v':
			{
        ...
        case 'B':
            {
                dltdata.Bflag = DLT_MODE_BLOCKING;
                break;
            }
        case 'c':
			{
    ...
    if (dlt_client_connect(&dltclient, dltdata.vflag) != DLT_RETURN_ERROR)
    {
        if (dltdata.Bflag)
        {
            ...
            if (0 != dlt_client_send_set_blockmode(&dltclient, 1))
            {
