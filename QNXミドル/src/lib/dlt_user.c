...
#define DLT_USER_RECEIVER_THREAD  (1<<1)
...
int g_dlt_buffer_empty = 1;
int g_dlt_buffer_full = 0;
...
static DltReturnValue dlt_user_set_blockmode(int8_t mode);
static int dlt_user_get_blockmode();
...
static int dlt_start_threads(int id);
static void dlt_stop_threads();
static void dlt_fork_child_fork_handler();

static void dlt_set_log_handle(int handle)
{
    DLT_SEM_LOCK();
    dlt_user.dlt_log_handle = handle;
    DLT_SEM_FREE();
}

static int dlt_get_log_handle()
{
    int handle;
    DLT_SEM_LOCK();
    handle = dlt_user.dlt_log_handle;
    DLT_SEM_FREE();
...
static DltReturnValue dlt_initialize_socket_connection(void)
{
    ...
    if (sockfd == DLT_FD_INIT)
    {
        ...
        {
            /* Socket is not started yet. Retry later */
            if ((dlt_user.connection_state != DLT_USER_RETRY_CONNECT) &&
                (dlt_user.connection_state != DLT_USER_RETRY_CREATE))
            {
                dlt_log(LOG_INFO,
                        "Socket cannot be created. Retrying later...\n");
                dlt_user.connection_state = DLT_USER_RETRY_CREATE;
            }
            DLT_SEM_FREE();
            return DLT_RETURN_OK;
...
#endif
        DLT_SEM_FREE();
        dlt_vlog(LOG_CRIT, "Failed to create socket: %s\n", strerror(errno));
        return DLT_RETURN_ERROR;
    }
    ...
    /* Change socket mode */
    if (dlt_set_socket_mode(dlt_user.socket_mode) == DLT_RETURN_ERROR)
    {
        DLT_SEM_FREE();
        return DLT_RETURN_ERROR;
    }
...
DltReturnValue dlt_set_socket_mode(DltUserSocketMode mode)
{
    ...
    if (dlt_user.dlt_log_handle != -1)
    {
        flags = fcntl(dlt_user.dlt_log_handle, F_GETFL);
        if (mode == DLT_USER_SOCKET_BLOCKING)
        {
            /* Set socket to Blocking mode */
            dlt_vlog(LOG_DEBUG, "%s: Set socket to Blocking mode\n", __func__);
            result = fcntl(dlt_user.dlt_log_handle, F_SETFL, flags & ~O_NONBLOCK);
            dlt_user.socket_mode = DLT_USER_SOCKET_BLOCKING;
        }
        else
        {
            /* Set socket to Non-blocking mode */
            dlt_vlog(LOG_DEBUG, "%s: Set socket to Non-blocking mode\n", __func__);
            result = fcntl(dlt_user.dlt_log_handle, F_SETFL, flags | O_NONBLOCK);
            dlt_user.socket_mode = DLT_USER_SOCKET_NON_BLOCKING;
        }
        ...
        if (result < 0)
        {
            dlt_vlog(LOG_WARNING,
                     ...
                     __func__, DLT_USER_SOCKET_PATH, strerror(errno));
...
/* init shared memory */
    ...
    if (dlt_start_threads(DLT_USER_RECEIVER_THREAD) < 0)
    {
        dlt_user_initialised = false;
        return DLT_RETURN_ERROR;
    }
...
void dlt_get_appid(char * appid)
{
    ...
    {
        snprintf(str,DLT_USER_BUFFER_LENGTH,"Invalid parameter \n");
            dlt_log(LOG_ERR, str);
    }
}
...
DltReturnValue dlt_init_common(void)
{
    ...
    {
        dlt_user.block_mode = DLT_MODE_BLOCKING;
        dlt_user.force_blocking = DLT_MODE_BLOCKING;
    }
...
int dlt_user_atexit_blow_out_user_buffer(void){
    ...
    if ((count > 0) && (dlt_user.timeout_at_exit_handler > 0))
    {
        while(dlt_uptime() < exitTime )
        {
            if (dlt_get_log_handle() == -1)
            {
                /* Reattach to daemon if neccesary */
                dlt_user_log_reattach_to_daemon();

                if ((dlt_get_log_handle() != -1) && (dlt_user.overflow_counter))
                {
...
DltReturnValue dlt_free(void)
{
    ...
    if (dlt_user.dlt_ll_ts)
    {
        for (i=0;i<dlt_user.dlt_ll_ts_max_num_entries;i++)
        {
            ...
            if (dlt_user.dlt_ll_ts[i].DltExtBuff_ptr!=0)
            {
                free(dlt_user.dlt_ll_ts[i].DltExtBuff_ptr);
                dlt_user.dlt_ll_ts[i].DltExtBuff_ptr = 0;
            }
#endif
        }
    ...
    /* destroy mutexes, condition and semaphore */
...
#ifdef DLT_HP_LOG_ENABLE
        if (dlt_user.dlt_ll_ts[handle->log_level_pos].DltExtBuff_ptr)
        {
            free(dlt_user.dlt_ll_ts[handle->log_level_pos].DltExtBuff_ptr);
            dlt_user.dlt_ll_ts[handle->log_level_pos].DltExtBuff_ptr = 0;
        }
#endif
    }
...
int dlt_get_log_state()
{
    ...
    DLT_SEM_LOCK();
    log_state = dlt_user.log_state;
    DLT_SEM_FREE();
    return log_state;
}

void dlt_set_log_state(int8_t state)
{
    DLT_SEM_LOCK();
    dlt_user.log_state = state;
    DLT_SEM_FREE();
}
...
DltReturnValue dlt_user_trace_network_segmented(DltContext *handle, DltNetworkTraceType nw_trace_type, uint16_t header_len, void *header, uint16_t payload_len, void *payload)
{
    ...
    /* start segmented thread if not started */
	if (!dlt_user.dlt_segmented_nwt_handle)
	{
	    dlt_start_threads(DLT_USER_SEGMENTED_THREAD);
	}
...
void dlt_user_receiverthread_function(__attribute__((unused)) void *ptr)
{
    ...
    while (1)
    {
        ...
        /* dlt_buffer_empty is set by main thread to 0 in case data is written to buffer */
        if (g_dlt_buffer_empty == 0)
        {
            /* Reattach to daemon if neccesary */
            dlt_user_log_reattach_to_daemon();

            if (dlt_user_log_resend_buffer() == DLT_RETURN_OK)
            {
                /* writing buffer was successful */
                g_dlt_buffer_empty = 1; /* buffer is empty after flushing */
                g_dlt_buffer_full = 0;
        ...
        usleep(DLT_USER_RECEIVE_UDELAY); /* delay */
    }
...
DltReturnValue dlt_user_log_send_log(DltContextData *log, int mtype)
{
    ...
    else
    {
        ...
        {
            pthread_mutex_lock(&flush_mutex);
            if (g_dlt_buffer_empty == 1)
            {
                /* The resend buffer is empty, thread can send message to DLT
                 * Daemon directly */
                pthread_mutex_unlock(&flush_mutex);
#ifdef DLT_SHM_ENABLE
                if(dlt_user.dlt_log_handle!=-1)
            ...
            {
                /* The resend buffer contains messages that are not yet sent
                 * to the DLT Daemon. In this case, append the messages to the
                 * resend buffer and let the receiverthread handle it later */
                pthread_mutex_unlock(&flush_mutex);
                ret = DLT_RETURN_ERROR;
            }
        }
        ...
        {
            /* Either an error occured while sending the message to DLT Daemon,
             * or the resend buffer is not emppty. We need to append the
             * message to the resend buffer */
        ...
        while (1)
        {
            ...
            while (1)
            {
                ...
                case DLT_USER_MESSAGE_LOG_STATE:
                {
                    ...
                    dlt_set_log_state(userlogstate->log_state);
                ...
                {
                    if (receiver->bytesRcvd < (int32_t)(sizeof(DltUserHeader)+sizeof(DltUserControlMsgBlockMode)))
                    {
                        leave_while = 1;
                        break;
                    }

                    blockmode = (DltUserControlMsgBlockMode*) (receiver->buf+sizeof(DltUserHeader));

                    /* only handle daemon requests if block mode not forced */
                    if (dlt_user.force_blocking == 0)
                    {
                        /* in case main thread currently blocks, it need to be
                         * waken up */
                        pthread_mutex_lock(&flush_mutex);
                        dlt_user_set_blockmode(blockmode->block_mode);
                        pthread_cond_signal(&cond_free); /* signal buffer free */
                        pthread_mutex_unlock(&flush_mutex);
                    }
                    else
                    ...
                    /* keep not read data in buffer */
                    if (dlt_receiver_remove(receiver,(sizeof(DltUserHeader)+sizeof(DltUserControlMsgBlockMode)))==-1)
                    {
                        return -1;
                    }
                }
...
void dlt_user_log_reattach_to_daemon(void)
{
    ...
    if (dlt_get_log_handle() < 0)
    {
        dlt_set_log_handle(-1);
...
int dlt_start_threads(int id)
{
    if ((dlt_receiverthread_handle == 0) && (id & DLT_USER_RECEIVER_THREAD))
    {
    ...
    if ((dlt_user.dlt_segmented_nwt_handle == 0) && (id & DLT_USER_SEGMENTED_THREAD))
    {
        /* Start the segmented thread */
        if (pthread_create(&(dlt_user.dlt_segmented_nwt_handle),
                           0,
                           (void *) dlt_user_trace_network_segmented_thread,
                           0) != 0)
        {
...
DltReturnValue dlt_user_log_out_error_handling(void *ptr1, size_t len1, void* ptr2, size_t len2, void *ptr3, size_t len3)
{
    ...
    if ((ret != DLT_RETURN_OK) && (dlt_user_get_blockmode() == DLT_MODE_BLOCKING))
    {
        pthread_mutex_lock(&flush_mutex);
        g_dlt_buffer_empty = 0;

        /* block until buffer free */
        g_dlt_buffer_full = 1;

        while (g_dlt_buffer_full == 1)
        {
                            ...
                            ptr3, len3) == DLT_RETURN_ERROR)
        {
            if(dlt_user.overflow_counter == 0)
            {
                dlt_log(LOG_CRIT,"Buffer full! Messages will be discarded in BLOCKING.\n");
            }
            ret = DLT_RETURN_BUFFER_FULL;
        }
