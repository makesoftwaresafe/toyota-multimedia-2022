...
int dlt_client_send_set_blockmode(DltClient *client, int block_mode)
{
    ...
    req->mode = block_mode;
    dlt_set_id(req->apid, DLT_ALL_APPLICATIONS); /* send for all applications */
    ...
    if (dlt_client_send_ctrl_msg(
            ...
            "",
            (uint8_t *) req,
            sizeof(DltServiceSetBlockMode)) == DLT_RETURN_ERROR)
    {
        free(req);
        return DLT_RETURN_ERROR;
    }
...
int dlt_client_send_get_blockmode(DltClient *client)
{
    ...
    req->mode = 0;
    dlt_set_id(req->apid, DLT_ALL_APPLICATIONS);
    ...
    if (dlt_client_send_ctrl_msg(
            ...
            "",
            (uint8_t *) req,
            sizeof(DltServiceGetBlockMode)) == DLT_RETURN_ERROR)
    {
        free(req);
        return DLT_RETURN_ERROR;
    }
