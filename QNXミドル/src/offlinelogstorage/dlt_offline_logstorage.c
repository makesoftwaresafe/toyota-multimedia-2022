...
#define DLT_DAEMON_NONBLOCKING 0
#define DLT_DAEMON_BLOCKING 1
#define DLT_DAEMON_BLOCKMODE_NOT_CONFIGURED -1
    ...
    while (*(list) != NULL)
    {
        ...
        *list = (*list)->next;
        ...
        if (tmp->data != NULL)
        {
            ...
            /* ignore return value */
            tmp->data->dlt_logstorage_sync(tmp->data, reason);
...
DLT_STATIC int dlt_logstorage_validate_filter_name(char *name)
{
    ...
    /* Check if section header is of format "FILTER" followed by a number */
                ...
                strlen(DLT_OFFLINE_LOGSTORAGE_NONVERBOSE_STORAGE_SECTION)) == 0)
    {
        for (idx = 26 ; idx < len - 1 ; idx++)
        {
            if (!isdigit(name[idx]))
            {
                return -1;
    ...
    /* Check if section header is of format "FILTER" followed by a number */
                ...
                strlen(DLT_OFFLINE_LOGSTORAGE_NONVERBOSE_CONTROL_SECTION)) == 0)
    {
        for (idx = 25 ; idx < len - 1 ; idx++)
        {
            if (!isdigit(name[idx]))
            {
                return -1;
                                               ...
                                               char *value)
{
    ...
    {
        config->reset_log_level = 2;
    }
    else if (strcmp(value, "DLT_LOG_WARN") == 0)
    {
        config->reset_log_level = 3;
    }
    else if (strcmp(value, "DLT_LOG_INFO") == 0)
    {
        config->reset_log_level = 4;
    }
    else if (strcmp(value, "DLT_LOG_DEBUG") == 0)
    {
        config->reset_log_level = 5;
    }
    else if (strcmp(value, "DLT_LOG_VERBOSE") == 0)
    {
        config->reset_log_level = 6;
    }
    else
    {
        config->reset_log_level = -1;
                                           ...
                                           char *value)
{
    ...
    if (strncmp(sec_name, DLT_OFFLINE_LOGSTORAGE_CONFIG_SECTION, 6) == 0)
    {
    ...
    else if (strncmp(sec_name, DLT_OFFLINE_LOGSTORAGE_NONVERBOSE_STORAGE_SECTION, 26) == 0)
    {
    ...
    else if ((strncmp(sec_name, DLT_OFFLINE_LOGSTORAGE_NONVERBOSE_CONTROL_SECTION, 25) == 0))
    {
 ...
 * @return             0 on success, -1 on error
 */
DLT_STATIC int dlt_logstorage_check_blockmode(DltLogStorage *handle,
                                          char *value)
{
    ...
    {
        handle->block_mode = DLT_DAEMON_BLOCKING;
    }
    else if (strncmp(value, "OFF", 3 || (strncmp(value, "0", 1) == 0)) == 0)
    {
        handle->block_mode = DLT_DAEMON_NONBLOCKING;
    }
    else
    {
        dlt_vlog(LOG_ERR,
...
DLT_STATIC DltLogstorageGeneralConf
    ...
    [DLT_LOGSTORAGE_GENERAL_CONF_BLOCKMODE] = {
        .key = "BlockMode",
        .func = dlt_logstorage_check_blockmode,
        .is_opt = 1
    },
};
...
DLT_STATIC int dlt_daemon_setup_general_properties(DltLogStorage *handle,
                                               ...
                                               char *sec_name)
{
    /* currently no general option supported */
...
int dlt_logstorage_device_connected(DltLogStorage *handle, char *mount_point)
{
    ...
    ret = dlt_logstorage_load_config(handle);
    if (ret == 1)
    {
        return 1;
    }
                         ...
                         int size3)
{
    ...
    if (DLT_IS_HTYP_UEH(standardHeader->htyp))
    {
        ...
        if (num == 0 || num == DLT_RETURN_WRONG_PARAMETER)
        {
            dlt_log(LOG_DEBUG, "No valid filter configuration found!\n");
            return 0;
        }
