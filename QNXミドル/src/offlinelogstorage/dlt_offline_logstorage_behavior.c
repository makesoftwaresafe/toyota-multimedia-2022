 ...
 * @return 0 on success, -1 on error
 */
int dlt_logstorage_sync_on_msg(DltLogStorageFilterConfig *config, int status)
{
    int ret;
                                  ...
                                  int status)
{
    ...
    if (DLT_OFFLINE_LOGSTORAGE_IS_STRATEGY_SET(config->sync, status) > 0)
    {
        ...
        /* sync cache data to file for specific_size strategy */
        if (status == DLT_LOGSTORAGE_SYNC_ON_SPECIFIC_SIZE)
        {
            ...
            ret = fwrite(config->cache, footer->offset, 1, config->log);
            ...
            config -> current_write_file_offset += footer->offset;
        ...
        else  /* sync  cache data to file for cache strategies except specific_size */
        {
            footer = (DltLogStorageCacheFooter *)(config->cache +
                                                  config->file_size);
            if (footer == NULL)
            ...
            /* sync cache to file in case of specific_size with combination */
            if (DLT_OFFLINE_LOGSTORAGE_IS_STRATEGY_SET(config->sync,
                                     DLT_LOGSTORAGE_SYNC_ON_SPECIFIC_SIZE) > 0)
            {
                ret = fwrite(config->cache, config->specific_size, 1, config->log);
            }
            else
            {
            ...
            {
                /* reset to 0 for specific_size strategy with combination  */
                config -> current_write_file_offset = 0;
                /* clean ring buffer and reset footer information */
                memset(config->cache,
                       0,
