        ...
        while(1)
        {
                        ...
                        if ( fgets (line , value_length - 1 , pFile) != NULL )
            {
                  ...
                  if(token[0] && value[0])
                  {
                        ...
                        {
                            memset(
                                daemon_local->flags.msgFilterConfFile,
                                0,
                                DLT_DAEMON_FLAG_MAX);
                            strncpy(
                                daemon_local->flags.msgFilterConfFile,
...
void dlt_daemon_daemonize(int verbose)
{
    ...
    #ifdef __linux__
    /* Some filesystem doesn't support lockf
       This needs to be replaced with better solution */
...
int dlt_daemon_log_internal(DltDaemon *daemon, DltDaemonLocal *daemon_local, char *str, int verbose)
{
    ...
    // Sending data...
    {
        ...
        ret = dlt_daemon_client_send(DLT_DAEMON_SEND_TO_ALL,
                                     daemon,daemon_local,
                                     msg.headerbuffer,
...
#ifdef DLT_USE_UNIX_SOCKET_IPC
// XXX: What is the difference to control unix connection?! -> harmonize
