...
TEST(t_dlt_connection_check_activate, normal)
{
    ...
    ret = dlt_connection_check_activate(&evhdl, &con, &filter, ACTIVATE);
    EXPECT_EQ(DLT_RETURN_OK, ret);

    ret = dlt_connection_check_activate(&evhdl, &con, &filter, DEACTIVATE);
    EXPECT_EQ(DLT_RETURN_OK, ret);
...
{
    EXPECT_EQ(DLT_RETURN_ERROR, dlt_connection_check_activate(NULL, NULL, NULL, DEACTIVATE));
}
