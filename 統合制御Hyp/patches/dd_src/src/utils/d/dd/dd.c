...
%C     [if=file] [of=file] [ibs=bytes] [obs=bytes] [bs=bytes] [cbs=bytes]
       [skip=blocks] [seek=blocks] [count=blocks] [ext={quickskip,quickseek}]
       [conv={ascii,ebcdic,ibm,block,unblock,lcase,ucase,swab,noerror,notrunc,
       sync}] [iflag={nonblock}] [oflag={nonblock}]
...
Options:
   ...
   ext=EXTENSION[,EXTENSION...]
     Extensions:
     quickskip     The skip option will re-position the file
                   pointer by setting it explicitly instead of
                   reading up to the specified offset.
     quickseek     The seek option will re-position the file
                   pointer by setting it explicitly instead of
                   reading up to the specified offset.
   iflag=FLAGS     Set flags on input stream
   oflag=FLAGS     Set flags on output stream
     Flags:
     nonblock      O_NONBLOCK
#endif
*/
...
/* QNX extension bit masks. */
/* If nonzero, seek for block devices will use lseek instead of read. */
#define QEXT_QUICKSEEK 0x00000001u
/* If nonzero, skip for block devices will use lseek instead of read. */
#define QEXT_QUICKSKIP 0x00000002u
...
/* Mask of enabled qnx extensions. */
unsigned extensions_mask = 0;

/* FLAGS mask for input and output stream */
int input_flags  = 0;
int output_flags = 0;
     ...
     char **argv;
{
  ...
  if (input_file != NULL)
    {
      input_fd = open (input_file, O_RDONLY | input_flags);
      if (input_fd < 0)
	error (1, errno, "%s", input_file);
    }
  else {
    int stdin_flags = fcntl (input_fd, F_GETFL);
    if (stdin_flags < 0)
      error("could not get file descriptor flags: %s", strerror(errno));
    if (fcntl (input_fd, F_SETFL, stdin_flags | input_flags) == (-1))
      error("could not set file descriptor flags: %s", strerror(errno));
  ...
  if (output_file != NULL)
    {
      int omode = O_RDWR | O_CREAT | output_flags;
    ...
    int stdout_flags = fcntl (output_fd, F_GETFL);
    if (stdout_flags < 0)
      error("could not get file descriptor flags: %s", strerror(errno));
    if (fcntl (output_fd, F_SETFL, stdout_flags | input_flags) == (-1))
      error("could not set file descriptor flags: %s", strerror(errno));
...
void
skip (fdesc, file, records, blocksize, buf, quick)
     int fdesc;
     char *file;
     long records;
     long blocksize;
     ...
     int quick;
{
  ...
  if (S_ISREG (stats.st_mode) || (quick && S_ISBLK(stats.st_mode)))
    {
      offset = (off_t) records * blocksize;
...
copy ()
{
  ...
  int exit_status = 0;
  int qkseek = (extensions_mask & QEXT_QUICKSEEK) != 0;
  int qkskip = (extensions_mask & QEXT_QUICKSKIP) != 0;
  ...
  if (skip_records > 0)
    skip (input_fd, input_file, skip_records, input_blocksize, ibuf, qkskip);

  if (seek_record > 0)
    skip (output_fd, output_file, seek_record, output_blocksize, obuf, qkseek);
     ...
     char **argv;
{
  ...
  for (i = 1; i < argc; i++)
	...
	parse_conversion (val);
      else if (equal (name, "ext"))
	parse_extension (val);
      else if (equal (name, "iflag"))
	parse_flags(val, &input_flags);
      else if (equal (name, "oflag"))
 	parse_flags(val, &output_flags);
      else
	{
	  n = parse_integer (val);
	  if (n < 0)
  ...
  do
    {
      ...
      if (new != NULL)
	*new++ = '\0';
      if (equal ("quickseek", str))
        {
          extensions_mask |= QEXT_QUICKSEEK;
        }
      else if (equal ("quickskip", str))
        {
          extensions_mask |= QEXT_QUICKSKIP;
        }
      else
        {
          usage ("%s: invalid extension", str, NULL);
          exit (1);
        }
      str = new;
  } while (new != NULL);
...
/* Interpret one "iflag=..." or "oflag=..." option. */
void
  ...
  do
    {
      ...
      if (new != NULL)
        *new++ = '\0';
      if (equal ("nonblock", str))
        {
          *flags |= O_NONBLOCK;
        }
      else
        {
          usage ("%s: invalid flag", str, NULL);
          exit (1);
        }
      str = new;
  } while (new != NULL);
...
__SRCVERSION("$URL: http://svn/product/branches/7.0.0/trunk/utils/d/dd/dd.c $ $Rev: 803013 $")
#endif
