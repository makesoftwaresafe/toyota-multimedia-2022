    ...
    GstBuffer * buf)
{
  ...
  guint8 *data;
  guint bitrate;
  unsigned long total_seek_size = 0;
  ...
  out_vbri:
    ...
    if ((upstream_total_bytes && mp3parse->vbri_bytes &&
         mp3parse->vbri_bytes * 0.8 > upstream_total_bytes)
                              || 
         (total_seek_size < 0.8 * upstream_total_bytes || 
          total_seek_size > upstream_total_bytes)) {
      GST_WARNING_OBJECT (mp3parse, "File appears to have been truncated/wrong size; "
