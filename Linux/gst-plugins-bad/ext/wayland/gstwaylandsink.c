...
static const char gst_waylandsink_caps_str[] =
    GST_WAYLANDSINK_DMA_MAKE_SURFACE_CAPS ";"
    GST_VIDEO_CAPS_MAKE (GST_WAYLANDSINK_SUPPORTED_INPUT_FORMATS);

static GstStaticPadTemplate sink_template = GST_STATIC_PAD_TEMPLATE ("sink",
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS (gst_waylandsink_caps_str));
...
gst_wayland_sink_class_init (GstWaylandSinkClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_SURFACE_ID,
      g_param_spec_uint ("surface-id", "IVI surface id",
          "surface ID to use for IVI shell",
          0, G_MAXINT32, 40, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  gst_wayland_sink_signals[SIGNAL_FIRST_VIDEOFRAME_RENDERED] =
      g_signal_new ("first-videoframe-rendered",
          G_TYPE_FROM_CLASS (klass),
          G_SIGNAL_RUN_LAST,
          G_STRUCT_OFFSET (GstWaylandSinkClass, first_videoframe_rendered),
          NULL, NULL,
...
g_object_class_install_property (gobject_class, PROP_WAYLAND_DISPLAY,
        g_param_spec_pointer ("wl-display", "wl-display",
            "wl-display handle, which will be created when the gst state"
            "change from NULL to ready.(default, the pointer is NULL)",
    ...
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_WAYLAND_DISPLAY:
      g_value_set_pointer(value, ((sink->display)? sink->display->display:NULL));
      break;
    default:
...
gst_wayland_sink_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      ...
      /* the event queue specific for wl_display_sync events */
      ...
      sink->display_wrapper = wl_proxy_create_wrapper(sink->display->display);
      if (!sink->display_wrapper) {
        GST_ERROR_OBJECT (sink, "failed to create display wrapper");
        wl_event_queue_destroy (sink->frame_queue);
        return GST_STATE_CHANGE_FAILURE;
      }

      wl_proxy_set_queue ((struct wl_proxy *) sink->display_wrapper, sink->frame_queue);
  ...
  switch (transition) {
    ...
    case GST_STATE_CHANGE_READY_TO_NULL:
      ...
      /* Wait for redraw_callback from the last frame*/
      ...
      if (sink->display && !sink->window) {     /* -> the window was toplevel */
        wl_event_queue_destroy (sink->frame_queue);
        wl_proxy_wrapper_destroy(sink->display_wrapper);
...
gst_wayland_sink_get_caps (GstBaseSink * bsink, GstCaps * filter)
{
  ...
  /* Only run this when operating without dmabuf supprt */
  if (sink->display && !sink->display->dmabuf) {
...
pool_failed:
  {
    GST_ERROR_OBJECT (sink, "Failed to create new shm pool");
    return TRUE;
  }
}
...
render_last_buffer (GstWaylandSink * sink)
{
  ...
  callback = wl_display_sync (sink->display_wrapper);
...
gst_wayland_sink_show_frame (GstVideoSink * vsink, GstBuffer * buffer)
{
  ...
  if (G_UNLIKELY (!sink->window)) {
    ...
    if (!sink->window) {
      ...
      sink->window = gst_wl_window_new_toplevel (sink->display,
          &sink->video_info, sink->use_subsurface, sink->surface_id, &sink->render_lock);
    }
  ...
  redraw_flag = sink->redraw_pending;
    ...
    if (G_LIKELY (gst_wayland_sink_find_display (sink))) {
      ...
      if (G_UNLIKELY (sink->display->own_display)) {
        ...
        sink->window = gst_wl_window_new_in_surface (sink->display, surface,
            sink->use_subsurface, 0, &sink->render_lock);
      }
    } else {
