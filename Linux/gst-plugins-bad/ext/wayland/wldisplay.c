...
gst_wl_display_finalize (GObject * gobject)
{
  ...
  if (self->application_ivi) {
    ivi_application_destroy (self->application_ivi);
    self->application_ivi = NULL;
  }
  ...
  if (self->dmabuf_own_queue)
    wl_event_queue_destroy (self->dmabuf_own_queue);
    ...
    uint32_t id, const char *interface, uint32_t version)
{
  ...
  if (g_strcmp0 (interface, "wl_compositor") == 0) {
    ...
    wl_shm_add_listener (self->shm, &shm_listener, self);
               ...
               self->application_ivi = wl_registry_bind (registry, id, &ivi_application_interface, 1);
...
gst_wl_display_thread_run (gpointer data)
{
  ...
  /* intialize for own event queue poll */
  self->pfd.fd = pollfd.fd;
    ...
    gboolean take_ownership, GError ** error)
{
  ...
  self->dmabuf_own_queue = wl_display_create_queue (self->display);
...
#define VERIFY_INTERFACE_EXISTS(var, interface) \
  ...
  /* verify we got all the required interfaces */
#define VERIFY_EITHER_INTERFACE_EXISTS(shell_var, shell_interface, ivi_var, ivi_interface) \
  if (self->shell_var && !self->ivi_var) { \
    VERIFY_INTERFACE_EXISTS (shell_var, shell_interface); \
  } else if (!self->shell_var && self->ivi_var) { \
    VERIFY_INTERFACE_EXISTS (ivi_var, ivi_interface); \
  } else { \
       g_set_error (error, g_quark_from_static_string ("GstWlDisplay"), 0, \
           "Could not bind to wl_shell or ivi_application. Either it is not implemented in " \
           "the compositor, or the implemented version doesn't match"); \
       g_object_unref (self); \
       return NULL; \
  }

  VERIFY_EITHER_INTERFACE_EXISTS (shell, "wl_shell", application_ivi, "ivi_application");

#undef VERIFY_EITHER_INTERFACE_EXISTS
