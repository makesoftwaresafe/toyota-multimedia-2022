...
static void
ivi_surface_handle_visibility(void *data, struct ivi_surface *surface, int32_t visibility)
{
}

static void
ivi_surface_handle_configure(void *data,
...
static const struct ivi_surface_listener our_ivi_surface_listener = {
    ivi_surface_handle_visibility,
    ivi_surface_handle_configure
};
...
gst_wl_window_finalize (GObject * gobject)
{
  ...
  if (self->surface_ivi) {
    ivi_surface_destroy(self->surface_ivi);
    self->surface_ivi = NULL;
  }
...
gst_wl_window_new_internal (GstWlDisplay * display, struct wl_surface *parent,
    gboolean use_subsurface, gpointer user_data, GMutex * render_lock)
{
  GstWlWindow *window;
...
gst_wl_window_new_toplevel (GstWlDisplay * display, const GstVideoInfo * info,
    gboolean use_subsurface, guint ivi_surface_id, GMutex * render_lock)
{
  GstWlWindow *window;
  gint width;

  window = gst_wl_window_new_internal (display, NULL, use_subsurface, 
         GINT_TO_POINTER(ivi_surface_id), render_lock);

  if (!display->application_ivi) {
  /* go toplevel */
    ...
    if (window->user_data != NULL)
      surface = GPOINTER_TO_INT(window->user_data);
    window->surface_ivi = ivi_application_surface_create(
          display->application_ivi, surface, use_subsurface ? window->area_surface : window->video_surface);
  }

  if (window->surface_ivi) {
       wl_proxy_set_queue((struct wl_proxy *)window->surface_ivi, window->display->queue);
       ivi_surface_add_listener(window->surface_ivi, &our_ivi_surface_listener, window);
  } else  if (window->shell_surface) {
    wl_shell_surface_add_listener (window->shell_surface,
        &shell_surface_listener, window);
    wl_shell_surface_set_toplevel (window->shell_surface);
  } else {
    GST_ERROR ("Unable to get wlindow surface");
...
gst_wl_window_new_in_surface (GstWlDisplay * display,
    struct wl_surface * parent, gboolean use_subsurface, guint surface_id, GMutex * render_lock)
{
  GstWlWindow *window;
  window = gst_wl_window_new_internal (display, parent, use_subsurface, NULL, render_lock);
  if (!use_subsurface)
    return window;
...
gst_wl_window_is_toplevel (GstWlWindow * window)
{
  ...
  return (window->shell_surface != NULL || window->surface_ivi != NULL);
}
