...
#define DEFAULT_IGNORE_SECTOR            0
...
gst_cd_paranoia_src_init (GstCdParanoiaSrc * src)
{
  ...
  src->ignore_sectors = DEFAULT_IGNORE_SECTOR;
}
...
gst_cd_paranoia_src_class_init (GstCdParanoiaSrcClass * klass)
{
  ...
  g_object_class_install_property (G_OBJECT_CLASS (klass), PROP_SEARCH_OVERLAP,
      g_param_spec_int ("search-overlap", "Search overlap",
          ...
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
   * Added "ignore-sectors" property for CD-Ripping feature to ignore initial 
   * unwanted audio data buffers.
   */
  g_object_class_install_property (G_OBJECT_CLASS (klass), PROP_IGNORE_SECTOR,
      g_param_spec_int ("ignore-sectors", "Ignore Sectors",
          "Ignore Sectors to avoid initial data", 0, G_MAXINT,
          DEFAULT_IGNORE_SECTOR, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  /**
...
gst_cd_paranoia_src_read_sector (GstAudioCdSrc * audiocdsrc, gint sector)
{
  ...
  /**
   * If ignore_sectors is defined with any values other than 0,
   * send an empty buffer structure which should just be skipped
   * to ignore unwanted data from the drive.
   */ 
  if ((src->ignore_sectors != 0) && (src->ign_sct < src->ignore_sectors)) {
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_IGNORE_SECTOR:{
      src->ignore_sectors = g_value_get_int (value);
      break;
    }
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_CACHE_SIZE:
      ...
      break;
    case PROP_IGNORE_SECTOR:
        g_value_set_int (value, src->ignore_sectors);
        break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
