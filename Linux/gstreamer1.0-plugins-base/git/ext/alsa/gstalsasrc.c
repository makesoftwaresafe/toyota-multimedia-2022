...
set_hwparams (GstAlsaSrc * alsa)
{
  ...
  if (alsa->period_time != -1) {
    ...
    CHECK (snd_pcm_hw_params_set_period_time_near (alsa->handle, params,
            &alsa->period_time, NULL), period_time_set);
  ...
  CHECK (snd_pcm_hw_params_get_period_size (params, &alsa->period_size, NULL),
      period_size);
  /* read back real configured value for correct timeout calculation */
  CHECK (snd_pcm_hw_params_get_period_time (params, &alsa->period_time, NULL),
      period_time_get);
...
period_time_get:
  {
    GST_ELEMENT_ERROR (alsa, RESOURCE, SETTINGS, (NULL),
        ("Unable to get period time for recording: %s", snd_strerror (err)));
    snd_pcm_hw_params_free (params);
    return err;
  }
period_size:
    ...
    GstClockTime * timestamp)
{
  ...
  while (cptr > 0) {
    /* need to be restarted also after xrun */
    if (!alsa->started) {
      err = snd_pcm_start(alsa->handle);
      if (err < 0) {
          GST_DEBUG_OBJECT (asrc, "Start error: %s", snd_strerror (err));
          goto read_error;
      }
      alsa->started = 1;
    }
    ...
    if (err < 0) {
      ...
      GST_DEBUG_OBJECT (asrc, "timeout error, %d", err);
      goto timeout_error;
    } else {
      err = snd_pcm_readi (alsa->handle, ptr, cptr);
    }
    GST_DEBUG_OBJECT (asrc, "read %d frames out of %d", err, cptr);
    if (err < 0) {
        if (err == -EAGAIN) {
...
read_error:
  g_usleep(alsa->period_time); /*slow down, as gst will immediately repeat the call*/
timeout_error:
  {
    snd_pcm_format_set_silence(alsa->format, data,
               (length / alsa->bpf) * alsa->channels);
    GST_ALSA_SRC_UNLOCK (asrc);
