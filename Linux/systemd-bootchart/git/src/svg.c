...
static void svg_do_initcall(FILE *of, struct list_sample_data *head, int count_only, double graph_start) {
		...
		if ((starttime = time_to_graph(t - (usecs / 1000000.0)) / 100) < 10.0) {
			fprintf(of, "  <text x=\"%.03f\" y=\"%.03f\"><tspan class=\"run\">%.03fs</tspan></text>\n",
				time_to_graph(t - (usecs / 1000000.0)) - 60,
				ps_to_graph(kcount) + 15,
				starttime);
		} else {
			fprintf(of, "  <text x=\"%.03f\" y=\"%.03f\"><tspan class=\"run\">%.03fs</tspan></text>\n",
				time_to_graph(t - (usecs / 1000000.0)) - 70,
				ps_to_graph(kcount) + 15,
				starttime);
		}
...
static void svg_ps_bars(FILE *of,
		...
		if (starttime < 10.0) {
			fprintf(of, "  <text x=\"%.03f\" y=\"%.03f\"><tspan class=\"run\">%.03fs</tspan></text>\n",
				time_to_graph(starttime - graph_start) - 60.0,
				ps_to_graph(j) + 14.0,
				starttime);
		} else {
			fprintf(of, "  <text x=\"%.03f\" y=\"%.03f\"><tspan class=\"run\">%.03fs</tspan></text>\n",
				time_to_graph(starttime - graph_start) - 70.0,
				ps_to_graph(j) + 14.0,
				starttime);
		}
