...
{
    /* signal not empty to buffer flush thread */
    pthread_mutex_lock(&flush_mutex);
...
DltReturnValue dlt_user_log_send_log(DltContextData *log, int mtype)
{
    ...
    else
    {
		...
		ret = DLT_RETURN_OK;
		if ((dlt_get_log_handle() != -1) && (dlt_user.appID[0] != '\0'))
		{
		    if (g_dlt_buffer_empty == 0) /* buffer not empty */
		    {
		        ret = dlt_user_log_resend_buffer();
		    }
		}
...
static void dlt_fork_child_fork_handler()
{
    ...
    pthread_mutex_unlock(&mq_mutex);
    DLT_SEM_FREE();
    dlt_receiverthread_handle = 0;
    dlt_user.dlt_segmented_nwt_handle = 0;
    dlt_user_initialised = true;
    dlt_user.dlt_log_handle = -1;
    dlt_log_free();
    dlt_free();
