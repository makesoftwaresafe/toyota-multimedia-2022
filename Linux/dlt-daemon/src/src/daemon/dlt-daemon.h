...
#define DLT_DAEMON_BLOCK_MODE_DISABLED 0
#define DLT_DAEMON_BLOCK_MODE_ENABLED 1
...
typedef struct
{
    ...
    char msgFilterConfFile[DLT_DAEMON_FLAG_MAX]; /**< Filter config file path */
...
typedef struct
{
    ...
    DltMessageFilter pFilter; /**< struct for message filter handling */
} DltDaemonLocal;
...
int dlt_daemon_client_update(DltDaemon *daemon, DltDaemonLocal *daemon_local, int verbose);
#endif /* DLT_DAEMON_H */
