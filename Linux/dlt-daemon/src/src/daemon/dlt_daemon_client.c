                                               ...
                                               int verbose)
{
    ...
    int type_mask = DLT_CONNECTION_NONE;
    int client_mask = DLT_FILTER_CLIENT_CONNECTION_DEFAULT_MASK;
    ...
    {
        type_mask |= DLT_CON_MASK_CLIENT_MSG_TCP;
    }

    if (client_mask & DLT_CONNECTION_TO_MASK(DLT_CONNECTION_CLIENT_MSG_SERIAL))
    {
        type_mask |= DLT_CON_MASK_CLIENT_MSG_SERIAL;
    }

    temp = dlt_connection_get_next(daemon_local->pEvent.connections, type_mask);
...
int dlt_daemon_client_send(int sock,DltDaemon *daemon,DltDaemonLocal *daemon_local,void* storage_header,int storage_header_size,void* data1,int size1,void* data2,int size2,int verbose)
{
    ...
    if ((daemon == NULL) || (daemon_local == NULL))
    {
        dlt_vlog(LOG_ERR, "%s: Invalid arguments", __func__);
    }
    ...
    if (sock!=DLT_DAEMON_SEND_TO_ALL && sock!=DLT_DAEMON_SEND_FORCE)
    {
         ...
         * connection or not. Currently, the DLT Viewer receives some daemon
	...
	{
        if ((client_mask & DLT_CONNECTION_TO_MASK(DLT_CONNECTION_CLIENT_MSG_OFFLINE_TRACE)) &&
        ...
        if ((client_mask & DLT_CONNECTION_TO_MASK(DLT_CONNECTION_CLIENT_MSG_OFFLINE_LOGSTORAGE)) &&
...
int dlt_daemon_client_process_control(int sock, DltDaemon *daemon, DltDaemonLocal *daemon_local, DltMessage *msg, int verbose)
{
    ...
    DltConnection *con = dlt_event_handler_find_connection(&daemon_local->pEvent,
                                                          sock);
    if (con == NULL)
    {
        /* this definitively should not happen */
        dlt_log(LOG_ERR, "Unexpected nullptr when searching for connection\n");
        return -1;
    }
    ...
    {
        if (dlt_daemon_filter_is_control_allowed(&daemon_local->pFilter, id) <= 0)
        {
            ...
            */
            ...
            if ((con->type == DLT_CONNECTION_CONTROL_MSG) &&
                (((daemon_local->pFilter.backend == NULL) &&
                (id == DLT_SERVICE_ID_SET_FILTER_LEVEL)) ||
                (id == DLT_SERVICE_ID_GET_FILTER_STATUS)))
            {
                dlt_vlog(LOG_INFO, "Set Filter Level request received\n");
            }
            else
            {
        ...
        case DLT_SERVICE_ID_SET_FILTER_LEVEL:
        {
            dlt_daemon_control_set_filter_level(sock,
                                                 daemon,
                                                 daemon_local,
                                                 msg,
                                                 verbose);
        ...
        case DLT_SERVICE_ID_GET_FILTER_STATUS:
        {
            dlt_daemon_control_get_filter_status(sock,
                                                 daemon,
                                                 daemon_local,
                                                 verbose);
            break;
        ...
        case DLT_SERVICE_ID_SET_BLOCK_MODE:
        {
            dlt_daemon_control_set_block_mode(sock,
                                              daemon,
                                              daemon_local,
                                              msg,
                                              verbose);
        ...
        case DLT_SERVICE_ID_GET_BLOCK_MODE:
        {
            dlt_daemon_control_get_block_mode(sock,
                                              daemon,
                                              daemon_local,
                                              msg,
                                              verbose);
...
void dlt_daemon_control_callsw_cinjection(int sock, DltDaemon *daemon, DltDaemonLocal *daemon_local, DltMessage *msg, int verbose)
{
    ...
    if ((id>=DLT_DAEMON_INJECTION_MIN) && (id<=DLT_DAEMON_INJECTION_MAX))
    {
         ...
         * we have some "natural" filtering (out of range Serivce ID, unknown
         * context etc.)
         */
         ...
         if (dlt_daemon_filter_is_injection_allowed(&daemon_local->pFilter,
                                                    apid,
                                                    ctid,
                                                    extra.ecu,
                                                    id) <= 0)
         {
             dlt_log(LOG_WARNING, "Not allowed injection message received!\n");
...
void dlt_daemon_control_set_filter_level(int sock,
                                         DltDaemon *daemon,
                                         DltDaemonLocal *daemon_local,
                                         DltMessage *msg,
                                         int verbose)
{
    ...
    DltServiceSetFilterLevel *req;
    uint32_t id = DLT_SERVICE_ID_SET_FILTER_LEVEL;
    int new_level = 0;
    ...
    req = (DltServiceSetFilterLevel *) msg->databuffer;
    new_level = req->level;

    if (dlt_daemon_filter_change_filter_level(daemon_local,
                                              new_level,
                                              verbose) < 0)
    {
        dlt_daemon_control_service_response(sock,
...
void dlt_daemon_control_get_filter_status(int sock,
                                          DltDaemon *daemon,
                                          DltDaemonLocal *daemon_local,
                                          int verbose)
{
    ...
    DltMessage msg = { 0 };
    DltServiceGetCurrentFilterInfo *resp;
    ...
    /* prepare payload of data */
    msg.datasize = sizeof(DltServiceGetCurrentFilterInfo);
    ...
    resp = (DltServiceGetCurrentFilterInfo *) msg.databuffer;
    resp->service_id = DLT_SERVICE_ID_GET_FILTER_STATUS;
    resp->status = DLT_SERVICE_RESPONSE_OK;
    memcpy(resp->name, daemon_local->pFilter.current->name, sizeof(resp->name));
    ...
    if (num_injections == -1)
    {
        strncpy(resp->injections,
                DLT_FILTER_CLIENT_ALL,
                strlen(DLT_FILTER_CLIENT_ALL));
    }
    else if (num_injections == 0)
    {
        strncpy(resp->injections,
                DLT_FILTER_CLIENT_NONE,
                strlen(DLT_FILTER_CLIENT_NONE));
    }
    else
    {
        ...
        for (i = 0; i < num_injections; i++)
        {
            ...
            if ((string_len + len + 1) < DLT_CONFIG_FILE_ENTRY_MAX_LEN)
            {
                strncpy(p, injections[i], len);
                p += len;
                                           ...
                                           int verbose)
{
    ...
    DltLogStorage *device;
    int device_index = -1;
    int i = 0;

    PRINT_FUNCTION_VERBOSE(verbose);

    if ((daemon == NULL) || (msg == NULL) ||
        (daemon_local == NULL) || (msg->databuffer == NULL))
    {
        dlt_vlog(LOG_ERR,
    ...
    if (req->connection_type == DLT_OFFLINE_LOGSTORAGE_DEVICE_CONNECTED)
    {
        ...
        if (daemon_local->flags.blockModeAllowed == DLT_DAEMON_BLOCK_MODE_ENABLED)
        {
            if (daemon->storage_handle[device_index].block_mode != DLT_MODE_BLOCKING_UNDEF)
            {
                if (dlt_daemon_user_update_blockmode(daemon,
                                                 DLT_ALL_APPLICATIONS,
                                                 daemon->storage_handle[device_index].block_mode,
...
void dlt_daemon_control_set_block_mode(int sock,
                                       DltDaemon *daemon,
                                       DltDaemonLocal *daemon_local,
                                       DltMessage *msg,
                                       int verbose)
{
    ...
    int32_t id = DLT_SERVICE_ID_SET_BLOCK_MODE;
    ...
    req = (DltServiceSetBlockMode*) (msg->databuffer);

    if ((daemon_local->flags.blockModeAllowed == DLT_DAEMON_BLOCK_MODE_ENABLED) &&
       (daemon_local->client_connections > 0 ||
        daemon_local->internal_client_connections > 0))
    {
        if (daemon->connectionState == 0 && req->mode > DLT_MODE_BLOCKING)
        {
            dlt_vlog(LOG_INFO,
                     "No DLT Client connected or incorrect setting.\n");
        }
        else
        {
            /* Send current block mode */
            if (dlt_daemon_user_update_blockmode(daemon,
                                                 req->apid,
                                                 req->mode,
                                                 verbose) == 0)
            ...
            {
                dlt_vlog(LOG_WARNING, "Update blockMode failed\n");
                dlt_daemon_control_service_response(sock,
                                                    daemon,
                                                    daemon_local,
                                                    id,
    ...
    {
        dlt_vlog(LOG_INFO, "BlockMode not allowed.\n");
    }
...
void dlt_daemon_control_get_block_mode(int sock,
                                       DltDaemon *daemon,
                                       DltDaemonLocal *daemon_local,
                                       DltMessage *msg,
                                       int verbose)
{
    ...
    req = (DltServiceGetBlockMode *) msg->databuffer;

    /* initialise new message */
    if (dlt_message_init(&resp_msg, 0) == -1)
    {
        dlt_daemon_control_service_response(sock,
                                            daemon,
                                            daemon_local,
    ...
    resp_msg.datasize = sizeof(DltServiceGetBlockMode);
    if (resp_msg.databuffer && (resp_msg.databuffersize < resp_msg.datasize))
    {
        free (resp_msg.databuffer);
        resp_msg.databuffer = NULL;
        resp_msg.databuffersize = 0;
    }

    if (resp_msg.databuffer == NULL)
    {
        resp_msg.databuffer = (uint8_t *) malloc(resp_msg.datasize);
        resp_msg.databuffersize = resp_msg.datasize;
    }
    ...
    resp = (DltServiceGetBlockMode *) resp_msg.databuffer;
    resp->service_id = DLT_SERVICE_ID_GET_BLOCK_MODE;
    resp->status = DLT_SERVICE_RESPONSE_OK;
    dlt_set_id(resp->apid, req->apid);

    if (strcmp(req->apid, DLT_ALL_APPLICATIONS) == 0)
    {
        resp->mode = daemon->blockMode;
    }
    else /* mode for specific application */
    {
         DltDaemonApplication *app = dlt_daemon_application_find(daemon,
                                                                 req->apid,
                                                                 daemon->ecuid,
                                                                 verbose);
         ...
         resp->mode = app->blockmode_status;
    }
