...
timer_changed:
	...
	default:
		if (empty) {
			...
			if period event arrived after the avail_update call above we might clear this event with the following clear_timer_queue.
			There is no way to do this in atomic manner, so we need to recheck avail_update if we successfully cleared a poll event. */
