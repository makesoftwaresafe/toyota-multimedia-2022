				    ...
				    GVariantBuilder *changed_props_vb)
{
    ...
    if (strstr (metadata, "DIDL-Lite") == NULL) {
        didl = g_strdup_printf("<DIDL-Lite>%s</DIDL-Lite>", metadata);
    } else {
        didl = g_strdup (metadata);
    }
...
{
	if (mpris_tp_speeds != NULL) {
		GVariantBuilder vb;
		g_variant_builder_init(&vb, G_VARIANT_TYPE("ad"));
		g_variant_builder_add(&vb, "d", 1.0);
		*mpris_tp_speeds = g_variant_ref_sink (g_variant_builder_end (&vb));
	}
	...
	if (upnp_tp_speeds != NULL) {
		...
		*upnp_tp_speeds = g_ptr_array_new_with_free_func (g_free);
		g_ptr_array_add (*upnp_tp_speeds, g_strdup ("1"));
	}
	...
	if (can_get_byte_pos != NULL) {
		*can_get_byte_pos = FALSE;
	}
	return TRUE;
#if 0
