 ...
 * - Output buffers are collected in 4kB chunks to reduce CPU load on small
 *   framesizes
 * - Avoid outputting uninitialized audio buffers for first frame on framesizes
	...
	while (offset < size) {
		...
		if(!output) {
			res = gst_pad_alloc_buffer_and_set_caps(dec->srcpad,
					...
					outbuf_size, NULL, &output);

			if (res != GST_FLOW_OK)
				goto done;
		}

		outbuf = GST_BUFFER_DATA(output) + written_bytes;
		consumed = sbc_decode(&dec->sbc, data + offset, size - offset,
					outbuf, outbuf_size - written_bytes,
					&written);
		...
		if((int)written > outbuf_size - written_bytes) {
			GST_BUFFER_SIZE(output) = written_bytes;
