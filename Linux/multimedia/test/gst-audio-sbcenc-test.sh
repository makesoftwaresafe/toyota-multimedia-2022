...
# --- AUTOMATED SMOKETEST ---
# 
# Description:  This test checks gstreamer sbc encoder
...
TEST_LIST="smoketest_audio"
TEST_TMP_DIR="${SMOKETEST_TMP_DIR}"
...
for TEST_FILE in ${TEST_LIST}; do
  FN_user_execute "${TEST_USER}" "${TEST_CMD} filesrc location=${TEST_DIR}/${TEST_FILE}.wav ! decodebin ! sbcenc ! filesink location=${TEST_TMP_DIR}/${TEST_FILE}.sbc"
done
...
for TEST_FILE in ${TEST_LIST}; do
  FN_user_execute "${TEST_USER}" "${TEST_CMD} filesrc location=${TEST_TMP_DIR}/${TEST_FILE}.sbc ! ${TEST_SBC_DECODER_PLUGIN} ! alsasink device=entertainment_main"
done
