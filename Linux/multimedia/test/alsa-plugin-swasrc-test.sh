...
# --- AUTOMATED SMOKETEST ---
# 
# Description:  This test checks audio source
# Group:        audio,alsa,alsa-swasrc
# Arch NOK:
# Board OK:     
# Board NOK:    
# Reset:        0
...
grep -q "pcm.smoketest_swasrc_playback" ${SMOKETEST_ALSA_TEST_CONFIG}
if [ "$?" != "0" ]; then
  echo -e "pcm.smoketest_swasrc_playback{
	type arate
	slave {
...
FN_user_execute "${TEST_USER}" "aplay -v -Dsmoketest_swasrc_playback ${TEST_WAV}"
