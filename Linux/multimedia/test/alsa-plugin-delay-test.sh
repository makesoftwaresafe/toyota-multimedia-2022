...
# --- AUTOMATED SMOKETEST ---
# 
# Description:  This test checks alsa plugin delay
# Group:        audio,alsa,alsa-delay
# Arch NOK:     
# Board OK:     
# Board NOK:    
# Reset:        0
...
grep -q "pcm.delay_smoketest" ${SMOKETEST_ALSA_TEST_CONFIG}
if [ "$?" != "0" ]; then
  echo -e "pcm.delay_smoketest{
	type delay
	delay {
		1 500
	}
...
FN_user_execute "${TEST_USER}" "aplay -v -Ddelay_smoketest ${TEST_WAV}"
