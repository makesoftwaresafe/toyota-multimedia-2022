...
static bool check_string_list(const GKeyFile *keyfile, const gchar *identifier,
				const gchar *key, const gchar **string_list)
{
	bool property_no_update = true;
	gchar **configurations;
	gsize length;
	gint i;

	configurations = g_key_file_get_string_list(keyfile,
				identifier, key, &length, NULL);
	if ((!string_list && configurations)
		|| (string_list && !configurations))
		property_no_update = false;
	else if (string_list && configurations) {
		if (g_strv_length(string_list) != length) {
			property_no_update = false;
		} else {
			for (i = 0; i < length; i++)
				if (g_strcmp0(string_list[i],
							configurations[i])) {
					property_no_update = false;
					break;
				}
	...
	g_strfreev(configurations);

	return property_no_update;
}

static bool check_for_service_property_update(struct connman_service *service)
{
	GKeyFile *keyfile;
	GError *error = NULL;
	bool property_no_update = true;
	...
	case CONNMAN_SERVICE_TYPE_P2P:
		break;
	case CONNMAN_SERVICE_TYPE_VPN:
		if (service->do_split_routing != g_key_file_get_boolean(keyfile,
				service->identifier, "SplitRouting", &error)
								|| error)
			property_no_update = false;
		g_clear_error(&error);

		if(service->favorite) {
			if (service->autoconnect != g_key_file_get_boolean(keyfile,
					service->identifier, "AutoConnect", &error)
									|| error)
				property_no_update = false;
			g_clear_error(&error);
	...
	case CONNMAN_SERVICE_TYPE_WIFI:
		if (service->network) {
			...
			if (ssid && ssid_len > 0 && ssid[0] != '\0') {
				...
				identifier = g_key_file_get_string(keyfile,
					service->identifier, "SSID", NULL);
				...
				if (g_strcmp0(ssid_str->str, identifier))
					property_no_update = false;
			...
			if (freq != g_key_file_get_integer(keyfile,
							service->identifier,
							"Frequency", NULL))
				property_no_update = false;
		}
	...
	case CONNMAN_SERVICE_TYPE_BLUETOOTH:
	case CONNMAN_SERVICE_TYPE_CELLULAR:
		if (service->favorite != g_key_file_get_boolean(keyfile,
					service->identifier, "Favorite", &error)
								 || error)
			property_no_update = false;
		g_clear_error(&error);
	...
	if (service->passphrase && strlen(service->passphrase) > 0) {
		if (g_strcmp0(service->passphrase, str))
			property_no_update = false;
	} else
		if (str)
			property_no_update = false;
	...
	if (service->ipconfig_ipv4)
		if (!__connman_ipconfig_check_property(service->ipconfig_ipv4,
					keyfile, service->identifier, "IPv4."))
			property_no_update = false;

	if (service->ipconfig_ipv6)
		if (!__connman_ipconfig_check_property(service->ipconfig_ipv6,
					keyfile, service->identifier, "IPv6."))
			property_no_update = false;

	if (!check_string_list(keyfile, service->identifier, "Nameservers",
						 service->nameservers_config))
		property_no_update = false;

	if (!check_string_list(keyfile, service->identifier, "Timeservers",
						service->timeservers_config))
		property_no_update = false;

	if (!check_string_list(keyfile, service->identifier, "Domains",
							service->domains))
		property_no_update = false;

	str = g_key_file_get_string(keyfile,
				service->identifier, "Proxy.Method", NULL);
	if (string2proxymethod(str) != service->proxy_config)
		property_no_update = false;
	g_free(str);

	if (!check_string_list(keyfile, service->identifier, "Proxy.Servers",
							service->proxies))
		property_no_update = false;

	if (!check_string_list(keyfile, service->identifier, "Proxy.Excludes",
							service->excludes))
		property_no_update = false;
	...
	if (g_key_file_get_boolean(keyfile, service->identifier,
				 "Hidden", NULL) != service->hidden_service)
		property_no_update = false;

	str = g_key_file_get_string(keyfile, service->identifier,
							"Config.file", NULL);
	if (g_strcmp0(service->config_file, str))
		property_no_update = false;
	g_free(str);

	str = g_key_file_get_string(keyfile, service->identifier,
							"Config.ident", NULL);
	if (g_strcmp0(service->config_entry, str))
		property_no_update = false;
	g_free(str);
	...
	DBG("service %p no update = %d", service, property_no_update);
	return property_no_update;
...
static int service_save(struct connman_service *service)
{
	...
	if (!check_for_service_property_update(service))
...
static void append_security(DBusMessageIter *iter, void *user_data)
{
	...
	/*
	 * GMMY17-3986: distinguish between WPA-PSK and WPA2-PSK
	 * networks instead of reporting both as "psk". Append
	 * Security as "wpa" for WPA networks and as "rsn" for
	 * WPA2 networks.
	 */
	if (g_strcmp0(str, "psk") == 0) {
		if(service->rsn)
			str = "rsn";
		else
			str = "wpa";
					...
					struct connman_network *network)
{
	...
	service->rsn = connman_network_get_bool(network, "WiFi.RSN");
...
void __connman_service_update_from_network(struct connman_network *network)
{
	...
	rsn = connman_network_get_bool(service->network,"WiFi.RSN");
	...
	/* Track the security change only for wpa/wpa2-psk */
	else if ((service->security == CONNMAN_SERVICE_SECURITY_PSK) && (service->rsn != rsn)) {
		service->rsn = rsn;

		/* Send an update that the security has been changed */
		connman_dbus_property_changed_array(service->path,
