...
int connman_network_set_pairwise_cipher(struct connman_network *network,
					unsigned int pairwise_cipher)
{
	DBG("network %p pairwise_cipher %d", network, pairwise_cipher);

	network->wifi.pairwise_cipher = pairwise_cipher;

	return 0;
}
unsigned int connman_network_get_pairwise_cipher(struct connman_network *network)
{
	return network->wifi.pairwise_cipher;
}

int connman_network_set_groupwise_cipher(struct connman_network *network,
					unsigned int groupwise_cipher)
{
	DBG("network %p groupwise_cipher %d", network, groupwise_cipher);

	network->wifi.group_cipher = groupwise_cipher;

	return 0;
}

unsigned int connman_network_get_groupwise_cipher(struct connman_network *network)
{
	return network->wifi.group_cipher;
}

int connman_network_set_protocol(struct connman_network *network,
		unsigned int protocol)
{
	DBG("network %p protocol %d", network, protocol);

	network->wifi.protocol = protocol;

	return 0;
}

unsigned int connman_network_get_protocol(struct connman_network *network)
{
	return network->wifi.protocol;
}
					...
					const char *key, bool value)
{
	...
	else if (g_strcmp0(key, "WiFi.RSN") == 0)
		network->wifi.rsn = value;
							...
							const char *key)
{
	...
	else if (g_str_equal(key, "WiFi.RSN"))
		return network->wifi.rsn;
