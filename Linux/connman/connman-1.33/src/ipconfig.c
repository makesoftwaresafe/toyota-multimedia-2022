...
bool __connman_ipconfig_check_property(struct connman_ipconfig *ipconfig,
		GKeyFile *keyfile, const char *identifier, const char *prefix)
{
	char *method;
	char *key;
	...
	key = g_strdup_printf("%smethod", prefix);
	method = g_key_file_get_string(keyfile, identifier, key, NULL);
	if (!method || g_strcmp0(__connman_ipconfig_method2string(ipconfig->method), method))
		property_no_update = false;
	g_free(method);
	g_free(key);

	if (!property_no_update)
		return false;
	...
	case CONNMAN_IPCONFIG_METHOD_MANUAL:
		...
		key = g_strdup_printf("%snetmask_prefixlen", prefix);
		if (ipconfig->address->prefixlen != g_key_file_get_integer(
					keyfile, identifier, key, NULL))
			property_no_update = false;
		g_free(key);
		...
		key = g_strdup_printf("%slocal_address", prefix);
		str = g_key_file_get_string(keyfile, identifier, key, NULL);
		if (g_strcmp0(ipconfig->address->local, str))
			property_no_update = false;
		g_free(str);
		g_free(key);
		...
		key = g_strdup_printf("%speer_address", prefix);
		str = g_key_file_get_string(keyfile, identifier, key, NULL);
		if (g_strcmp0(ipconfig->address->peer, str))
			property_no_update = false;
		g_free(str);
		g_free(key);
		...
		key = g_strdup_printf("%sbroadcast_address", prefix);
		str = g_key_file_get_string(keyfile, identifier, key, NULL);
		if (g_strcmp0(ipconfig->address->broadcast, str))
			property_no_update = false;
		g_free(str);
		g_free(key);
		...
		key = g_strdup_printf("%sgateway", prefix);
		str = g_key_file_get_string(keyfile, identifier, key, NULL);
		if (g_strcmp0(ipconfig->address->gateway, str))
			property_no_update = false;
		g_free(str);
		g_free(key);
		break;
	...
	case CONNMAN_IPCONFIG_METHOD_DHCP:
		key = g_strdup_printf("%sDHCP.LastAddress", prefix);
		str = g_key_file_get_string(keyfile, identifier, key, NULL);
		if (g_strcmp0(ipconfig->last_dhcp_address, str))
			property_no_update = false;
		g_free(str);
		g_free(key);
		break;
	...
	if (ipconfig->type == CONNMAN_IPCONFIG_TYPE_IPV6) {
		...
		if (ipconfig->last_dhcpv6_prefixes &&
				ipconfig->last_dhcpv6_prefixes[0]) {
			pprefix =
				g_strdup_printf("%sDHCP.LastPrefixes", prefix);
			last_dhcpv6_prefixes =
					g_key_file_get_string_list(keyfile,
					 identifier, pprefix, &length, NULL);
			if (ipconfig->last_dhcpv6_prefixes &&
				g_strv_length(ipconfig->last_dhcpv6_prefixes)
								 != length)
					property_no_update = false;
			for (i = 0; i < length; i++)
				if (g_strcmp0(ipconfig->last_dhcpv6_prefixes[i],
						last_dhcpv6_prefixes[i]))
					property_no_update = false;
			g_strfreev(last_dhcpv6_prefixes);
			g_free(pprefix);
		}
	}
