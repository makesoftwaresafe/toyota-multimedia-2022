...
struct _GSupplicantNetwork {
	...
	dbus_uint16_t frequency;
	/**
	 * GMMY16-18405 - Adding ciphers and protocol*/
...
bool g_supplicant_network_get_rsn(GSupplicantNetwork *network)
{
	if (!network)
		return false;
...
unsigned int g_supplicant_network_get_pairwise_cipher(GSupplicantNetwork *network)
{
	if(network == NULL)
		return 0;

	SUPPLICANT_DBG("pairwise :%d",network->pairwise_cipher);
	return network->pairwise_cipher;
}
unsigned int g_supplicant_network_get_groupwise_cipher(GSupplicantNetwork *network)
{
	if(network == NULL)
		return 0;

	SUPPLICANT_DBG("group :%d",network->group_cipher);
	return network->group_cipher;
}

unsigned int g_supplicant_network_get_protocol(GSupplicantNetwork *network)
{
	if(network == NULL)
		return 0;

	SUPPLICANT_DBG("protocol :%d",network->protocol);
...
static int add_or_replace_bss_to_network(struct g_supplicant_bss *bss)
{
	...
	/* Set 'rsn' based on whether the AP uses WPA2 or not */
	network->rsn = bss->rsn_keymgmt ? true : false;
	...
	/**
	 * GMMY16-18405 - Could not connect to the Networks with Security type WPA
	 *
	 * We now have the ap_scan parameter of wpa_supplicant set to 2 to connect
	 * to Hidden SSIDs. This forces ConnMan (any external AP using wpa_supplicant)
	 * to explicitly configure the network block with the required security parameters
	 * (key_mgmt, pairwise, group, proto variables) before adding and connecting the network.
	 * Otherwise wpa_supplicant uses the default ciphers (CCMP) for WPA (has TKIP) and the
	 * connection fails*/
	if(bss->rsn_keymgmt) {
		SUPPLICANT_DBG("AP is rsn enabled, pairwise : %d, group :%d",bss->rsn_pairwise,bss->rsn_group);
		network->protocol |= G_SUPPLICANT_PROTO_RSN;
		network->pairwise_cipher = bss->rsn_pairwise;
		network->group_cipher = bss->rsn_group;
	}
	else if (bss->wpa_keymgmt){
		SUPPLICANT_DBG("AP is wpa enabled, pairwise : %d, group :%d",bss->wpa_pairwise,bss->wpa_group);
		network->protocol |= G_SUPPLICANT_PROTO_WPA;
		network->pairwise_cipher = bss->wpa_pairwise;
		network->group_cipher = bss->wpa_group;
	}
			...
			void *user_data)
{
	...
	if (g_strcmp0(key, "KeyMgmt") == 0) {
		...
		/* Clear all the keymgmt bits except wps, because that can
		 * be set via IEs as well as via KeyMgmt.  The WPS bit never does get
		 * cleared once it's set.  Clearing the other bits allows security
		 * changes to be identified correctly when processing KeyMgmt updates
		 * from signal_bss_changed(). */
		...
		/**
		 * GMMY16-18405
		 * Group is not an array but a string*/
...
//				*group |= group_map[i].val;
		...
		SUPPLICANT_DBG("pairwise :%d",value);
	}
}
							...
							void *user_data)
{
	...
	if (g_strcmp0(key, "Capabilities") == 0) {
		...
		if (dbus_message_iter_get_arg_type(iter) != DBUS_TYPE_INVALID) {
			...
			SUPPLICANT_DBG("Disconnect reason %d", reason_code);
		}
	} else if (g_strcmp0(key, "BSSExpireAge") == 0) {
		unsigned int bssexpireage = 0;
		dbus_message_iter_get_basic(iter, &bssexpireage);
		SUPPLICANT_DBG("bssexpireage %d", bssexpireage);
	} else if (g_strcmp0(key, "BSSExpireCount") == 0) {
		unsigned int bssexpirecount = 0;
		dbus_message_iter_get_basic(iter, &bssexpirecount);
		SUPPLICANT_DBG("bssexpirecount %d", bssexpirecount);
...
static void signal_bss_changed(const char *path, DBusMessageIter *iter)
{
	...
	struct g_supplicant_bss *bss;
	bool securitychange = false, cipherchange = false;
	...
	if (old_security != bss->security) {
		...
		SUPPLICANT_DBG("New network security for %s with path %s", bss->ssid, bss->path);
		 ...
		 * be added either to an existing network or an additional network
		 * will be created */
		...
		if (add_or_replace_bss_to_network(new_bss) < 0) {
			...
			SUPPLICANT_DBG("Failed to add the bss %s to network table", new_bss->path);
			g_free (new_bss->path);
			g_free (new_bss);
		}
	...
	/* Extension of GMMY16-18405: Since now it is required to pass cipher
	 * and proto details to wpa_supplicant while adding a network
	 * - wpa-psk and wpa2-psk are closely coupled and the keymgmt
	 * updates are ignored by connman and hence a connection might
	 * not be possible if we do not specify the correct GTK&PTK details
	 * as we need to explicitly configure the network while adding it
	 * to wpa_supplicant for association
	 * - So update the GSupplicantNetwork and inform the wifi plugin
	 * that if the proto or ciphers have been changed */

	/* Track only for wpa/wpa2-psk*/
	if (bss->security == G_SUPPLICANT_SECURITY_PSK) {

		/* Some access points supports/reports both wpa-psk and wpa2-psk
		 * - we might have already informed the security has wpa2-psk
		 * and the proto as rsn, so do not check for the aps which reports
		 * the support of wpa/wpa2-psk
		 * - only check for cipher changes in this case */
		if (!(bss->rsn_keymgmt && bss->wpa_keymgmt)) {

			if (network->rsn && bss->wpa_keymgmt) {
				SUPPLICANT_DBG("security changed for %s from rsn to wpa", bss->ssid);
				network->rsn = false;
				network->group_cipher = bss->wpa_group;
				network->pairwise_cipher = bss->wpa_pairwise;

				if(network->protocol & G_SUPPLICANT_PROTO_RSN)
					network->protocol &= ~G_SUPPLICANT_PROTO_RSN;
				network->protocol |= G_SUPPLICANT_PROTO_WPA;

				securitychange = true;
			} else if (!network->rsn && bss->rsn_keymgmt) {
				SUPPLICANT_DBG("New network security for %s from wpa to rsn", bss->ssid);
				network->rsn = true;
				network->group_cipher = bss->rsn_group;
				network->pairwise_cipher = bss->rsn_pairwise;

				if(network->protocol & G_SUPPLICANT_PROTO_WPA)
					network->protocol &= ~G_SUPPLICANT_PROTO_WPA;
				network->protocol |= G_SUPPLICANT_PROTO_RSN;
			...
			if (securitychange)
				callback_network_changed(network, "Security");
		}

		/* Check for cipher suit change and update the GSupplicantNetwork
		 * and wifi plugin accordingly if there is a change */
		if (network->rsn && bss->rsn_keymgmt) {
			if (network->group_cipher != bss->rsn_group) {
				SUPPLICANT_DBG("New group cipher for %s", bss->ssid);
				network->group_cipher = bss->rsn_group;
				cipherchange = true;
			}
			if (network->pairwise_cipher != bss->rsn_pairwise) {
				SUPPLICANT_DBG("New pairwise cipher for %s", bss->ssid);
				network->pairwise_cipher = bss->rsn_pairwise;
				cipherchange = true;
			}
		} else if (!network->rsn && bss->wpa_keymgmt) {
			if (network->group_cipher != bss->wpa_group) {
				SUPPLICANT_DBG("New group cipher for %s", bss->ssid);
				network->group_cipher = bss->wpa_group;
				cipherchange = true;
			}
			if (network->pairwise_cipher != bss->wpa_pairwise) {
				SUPPLICANT_DBG("New pairwise cipher for %s", bss->ssid);
				network->pairwise_cipher = bss->wpa_pairwise;
				cipherchange = true;
			}
		...
		if (cipherchange)
			callback_network_changed(network, "Cipher");
	}
...
static void interface_create_params(DBusMessageIter *iter, void *user_data)
{
	...
	if(data->configfile)
		supplicant_dbus_dict_append_basic(&dict, "ConfigFile",
					DBUS_TYPE_STRING, &data->configfile);
...
int g_supplicant_interface_create(const char *ifname, const char *driver,
					const char *bridge, const char *configfile,
					GSupplicantInterfaceCallback callback,
							void *user_data)
{
	...
	SUPPLICANT_DBG("ifname %s configfile %s", ifname,configfile);
