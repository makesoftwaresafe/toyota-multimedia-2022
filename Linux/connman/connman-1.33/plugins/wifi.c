...
static int wifi_enable(struct connman_device *device)
{
	...
	const char *configfile = connman_option_get_string("wpaconfigfile");
	...
	DBG("interface %s driver %s configfile %s", interface,driver,configfile);
	ret = g_supplicant_interface_create(interface, driver, NULL, configfile,
...
static void ssid_init(GSupplicantSSID *ssid, struct connman_network *network)
{
	...
	ssid->pairwise_cipher = connman_network_get_pairwise_cipher(network);
	ssid->group_cipher = connman_network_get_groupwise_cipher(network);
	ssid->protocol = connman_network_get_protocol(network);
					...
					struct wifi_data *wifi)
{
	...
	/**
	 * GMMY16-15636 - [Wi-Fi] NGI takes 1 to 2 minutes to pop up the disconnection message
	 * after hotspot password change
	 *
	 * When a passphrase of Connected Access Point (some of them) is changed, wpa_supplicant
	 * falls back to 4WAY_HANSHAKE state and ConnMan considers that the access point is ROAMING.
	 * But then wpa_supplicant gets deAuthenticated from the AP and it continuously tries
	 * to ASSOCIATE with the old pre-shared key even though the Access point deAuthenticates
	 * the association request. Only consider maximum re-tries when the wifi was not connected
	 * and stop re-trying when the wifi was connected before and a DISCONNECT has received.
	 * This will generate the disconnect event and will inform the application about the disconnection*/

	if(wifi->connected == FALSE) {
		DBG("Wifi was not connected before");
		...
		DBG("Wifi was connected, Disconnect has been received ");
	}
...
static void network_added(GSupplicantNetwork *supplicant_network)
{
	...
	rsn = g_supplicant_network_get_rsn(supplicant_network);
	...
	connman_network_set_bool(network, "WiFi.RSN", rsn);
	...
	connman_network_set_protocol(network,
			g_supplicant_network_get_protocol(supplicant_network));
	connman_network_set_pairwise_cipher(network,
			g_supplicant_network_get_pairwise_cipher(supplicant_network));
	connman_network_set_groupwise_cipher(network,
			g_supplicant_network_get_groupwise_cipher(supplicant_network));
...
static void network_changed(GSupplicantNetwork *network, const char *property)
{
	...
	/* GMMY16-18405 Tracking the change in protocol */
	else if (g_str_equal(property, "Security")) {

		connman_network_set_bool(connman_network, "WiFi.RSN", g_supplicant_network_get_rsn(network));
		connman_network_set_protocol(connman_network,
				g_supplicant_network_get_protocol(network));

		/* Update the network plugin which will update the service */
		connman_network_update(connman_network);
	}
	/* GMMY16-18405 Tracking the change in Ciphers */
	else if (g_str_equal(property, "Cipher")) {

		/* Do not update the service plugin, just update the
		 * network */
		connman_network_set_pairwise_cipher(connman_network,
				g_supplicant_network_get_pairwise_cipher(network));
		connman_network_set_groupwise_cipher(connman_network,
				g_supplicant_network_get_groupwise_cipher(network));
	}
					...
					void *user_data)
{
	...
	g_supplicant_interface_create(info->ifname, driver, info->wifi->bridge, configfile,
						ap_create_callback,
							info);
}
