...
LOCAL S32 iap2SetAccInfo(iAP2InitParam_t* iAP2InitParam, iap2UserConfig_t iap2UserConfig)
{
    ...
    if(rc == IAP2_OK)
    {
        ...
        U64 mac_address[] = {IAP2_BT_MAC_ADDRESS2};
    ...
    if(rc == IAP2_OK)
    {
        ...
        accInfo->iAP2BluetoothTransportMAC = calloc(IAP2_BT_MAC_ADDRESS_CNT, sizeof(U64));
        if(accInfo->iAP2BluetoothTransportMAC != NULL)
        {
            for(i=0; i<IAP2_BT_MAC_ADDRESS_CNT; i++)
            {
                accInfo->iAP2BluetoothTransportMAC[i] = mac_address[i];
            }
            accInfo->iAP2BluetoothTransportMAC_count = IAP2_BT_MAC_ADDRESS_CNT;
        }
        else
        {
            rc = IAP2_ERR_NO_MEM;
