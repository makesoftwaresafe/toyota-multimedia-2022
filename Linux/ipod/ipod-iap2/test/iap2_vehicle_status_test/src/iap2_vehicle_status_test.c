...
#include "iap2_vehicle_status_test.h"
...
LOCAL S32 iap2TestStartVehicleStatusUpdates_CB(iAP2Device_t* iap2Device, iAP2StartVehicleStatusUpdatesParameter* theiAP2StartVehicleStatusUpdatesParameter, void* context)
{
    S32 rc = IAP2_CTL_ERROR;
    ...
    printf("\n %d ms, Recieved iAP2StartVehicleStatusUpdates message\n", iap2CurrTimeMs());

    if(theiAP2StartVehicleStatusUpdatesParameter != NULL)
    {
        if(theiAP2StartVehicleStatusUpdatesParameter->iAP2NightMode_count > 0)
        {
            vehicleInfo.iAP2VehicleNightMode = TRUE;
        }
        if(theiAP2StartVehicleStatusUpdatesParameter->iAP2Range_count > 0)
        {
            vehicleInfo.iAP2VehicleRange = TRUE;
        }
        if(theiAP2StartVehicleStatusUpdatesParameter->iAP2InsideTemperature_count > 0)
        {
            vehicleInfo.iAP2VehicleInsideTemperature = TRUE;
        }
        if(theiAP2StartVehicleStatusUpdatesParameter->iAP2OutsideTemperature_count > 0)
        {
            vehicleInfo.iAP2VehicleOutsideTemperature = TRUE;
        }
        if(theiAP2StartVehicleStatusUpdatesParameter->iAP2RangeWarning_count > 0)
        {
            vehicleInfo.iAP2VehicleRangeWarning = TRUE;
        }

        iap2SetTestState(VEHICLE_STATUS_UPDT_RECV,TRUE);
        rc = IAP2_OK;
    }
...
LOCAL S32 iap2TestVehicleStatusUpdate(S32 mq_fd, iAP2Device_t* iap2Device)
{
    S32 rc = IAP2_OK;
    U8 iap2Nightmode = 1;
    U16 iap2Range = 0x3A98;
    S16 iap2InsideTemperature = 0x002A;
    S16 iap2OutsideTemperature = 0x002A;
    U8 iap2RangeWarning = 1;
    iAP2VehicleStatusUpdateParameter theiAP2VehicleStatusUpdateParameter;
    int retry_count = 0;

    /* temporary fix for compiler warnings */
    iap2Device = iap2Device;

    while(iap2GetTestState(VEHICLE_STATUS_UPDT_RECV) != TRUE)
    {
        if(retry_count++ < 100)
        {
            iap2SleepMs(10);
    ...
    if(rc == IAP2_OK)
    {
        printf("\n %d ms, Start Vehicle Status Updates is received (retry count = %d)\n", iap2CurrTimeMs(), retry_count);

        memset(&theiAP2VehicleStatusUpdateParameter, 0, sizeof(iAP2VehicleStatusUpdateParameter));
        if(vehicleInfo.iAP2VehicleNightMode == TRUE)
        {
            rc = iap2AllocateandUpdateData(&(theiAP2VehicleStatusUpdateParameter.iAP2NightMode), &iap2Nightmode,
                &(theiAP2VehicleStatusUpdateParameter.iAP2NightMode_count),
                1, iAP2_uint8);

        }
        if(vehicleInfo.iAP2VehicleRange == TRUE)
        {
             rc = iap2AllocateandUpdateData(&(theiAP2VehicleStatusUpdateParameter.iAP2Range), &iap2Range,
                &(theiAP2VehicleStatusUpdateParameter.iAP2Range_count),
                1, iAP2_uint16);
        }
        if(vehicleInfo.iAP2VehicleRange == TRUE)
        {
            rc = iap2AllocateandUpdateData(&(theiAP2VehicleStatusUpdateParameter.iAP2InsideTemperature), &iap2InsideTemperature,
                            &(theiAP2VehicleStatusUpdateParameter.iAP2InsideTemperature_count),
                            1, iAP2_int16);
        }
        if(vehicleInfo.iAP2VehicleRange == TRUE)
        {
            rc = iap2AllocateandUpdateData(&(theiAP2VehicleStatusUpdateParameter.iAP2OutsideTemperature), &iap2OutsideTemperature,
                              &(theiAP2VehicleStatusUpdateParameter.iAP2OutsideTemperature_count),
                              1, iAP2_int16);
        }
        if(vehicleInfo.iAP2VehicleRange == TRUE)
        {
            rc = iap2AllocateandUpdateData(&(theiAP2VehicleStatusUpdateParameter.iAP2RangeWarning), &iap2RangeWarning,
        ...
        if(rc == IAP2_OK)
        {
            rc = iap2SendMqRecvAck(mq_fd, g_iap2TestDevice.mqAppTskFd,
                                    MQ_CMD_VEHICLE_STATUS_UPDATE,
                                   &theiAP2VehicleStatusUpdateParameter, sizeof(theiAP2VehicleStatusUpdateParameter));
        }
        iAP2FreeiAP2VehicleStatusUpdateParameter(&theiAP2VehicleStatusUpdateParameter);
    }
    else
    {
        printf("\n %d ms, iap2TestVehicleStatusUpdate() failed, iAP2StartVehicleStatusUpdates() not received\n", iap2CurrTimeMs());
    } 
   
    return rc;
}

LOCAL S32 iap2TestStopVehicleStatusUpdates_CB(iAP2Device_t* iap2Device, iAP2StopVehicleStatusUpdatesParameter* theiAP2StopVehicleStatusUpdatesParameter, void* context)
{
    S32 rc = IAP2_OK;
    ...
    iap2Device = iap2Device;
    context = context;
    theiAP2StopVehicleStatusUpdatesParameter = theiAP2StopVehicleStatusUpdatesParameter;

    printf("\n %d ms, Recieved iAP2StopVehicleStatusUpdates message\n", iap2CurrTimeMs());
...
void iap2InitCSCallbacks(iAP2SessionCallbacks_t* iap2CSCallbacks)
{
    ...
    iap2CSCallbacks->iAP2StartVehicleStatusUpdates_cb               = &iap2TestStartVehicleStatusUpdates_CB;
    iap2CSCallbacks->iAP2StopVehicleStatusUpdates_cb                = &iap2TestStopVehicleStatusUpdates_CB;
    ...
    if(rc > 0)
    {
        ...
        if(mq_st == NULL){
            ...
            switch(mq_st->mq_cmd)
            {
                ...
                case MQ_CMD_VEHICLE_STATUS_UPDATE:
                {
                    /* Sending iAP2VehicleStatusUpdate to device */
                    rc = iAP2VehicleStatusUpdate(iap2Device, (iAP2VehicleStatusUpdateParameter*)mq_st->param);
                    if(rc != IAP2_OK)
                    {
                        printf(" %d ms:  iAP2VehicleStatusUpdate failed!  rc = %d \n", iap2CurrTimeMs(), rc);
                        iap2SetTestStateError(TRUE);
                    }
                    break;
                }
...
LOCAL S32 iap2StartTest(S32 mq_fd, iAP2Device_t* iap2device)
{
    ...
    {
        printf("\n %d ms  iap2TestVehicleStatusUpdate() \n", iap2CurrTimeMs());
        rc = iap2TestVehicleStatusUpdate(mq_fd, iap2device);
    }
...
LOCAL S32 application_process_main (int socket_fd) {
    ...
    if(g_rc == IAP2_OK)
    {
        printf("iap2_vehicle_status_test                 PASS       %d\n", g_rc);
        IAP2TESTDLTLOG(DLT_LOG_INFO, "***** iAP2 VEHICLE STAUS TEST PASS *****");
    }
    else
    {
        printf("iap2_vehicle_status_test                 FAIL       %d\n", g_rc);
        IAP2TESTDLTLOG(DLT_LOG_ERROR, "***** iAP2 VEHICLE STAUS TEST FAIL *****");
    }
