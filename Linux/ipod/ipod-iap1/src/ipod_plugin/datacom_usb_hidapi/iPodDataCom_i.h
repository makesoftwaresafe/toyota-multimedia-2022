...
typedef struct _IPOD_HID_DEVINFO
{
    hid_device *dd;
    U16 vid;
    U16 pid;
    ...
    IPOD_REPORT_DATA* reportData; /* HID report table for outgoing reports */
    U32 dataCounter; /* Number of entries in table above */
    U32 maxDataLen;
    U8 *outBuf;

    IPOD_REPORT_DATA* inReportData; /* HID report table for incoming reports */
...
S32 iPodHIDWaitEvent(IPOD_HID_DEVINFO* pDevInfo, U32 buffer_size, U8* msgBuffer);
S32 iPodHIDWriteReport(IPOD_HID_DEVINFO* devinfo, U32 totalLen, const U8* iPod_msg);
