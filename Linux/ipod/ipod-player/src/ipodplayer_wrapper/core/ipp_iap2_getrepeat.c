/****************************************************
 *  ipp_iap2_getrepeat.c                                         
 *  Created on: 2014/01/17 17:55:30                      
 *  Implementation of the Class ipp_iap2_getrepeat       
...
S32 ippiAP2GetRepeat(IPOD_PLAYER_CORE_IPODCTRL_CFG * iPodCtrlCfg, IPOD_PLAYER_CORE_IAP2_EXECUTE_PARAM *param)
{
    S32 rc = IPOD_PLAYER_ERROR;
    IPOD_PLAYER_REPEAT_STATUS repeatMode;
    ...
    /* Init current repeat status to result */
    param->waitData->contents.getRepeatResult.status = IPOD_PLAYER_REPEAT_UNKNOWN;
    
    rc = ippiAP2DBGetRepeat(iPodCtrlCfg->iap2Param.dbHandle, &repeatMode);
    if(rc == IPOD_PLAYER_OK)
    {
        /* Set the current repeat status to result */
        param->waitData->contents.getRepeatResult.status = repeatMode;
    }
