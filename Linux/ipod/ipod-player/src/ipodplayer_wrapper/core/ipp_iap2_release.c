/*****************************************************
 *  ipp_iap2_release.c                                  
 *  Created on: 2017/11/28 21:21:00                   
...
S32 ippiAP2Release( IPOD_PLAYER_CORE_IPODCTRL_CFG *iPodCtrlCfg,
                    IPOD_PLAYER_CORE_IAP2_EXECUTE_PARAM *param)
{
    S32 rc = IPOD_PLAYER_ERROR;
    iAP2Device_t* iap2Device = NULL;
    ReportTable_t   reports_tbl[] = {   IAP2_RELEASE_REPORT,
    ...
    {
        IPOD_DLT_INFO("HID report release sent successfully.");
    }
    else
    {
        IPOD_DLT_ERROR("HID report release could not send. rc = %d", rc);
    }
