...
/* History:
 * 16-Jan-2014 Changes by Advanced Driver Information Technology GmbH:
 * - Changed return value of sbc_decode to always return consumed
 *   bytes and not error codes. This patch was made to avoid endless
 *   loops in the gstsbcdec plugin when errors on the bitstream occur
 * - Added separate return value of consumed bytes to sbc_unpack_frame
 */
...
static int sbc_unpack_frame_internal(const uint8_t *data,
		struct sbc_frame *frame, size_t len, unsigned int *consumed)
{
		...
		struct sbc_frame *frame, size_t len, unsigned int *consumed)
{
	...
	return sbc_unpack_frame_internal(data, frame, len, consumed);
}
...
struct sbc_priv {
	...
	int (*unpack_frame)(const uint8_t *data, struct sbc_frame *frame,
			size_t len, unsigned int *consumed);
			...
			void *output, size_t output_len, size_t *written)
{
	...
	framelen = priv->unpack_frame(input, &priv->frame, input_len, &consumed);
	if (framelen <= 0)
		return consumed;
