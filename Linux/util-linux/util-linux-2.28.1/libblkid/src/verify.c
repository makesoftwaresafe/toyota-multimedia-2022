...
blkid_dev blkid_verify(blkid_cache cache, blkid_dev dev)
{
	...
	if (dev) {
        /*
         * Check for partition table information in probe structure.
         * If partition table and superblock information exist simultaneously,
         * discard superblock information.
         */
        int nvals = blkid_probe_numof_values(cache->probe);
        
        if ((1 == cache->probe->chains[BLKID_CHAIN_PARTS].enabled) && (0 < nvals)) {
            struct blkid_chain *chain = NULL;

            /* Search PTTYPE in probe structure */
            int ptFound = 0;
            int i;
            for (i = 0; i < nvals; i++) {
                struct blkid_prval *v = __blkid_probe_get_value(cache->probe, i);
                if (v != NULL) {
                    const char *name = v->name;
                    const char *data = v->data;
                    size_t len = v->len;
                    chain = v->chain;
                    if ((strcmp(name, "PTTYPE") == 0)
                        && (strcmp(data, "dos") == 0)) {
                        ptFound = 1;
                        break;
                    }
            ...
            /* If PTTYPE found, do processing */
            if (1 == ptFound) {
                /* If only PTTYPE information exists, then no need to process. */
                if (1 < nvals) {
                    /* Reset probe structure */
                    blkid_reset_probe(cache->probe);

                    /* Fill probe structure with values specific to PTTYE only */
                    const char *name = "PTTYPE";
                    const char *data = "dos";
                    size_t len = strlen(data) + 1;
                    cache->probe->cur_chain = chain;
                    blkid_probe_set_value(cache->probe, name, data, len);
                    cache->probe->cur_chain = NULL;
                }
            }
        }
