...
void QWebView::forceGC()
{
	WebCore::gcController().garbageCollectNow();
}

bool QWebView::testBaseElementCanRender(QWebView::BaseElementType type)
{
    QWebFrame *frame = d->page->mainFrame();
	return frame->d->testBaseElementCanRender(static_cast<QWebFrameAdapter::BaseElementType>(type));
}
...
/**/
//add for cookie
void QWebView::deleteCookie(const QUrl& url, const QString& name)
{
    QWebPage* p = page();
    Page* pp = p->d->core(p);
    ...
    WebCore::deleteCookie(pp->mainFrame().document(), aUrl, aName);
}
...
void QWebView::deleteAllCookies()
{
    ...
    WebCore::deleteAllCookies(pp->mainFrame().document());
}

void QWebView::deleteCookiesByHostname(const QString& hostname)
{
    QWebPage* p = page();
    Page* pp = p->d->core(p);
    
    String host = hostname.toLatin1().data();
    
    WebCore::deleteCookiesForHostname(pp->mainFrame().document(), host);
}
/**/
