...
#if ENABLE(FJIB_PDF)
void QWebPagePrivate::showPDFErrorAlert(QWebFrameAdapter* frame, int errorcode, const QString& msg)
{
    q->showPDFErrorAlert(QWebFramePrivate::kit(frame), errorcode, msg);
}

bool QWebPagePrivate::requirePDFPasswordPrompt(QWebFrameAdapter *frame, const QString &msg, const QString &defaultValue, QString *result)
{
    return q->requirePDFPasswordPrompt(QWebFramePrivate::kit(frame), msg, defaultValue, result);
}
#endif
...
void QWebPagePrivate::yield(QWebPageAdapter::YieldReason reason){
	q->yield((QWebPage::YieldReason)reason);
}
...
#endif
/**/
    StorageTracker::initializeTracker("",NULL);
/**/
}
...
/**/
/****************************************************************************
* Description : Set the max size of local storage.
*
* Arguments   : maxSize	  the max size of local storage
*
* Return      : NONE
****************************************************************************/
void QWebPage::setLocalStorageMaxSize(unsigned bytes)
{
    if (!d->page || !d->page->groupPtr())
    {
        return;
    }
    d->page->groupPtr()->groupSettings().setLocalStorageQuotaBytes(bytes);
}

/****************************************************************************
* Description : Get the max size of local storage.
*
* Arguments   : NONE
*
* Return      : the max size of local storage
****************************************************************************/
unsigned QWebPage::localStorageMaxSize()
{
    if (!d->page || !d->page->groupPtr())
    {
        return 0;
    }
    return d->page->groupPtr()->groupSettings().localStorageQuotaBytes();	
}
...
* Return      : NONE
****************************************************************************/
void QWebPage::clearLocalStorage()
{
    if (!d->page || !d->page->mainFrame().document()||
            !d->page->mainFrame().document()->domWindow())
    {
        return;
    }
    MainFrame& mainFrame = d->page->mainFrame();
    Frame* frame = &mainFrame;
    while(frame && frame->document() && frame->document()->domWindow())
    {
        ExceptionCode ec = 0;
        ...
        storage->clear(ec);
        frame = frame->tree().traverseNext(0);
    }
}

/****************************************************************************
* Description : remove the item in local storage with key.
*
* Arguments   : key			[IN] the key of removed item.
*
* Return      : NONE
****************************************************************************/
void QWebPage::removeStorageItem(const QString& itemName)
{
    String key(itemName);
...
* Return      : NONE
****************************************************************************/
void QWebPage::deleteLocalStorage()
{
...
bool QWebPage::handleGSTConnection(GSTMediaType type, GSTPermissionType permission){
    //set flags
    return MediaPlayerPrivateGStreamer::fjib_set_gstplayer_permission((MediaPlayerPrivateGStreamer::GST_MediaType)type, (MediaPlayerPrivateGStreamer::GST_PermissionType)permission);
}
#else
bool QWebPage::handleGSTConnection(GSTMediaType type, GSTActType act){
    return false;
}
#endif
//--
...
void QWebPage::showPDFErrorAlert(QWebFrame *frame, int errorcode, const QString& msg)
{
#if ENABLE(FJIB_PDF)
    Q_UNUSED(frame);
#ifndef QT_NO_MESSAGEBOX
    QMessageBox box(view());
    box.setWindowTitle(tr("PDF Error - %1").arg(mainFrame()->url().host()));
    box.setTextFormat(Qt::PlainText);
    box.setText(msg);
    box.setStandardButtons(QMessageBox::Ok);
    box.exec();
...
bool QWebPage::requirePDFPasswordPrompt(QWebFrame *frame, const QString& msg, const QString& defaultValue, QString* result)
{
	bool ok = false;
#if ENABLE(FJIB_PDF)
    Q_UNUSED(frame);
    ...
    QInputDialog dlg(view());
    dlg.setWindowTitle(tr("PDF Input Password - %1").arg(mainFrame()->url().host()));
...
void QWebPage::setFeaturePermission(QWebFrame* frame, Feature feature, PermissionPolicy policy)
{
    ...
    case Notifications:
        ...
        break;
    case Geolocation:
#if ENABLE(GEOLOCATION) && 1//HAVE(QTLOCATION)
        if (policy != PermissionUnknown)
    ...
    case MediaAudioVideoCapture:
        ...
        if (policy != PermissionUnknown) {
        	if(policy == PermissionGrantedByUser)
            	d->handleUserMediaRequest(true);
        	else
            	d->handleUserMediaRequest(false);
    	}
#endif
        break;
...
/**/
void QWebPage::setGpsInfo(const GpsInfo& gpsInfo)
{
#if ENABLE(GEOLOCATION)
    m_gpsInfo=gpsInfo;
    emit positionUpdated(QString::number(m_gpsInfo.m_latitude), 
		                 QString::number(m_gpsInfo.m_longitude), 
		                 QString::number(m_gpsInfo.m_altitude),
		                 QString::number(m_gpsInfo.m_accuracy), 
		                 QString::number(m_gpsInfo.m_altitudeAccuracy),
		                 QString::number(m_gpsInfo.m_heading),
		                 QString::number(m_gpsInfo.m_speed));
#endif 
}

GpsInfo QWebPage::getGpsInfo() const
{
#if ENABLE(GEOLOCATION)
    return m_gpsInfo;
#endif
...
void QWebPage::bindingGeolocationPositionSource(GeolocationPositionSource* geoSource)
{
     connect(this, 
	    SIGNAL(positionUpdated(QString, QString, QString, QString, QString, QString, QString)),
		geoSource, 
		SLOT(geolocationPositionUpdated(QString, QString, QString, QString, QString, QString, QString)),
		Qt::QueuedConnection);

	connect(geoSource, SIGNAL(getGeolocationPosition()),
		this, SIGNAL(getGeolocationPosition()), Qt::QueuedConnection);   
}
/**/
...
/**/
void QWebPagePrivate::bindingGeolocationPositionSource(GeolocationPositionSource* geoSource)
{
    q->bindingGeolocationPositionSource(geoSource);
}
/**/

#if ENABLE(MEDIA_STREAM)
void QWebPagePrivate::requestUserMediaPermission(PassRefPtr<WebCore::UserMediaRequest> request)
{
    m_userMediaRequest = request;
    QWebPage::Feature flag;
    if(m_userMediaRequest) {
		if(!m_userMediaRequest->hasAudioConstraints() && !m_userMediaRequest->hasVideoConstraints())
			return;
		if(m_userMediaRequest->hasAudioConstraints()) {
			if(m_userMediaRequest->hasVideoConstraints())
            	flag = QWebPage::MediaAudioVideoCapture;
			else
				flag = QWebPage::MediaAudioCapture;
    	} else
			flag = QWebPage::MediaVideoCapture;
    } else
		return;

    emit q->userMediaPermissionRequested(flag);
}
#endif
...
bool QWebPage::event(QEvent *ev)
{
...
#else
        QMouseEvent dummyEvent(QEvent::MouseMove, gsEv->pos().toPoint(), gsEv->screenPos(), gsEv->button(), gsEv->buttons(), gsEv->modifiers());
#endif
...
#else
        QMouseEvent dummyEvent(QEvent::MouseButtonRelease, gsEv->pos().toPoint(), gsEv->screenPos(), gsEv->button(), gsEv->buttons(), gsEv->modifiers());
#endif
...
#else
        QMouseEvent dummyEvent(QEvent::MouseButtonPress, gsEv->pos().toPoint(), gsEv->screenPos(), gsEv->button(), gsEv->buttons(), gsEv->modifiers());
#endif
        d->adjustPointForClicking(&dummyEvent);
...
#else
        QMouseEvent dummyEvent(QEvent::MouseButtonDblClick, gsEv->pos().toPoint(), gsEv->screenPos(), gsEv->button(), gsEv->buttons(), gsEv->modifiers());
#endif
        d->adjustPointForClicking(&dummyEvent);
    ...
    case QEvent::Wheel:
        ...
        break;
#if 0//!defined(QT_NO_GRAPHICSVIEW)
...
void QWebPage::yield(YieldReason reason)
{
    //notp
}
...
void QWebPage::setVehicle(QVehicle* vehicle) {
	d->m_vehicle = vehicle;
}

void QWebPage::setQAMBInterface(QAMBInterface* ambInterface) {
	d->m_ambInterface = ambInterface;
}

void QWebPage::setQNetworkInfoInterface(QNetworkInfo* qNetworkInfo) {
#if ENABLE(NETWORK_INFO)
	d->setQNetworkInfoInterface(qNetworkInfo);
#endif
}

void QWebPage::setQSpeechRecognitionInterface(QSpeechRecognition* qSpeechRecog) {
#if ENABLE(SCRIPTED_SPEECH)
	d->setQSpeechRecognitionInterface(qSpeechRecog);
#endif
}

void QWebPage::setQSpeechSynthesizerInterface(QSpeechSynthesizer* qSpeechSynth) {
#if ENABLE(SPEECH_SYNTHESIS)
	d->setQSpeechSynthesizerInterface(qSpeechSynth);
#endif
}

void QWebPage::setCrashCallback(HookFunction crashHookFunction) {
    d->setCrashCallback(crashHookFunction);
}
