...
#if 1//HAVE(QTLOCATION)
#include "GeolocationClientQt.h"
#endif
#endif

#if ENABLE(FJIB_VIAPI)
#include "VehicleClientQt.h"
#endif

#if ENABLE(FJIB_AMB)
#include "AMBClientQt.h"
#endif

#if ENABLE(NETWORK_INFO)
#include "NetworkInfoClient.h"
#include "NetworkInfoClientQt.h"
#endif

#if ENABLE(SCRIPTED_SPEECH)
#include "SpeechRecognitionClient.h"
#include "SpeechRecognitionClientQt.h"
#endif

#if ENABLE(SPEECH_SYNTHESIS)
#include "SpeechSynthesizerController.h"
#include "SpeechSynthesizerControllerQt.h"
#endif
...
void QWebPageAdapter::initializeWebCorePage()
{
    ...
    pageClients.progressTrackerClient = new ProgressTrackerClientQt(static_cast<FrameLoaderClientQt*>(pageClients.loaderClientForMainFrame));
...
#endif
#if ENABLE(MEDIA_STREAM)
    WebCore::provideUserMediaTo(page, new UserMediaClientImpl(this));
#endif
#if ENABLE(DEVICE_ORIENTATION)
    if (useMock) {
...
#if ENABLE(FJIB_VIAPI)
	WebCore::provideVehicleTo(page, new VehicleClientQt(this));
#endif

#if ENABLE(FJIB_AMB)
//printf("&&&p=%p\n", m_ambInterface);
	WebCore::provideAMBTo(page, new AMBClientQt(this));
#endif
...
void QWebPageAdapter::setVisibilityState(VisibilityState state)
{
    ...
    if (!page)
        return;
    page->setIsVisible(webPageVisibilityStateToWebCoreVisibilityState(state) == WebCore::PageVisibilityStateVisible);
#else
    Q_UNUSED(state);
#endif
}
...
#else
    if (tripleClickTimer.isActive()
        && (ev->pos() - tripleClick).manhattanLength() < QApplication::startDragDistance()) {
#endif
/**/
...
#else
    tripleClickTimer.start(QApplication::doubleClickInterval(), handle());
#endif
/**/
...
#if ENABLE(MEDIA_STREAM)
void QWebPageAdapter::handleUserMediaRequest(bool granted)
{
    if(m_userMediaRequest) {
        if(granted)
            m_userMediaRequest->userMediaAccessGranted();
        else
            m_userMediaRequest->userMediaAccessDenied();
    }
}
#endif

#if ENABLE(NETWORK_INFO)
void QWebPageAdapter::setQNetworkInfoInterface(QNetworkInfo* qNetworkInfo)
{
	NetworkInfoClient* client = new NetworkInfoClientQt(qNetworkInfo, this);
	WebCore::provideNetworkInfoTo(page, client);
}
#endif

#if ENABLE(SCRIPTED_SPEECH)
void QWebPageAdapter::setQSpeechRecognitionInterface(QSpeechRecognition* qSpeechRecog)
{
	SpeechRecognitionClient* client = new SpeechRecognitionClientQt(qSpeechRecog);
	WebCore::provideSpeechRecognitionTo(page, client);
}
#endif

#if ENABLE(SPEECH_SYNTHESIS)
void QWebPageAdapter::setQSpeechSynthesizerInterface(QSpeechSynthesizer* qSpeechSynth)
{
	SpeechSynthesizerController* controller = new SpeechSynthesizerControllerQt(qSpeechSynth);
	WebCore::provideSpeechSynthesizerTo(page, controller);
}
#endif

void QWebPageAdapter::setCrashCallback(WTFCrashHookFunction crashHookFunction)
{
    WTFSetCrashHook((WTFCrashHookFunction)crashHookFunction);
}

bool QWebPageAdapter::popupMenuSupport()
{
	return page->settings().popupMenuSupport();
}
