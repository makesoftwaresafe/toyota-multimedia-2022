...
class Page;
class UndoStep;
class NetworkInfoClientQt;
class SpeechRecognitionClientQt;
class SpeechSynthesizerControllerQt;
}
...
#if ENABLE(FJIB_PDF)
    //PDF API
    virtual void showPDFErrorAlert(QWebFrameAdapter*, int errorcode, const QString& msg) = 0;
    virtual bool requirePDFPasswordPrompt(QWebFrameAdapter*, const QString& msg, const QString& defaultValue, QString* result) = 0;
#endif
...
/**/
    virtual void bindingGeolocationPositionSource(GeolocationPositionSource*) = 0;
/**/
    ...
    virtual void yield(YieldReason reason) = 0;
	...
	QVehicle* m_vehicle;
	QAMBInterface* m_ambInterface;

    #if ENABLE(MEDIA_STREAM)
    virtual void requestUserMediaPermission(PassRefPtr<WebCore::UserMediaRequest>) = 0;
    RefPtr<WebCore::UserMediaRequest> m_userMediaRequest;
    void handleUserMediaRequest(bool);
#endif

#if ENABLE(NETWORK_INFO)
	virtual void setQNetworkInfoInterface(QNetworkInfo* qNetworkInfo);
#endif

#if ENABLE(SCRIPTED_SPEECH)
	virtual void setQSpeechRecognitionInterface(QSpeechRecognition* qSpeechRecog);
#endif

#if ENABLE(SPEECH_SYNTHESIS)
	virtual void setQSpeechSynthesizerInterface(QSpeechSynthesizer* qSpeechSynth);
#endif

    void setCrashCallback(WTFCrashHookFunction crashHookFunction);
