...
#elif USE(V8)
    ScriptValue scriptValue = scriptController.executeScript(ScriptSourceCode(scriptSource));
    v8::Isolate* isolate = scriptValue.isolate();
    v8::HandleScope handleScope(isolate);
    v8::Handle<v8::Value> value = scriptValue.v8Value(); 
    rc = V8::Bindings::convertValueToQVariant(value, QMetaType::Void, &distance, isolate); 
#endif
    return rc;
}
...
bool QWebFrameAdapter::testBaseElementCanRender(BaseElementType type)
{
	bool canRender;
	...
	canRender = false;
	if(view && (view->frame().document())) {
		if(type & HTMLElement) {
			Element* htmlElement = view->frame().document()->documentElement();
			canRender |= htmlElement && htmlElement->renderer();
		}
		if(type & BodyElement) {
			Element* bodyElement = view->frame().document()->body();
			canRender |= bodyElement && bodyElement->renderer();
		}
	}
	else {
		;
