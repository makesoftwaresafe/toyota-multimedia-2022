...
QVariant QWebElement::evaluateJavaScript(const QString& scriptSource)
{
    ...
    ScriptValue scriptValue = controller.executeScript(ScriptSourceCode(scriptSource), m_element);
    v8::Isolate* isolate = scriptValue.isolate();
    v8::HandleScope handleScope(isolate);
    v8::Handle<v8::Value> value = scriptValue.v8Value();
    int distance = 0;
    return V8::Bindings::convertValueToQVariant(value, QMetaType::Void, &distance, isolate); 
#endif
}
...
#elif USE(V8)
static QVariant convertJSValueToWebElementVariant(v8::Handle<v8::Object> object, int *distance, HashSet<int>* visitedObjects)
{
    QVariant ret;
    v8::Isolate* isolate = v8::Isolate::GetCurrent();
    if (!object.IsEmpty() && V8HTMLElement::HasInstance(object, isolate, worldType(isolate))) {
        HTMLElement* htmlelement = V8HTMLElement::toNative(object);
        ret = QVariant::fromValue<QWebElement>(QtWebElementRuntime::create(htmlelement));
    }
    return ret;
}
#endif
...
#elif USE(V8)
    int id = qRegisterMetaType<QWebElement>();
    V8::Bindings::registerCustomType(id, convertJSValueToWebElementVariant);
#endif
}
