...
#else
#include <wtf/v8/ArrayBuffer.h>
#include <wtf/v8/ArrayBufferView.h>
#endif
#include <wtf/Ref.h>
...
/**/
#if USE(V8) // 20140717 try GC when XHR state changes "DONE".
	if(m_state == DONE) {
...
void XMLHttpRequest::didReceiveResponse(unsigned long identifier, const ResourceResponse& response)
{
    ...
    m_response = response;
/**/
    if(m_state < HEADERS_RECEIVED && response.httpStatusCode() != 100) 
        changeState(HEADERS_RECEIVED); 
/**/
