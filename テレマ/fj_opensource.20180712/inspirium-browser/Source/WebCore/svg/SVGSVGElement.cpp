...
bool SVGSVGElement::widthAttributeEstablishesViewport() const
{
    ...
    // The â€˜widthâ€?attribute on the outermost svg element establishes the viewport's width, unless the following conditions are met:
    // - the SVG content is a separately stored resource that is embedded by reference (such as the â€˜objectâ€?element in XHTML [XHTML]), or
    ...
    // - and there are CSS-compatible positioning properties ([CSS2], section 9.3) specified on the referencing element (e.g., the â€˜objectâ€?element)
...
bool SVGSVGElement::heightAttributeEstablishesViewport() const
{
    ...
    // height; otherwise, the â€˜heightâ€?attribute on the outermost svg element establishes the viewport's height.
    RenderSVGRoot* root = toRenderSVGRoot(renderer());
...
void SVGSVGElement::setupInitialView(const String& fragmentIdentifier, Element* anchorNode)
{
    ...
    // Spec: If the SVG fragment identifier addresses a â€˜viewâ€?element within an SVG document (e.g., MyDrawing.svg#MyView
    // or MyDrawing.svg#xpointer(id('MyView'))) then the closest ancestor â€˜svgâ€?element is displayed in the viewport.
    // Any view specification attributes included on the given â€˜viewâ€?element override the corresponding view specification
    // attributes on the closest ancestor â€˜svgâ€?element.
