...
/**/
void SVGDocumentExtensions::buildPendingResourcesIfNeeded(const AtomicString& resourceId)
{
    if (!isIdOfPendingResource(resourceId))
        return;
    std::unique_ptr<SVGDocumentExtensions::PendingElements> clients(removePendingResource(resourceId));
    ASSERT(!clients->isEmpty());

    const PendingElements::const_iterator end = clients->end();
    for (PendingElements::const_iterator it = clients->begin(); it != end; ++it) {
		ASSERT((*it)->hasPendingResources());
        (*it)->buildPendingResource();
        (*it)->setHasPendingResources();
    }
}
/**/
