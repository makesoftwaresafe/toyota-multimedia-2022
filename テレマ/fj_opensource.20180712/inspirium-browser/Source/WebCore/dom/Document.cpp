...
void Document::setWindowAttributeEventListener(const AtomicString& eventType, const QualifiedName& attributeName, const AtomicString& attributeValue)
{
    if (!m_frame)
        return;
#if USE(V8)
	setWindowAttributeEventListener(eventType, createAttributeEventListener(m_frame, attributeName, attributeValue));
#else
...
{
/**/
    if (frame() && !WebCore::Settings::clearReferer())
...
#if USE(V8)
void Document::addMessage(MessageSource source, MessageLevel level, const String& message, const String& sourceURL, unsigned lineNumber, unsigned columnNumber, PassRefPtr<Inspector::ScriptCallStack> callStack, ScriptState* state, unsigned long requestIdentifier)
#else
