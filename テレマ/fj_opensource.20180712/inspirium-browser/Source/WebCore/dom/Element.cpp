    ...
    if (WindowNameCollection::nodeMatchesIfNameAttributeMatch(this)) {
        ...
        if (!oldName.isEmpty() && oldName != id){
            ...
            if (Frame* f = document.frame())
                f->script().namedItemRemoved(&document, oldName);
#endif
        }
...
#if USE(V8)
            if (Frame* f = document.frame())
                f->script().namedItemAdded(&document, newName);
#endif
        }
    }
...
#if USE(V8)
            if (Frame* f = document.frame())
                f->script().namedItemRemoved(&document, oldId);
#endif
        }
...
#if USE(V8)
            if (Frame* f = document.frame())
                f->script().namedItemAdded(&document, newId);
#endif
        }
    }
