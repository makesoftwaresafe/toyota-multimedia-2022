...
public:
...
#if ENABLE(FJIB_PDF)
    //pdf API
    virtual void showPDFErrorAlert(Frame*, int errorcode, const String&) = 0;
    virtual bool requirePDFPasswordPrompt(Frame*, const String& message, const String& defaultValue, String& result) = 0;
#endif
    ...
    virtual WEBCORE_API void populateVisitedLinks();

    virtual WEBCORE_API FloatRect customHighlightRect(Node*, const AtomicString& type, const FloatRect& lineRect);
    virtual WEBCORE_API void paintCustomHighlight(Node*, const AtomicString& type, const FloatRect& boxRect, const FloatRect& lineRect, bool behindText, bool entireLine);
            
    virtual WEBCORE_API bool shouldReplaceWithGeneratedFileForUpload(const String& path, String& generatedFilename);
    virtual WEBCORE_API String generateReplacementFile(const String& path);
