...
class PrintContext {
public:
    explicit WEBCORE_API PrintContext(Frame*);
    WEBCORE_API ~PrintContext();
    ...
    WEBCORE_API void computePageRects(const FloatRect& printRect, float headerHeight, float footerHeight, float userScaleFactor, float& outPageHeight, bool allowHorizontalTiling = false);
    ...
    WEBCORE_API void begin(float width, float height = 0);

    // FIXME: eliminate width argument.
    WEBCORE_API void spoolPage(GraphicsContext& ctx, int pageNumber, float width);

    void spoolRect(GraphicsContext& ctx, const IntRect&);

    // Return to screen mode.
    WEBCORE_API void end();
