...
#if ENABLE(FJIB_PDF)
void Chrome::showPDFErrorAlert(Frame* frame, int errorcode, const String& message)
{
    if (!canRunModalIfDuringPageDismissal(m_page, ChromeClient::AlertDialog, message))
        return;
    ...
    m_client.showPDFErrorAlert(frame, errorcode, displayMessage);
}

bool Chrome::requirePDFPasswordPrompt(Frame* frame, const String& prompt, const String& defaultValue, String& result)
{
    if (!canRunModalIfDuringPageDismissal(m_page, ChromeClient::PromptDialog, prompt))
        return false;
    ...
    bool ok = m_client.requirePDFPasswordPrompt(frame, displayPrompt, frame->displayStringModifiedByEncoding(defaultValue), result);
    if (ok)
        result = frame->displayStringModifiedByEncoding(result);
