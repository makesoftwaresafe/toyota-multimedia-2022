...
#include "AbstractNetworkCache.h"
...
#if OS(WINCE)
	, m_isInServePendingRequests(false)
#endif
{
...
{
#if OS(WINCE)
	if (AbstractNetworkCache::networkCache()->cacheEnabled() && m_isInServePendingRequests) {
		scheduleServePendingRequests();
		return;
	}
#endif
...
#if OS(WINCE)
	m_isInServePendingRequests = true;
#endif
    m_requestTimer.stop();
