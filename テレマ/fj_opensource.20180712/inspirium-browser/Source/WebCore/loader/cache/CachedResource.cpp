...
/**/
    CachedResourceClientWalker<CachedResourceClient> w(m_clients);
    while (CachedResourceClient* client = w.next())
    {
        Page* page = client->getErrorNotifyPage();
        if(page != NULL)
        {
            ResourceError error;
            ...
            {
                int httpStatusCode = m_response.httpStatusCode();
                URL url = m_response.url();
                String httpStatusText = m_response.httpStatusText();
                ResourceError e("HTTP", httpStatusCode, url.string(), httpStatusText);
                error = e;
            }

            page->dispatchError(error);
        }
    }
/**/
    setStatus(status);
...
/**/
void CachedResource::dispatchError()
{
    CachedResourceClientWalker<CachedResourceClient> w(m_clients);
    while (CachedResourceClient* client = w.next())
    {
