...
public:
    friend WEBCORE_API MemoryCache* memoryCache();
    ...
    WEBCORE_API void setCapacities(unsigned minDeadBytes, unsigned maxDeadBytes, unsigned totalBytes);

    // Turn the cache on and off.  Disabling the cache will remove all resources from the cache.  They may
    // still live on if they are referenced by some Web page though.
    WEBCORE_API void setDisabled(bool);
