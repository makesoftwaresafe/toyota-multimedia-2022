...
JSValue QtClass::fallbackObject(ExecState* exec, Instance* inst, PropertyName identifier)
{
    ...
    const QByteArray name = QString(reinterpret_cast<const QChar*>(ustring.deprecatedCharacters()), ustring.length()).toLatin1();
...
#if !HAVE(QT5)
            int iter = 0;
            const char* signature = metaMethod.signature();
...
Field* QtClass::fieldNamed(PropertyName identifier, Instance* instance) const
{
    ...
    String ustring(identifier.publicName());
    const QString name(reinterpret_cast<const QChar*>(ustring.deprecatedCharacters()), ustring.length());
