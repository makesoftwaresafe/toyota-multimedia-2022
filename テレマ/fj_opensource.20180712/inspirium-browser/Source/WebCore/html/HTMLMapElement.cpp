...
static void updateCachedValue(AtomicString& cachedValue, const AtomicString& newValue, TreeScope& treeScope, HTMLMapElement& mapElement)
{
    if (cachedValue != nullAtom)
        treeScope.removeImageMapForNameOrId(cachedValue, mapElement);
    if (newValue != nullAtom)
        treeScope.addImageMapForNameOrId(newValue, mapElement);
...
void HTMLMapElement::parseAttribute(const QualifiedName& name, const AtomicString& value)
{
    ...
    if (isIdAttributeName(name) || name == nameAttr) {
        if (isIdAttributeName(name)) {
            if (inDocument()) {
                updateCachedValue(m_id, value, treeScope(), *this);      
            }
        ...
        if (inDocument()) {
            treeScope().removeImageMapForNameOrId(m_name, *this);
            treeScope().addImageMapForNameOrId(m_name, *this);
        }
        return;
    }
...
Node::InsertionNotificationRequest HTMLMapElement::insertedInto(ContainerNode& insertionPoint)
{
    ...
    if (insertionPoint.inDocument()) {
        updateCachedValue(m_id, document().isHTMLDocument() ? getIdAttribute().lower() : getIdAttribute(), treeScope(), *this);
        updateCachedValue(m_name, document().isHTMLDocument() ? getName().lower() : getName(), treeScope(), *this);
    }
    return request;
}
...
{
    if (insertionPoint.inDocument()) {
        treeScope().removeImageMapForNameOrId(m_id, *this);
        treeScope().removeImageMapForNameOrId(m_name, *this);
    }
