...
void HTMLPlugInImageElement::didAddUserAgentShadowRoot(ShadowRoot* root)
{
    ...
    if (document().page()) {
        String clientTitleText = document().page()->chrome().client().plugInStartLabelTitle(mimeType);
        if (!clientTitleText.isEmpty())
            titleText = clientTitleText;
        String clientSubtitleText = document().page()->chrome().client().plugInStartLabelSubtitle(mimeType);
