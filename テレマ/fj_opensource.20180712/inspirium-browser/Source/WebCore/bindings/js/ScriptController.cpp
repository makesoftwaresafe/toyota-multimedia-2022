...
/**/
Deprecated::ScriptValue ScriptController::evaluate(const ScriptSourceCode&, WebCore::Element*)
{
	// TODO:
	CRASH();
	return Deprecated::ScriptValue();
...
/**/
#if ENABLE(INSPIRIUM_PLUGIN) && !PLATFORM(MAC) && !PLATFORM(QT)
/**/
...
/**/
Deprecated::ScriptValue ScriptController::executeScript(const ScriptSourceCode& sourceCode, WebCore::Element* element)
{
	if (!canExecuteScripts(AboutToExecuteScript) || isPaused())
		return Deprecated::ScriptValue();
	...
	return evaluate(sourceCode, element);
}
/**/
