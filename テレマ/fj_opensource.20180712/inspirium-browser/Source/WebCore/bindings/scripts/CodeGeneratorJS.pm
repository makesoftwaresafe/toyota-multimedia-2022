...
sub GenerateHeader
{
    ...
    if ($interfaceName eq "Node") {
        ...
        push(@headerContent, "    static WEBCORE_EXPORTDATA const JSC::ClassInfo s_info;\n");
        push(@headerContent, "public:");
        push(@headerContent, "    static const JSC::ClassInfo* info() { return &s_info; }\n\n");
    } elsif ($interfaceName eq "Document") {
        push(@headerContent, "protected:");
        push(@headerContent, "    static WEBCORE_EXPORTDATA const ::JSC::ClassInfo s_info;\n");
        push(@headerContent, "public:");
        push(@headerContent, "    static const ::JSC::ClassInfo* info() { return &s_info; }\n\n");
...
sub GenerateImplementation
{
    ...
    if ($numAttributes > 0) {
        foreach my $attribute (@{$interface->attributes}) {
            ...
            if($getFunctionName eq "jsWorkerContextFileExceptionConstructor") {
                push(@implContent, "//comment by FNST\n");
                push(@implContent, "//JSValue jsWorkerContextFileExceptionConstructor(ExecState* exec, JSValue slotBase, PropertyName)\n");
                push(@implContent, "#endif\n") if $attributeConditionalString;
                next;
            }
    ...
    if ($hasReadWriteProperties) {
        foreach my $attribute (@{$interface->attributes}) {
            if (!IsReadonly($attribute)) {
                ...
                my $attributeConditionalString = $codeGenerator->GenerateConditionalString($attribute->signature);
                push(@implContent, "#if ${attributeConditionalString}\n") if $attributeConditionalString;
                        if($putFunctionName eq "setJSWorkerContextFileExceptionConstructor") {
                            push(@implContent, "//comment by FNST\n");
                            push(@implContent, "//void setJSWorkerContextFileExceptionConstructor(ExecState* exec, JSObject* thisObject, JSValue value)\n");
                            push(@implContent, "#endif\n") if $attributeConditionalString;
                            next;
                        }
...
sub GenerateHashTable
{
    ...
    foreach my $key (@{$keys}) {
        ...
        if(@$value1[$i] eq "jsWorkerContextFileExceptionConstructor" && @$value2[$i] eq "setJSWorkerContextFileExceptionConstructor"){
            push(@implContent, "    //comment by FNST\n");
            push(@implContent, "    //{ \"$key\", @$specials[$i], (intptr_t)" . $firstTargetType . "(@$value1[$i]), (intptr_t)@$value2[$i], NoIntrinsic },\n");
        } else {
...
#
            if ($codeGenerator->ExtendedAttributeContains($interface->extendedAttributes->{"CallWith"}, "Document")) {
                push(@constructorArgList, "document");
                push(@$outputArray, "    ScriptExecutionContext* context = castedThis->scriptExecutionContext();\n");
                push(@$outputArray, "    if (!context)\n");
                push(@$outputArray, "        return throwVMError(exec, createReferenceError(exec, \"${interfaceName} constructor associated document is unavailable\"));\n");
                push(@$outputArray, "    Document* document = static_cast<Document*>(context);\n");
            }
#
...
sub GenerateConstructorHelperMethods
{
	...
	push(@$outputArray, "#if USE(NON_STANDARD_CONSTRUCTOR_NAME)\n");
	push(@$outputArray, "    putDirect(vm, vm.propertyNames->name, jsString(&vm, ${className}::info()->className), ReadOnly | DontDelete | DontEnum);\n");
