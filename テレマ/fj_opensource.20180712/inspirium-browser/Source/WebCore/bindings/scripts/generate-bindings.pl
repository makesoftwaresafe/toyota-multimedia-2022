...
GetOptions('include=s@' => \@idlDirectories,
           'idlIncludeFile=s' => \$idlIncludeFile,
...
if ($idlIncludeFile) {
    open INCLUDE_FILE, "< $idlIncludeFile" or die "Cannot open $idlIncludeFile\n";
    while (my $line = <INCLUDE_FILE>) {
    #	printf("line=%s\n",$line);
        my ($includeFile, @followingIncludeFile) = split(/\s+/, $line);
    #    printf("includeFile=%s\n",$includeFile);
        push @idlDirectories, $includeFile;
    }
    close INCLUDE_FILE;
    
    for my $p (@idlDirectories) {
    #	printf("idlDirectories=%s",$p);
    }
}
