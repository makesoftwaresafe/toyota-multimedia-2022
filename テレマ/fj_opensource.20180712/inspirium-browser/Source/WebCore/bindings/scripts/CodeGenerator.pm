...
# 
#	return 1 if $object->IsTypedArrayType($type) && $object->FileNamePrefix eq "JS";
	return 1 if $object->IsTypedArrayType($type) && ($object->FileNamePrefix eq "JS" || $object->FileNamePrefix eq "V8");
...
sub IsCallbackInterface
{
  ...
  return 0 if $object->IsTypedArrayType($type) &&  $object->FileNamePrefix eq "V8";
