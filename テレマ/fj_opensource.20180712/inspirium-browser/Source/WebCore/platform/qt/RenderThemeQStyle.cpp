...
bool RenderThemeQStyle::paintButton(RenderObject* o, const PaintInfo& i, const IntRect& r)
{
    ...
    if (p.appearance == PushButtonPart || p.appearance == ButtonPart) {
        ...
        if (isIndeterminate(o)) {
            QBrush fill = QBrush(QColor(77,145,145), Qt::SolidPattern); 
            qDrawWinPanel(p.painter, p.styleOption.rect,  p.styleOption.palette, true, &fill);
        }
#endif
    }
    return false;
...
bool RenderThemeQStyle::paintSearchFieldDecorationPart(RenderObject* o, const PaintInfo& pi, const IntRect& r)
{
    notImplemented();
...
bool RenderThemeQStyle::paintSearchFieldResultsDecorationPart(RenderObject* o, const PaintInfo& pi, const IntRect& r)
{
    notImplemented();
...
ControlPart RenderThemeQStyle::initializeCommonQStyleOptions(QStyleFacadeOption &option, RenderObject* o) const
{
    ...
    if (o->style().direction() == WebCore::RTL)
...
/**/
        if (result == CheckboxPart && isIndeterminate(o))
        {
            option.state |= QStyleFacade::State_NoChange;
        }
        break;
/**/
    }
