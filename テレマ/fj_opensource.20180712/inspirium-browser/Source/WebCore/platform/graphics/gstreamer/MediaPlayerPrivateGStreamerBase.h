...
#if ENABLE(MEDIA_STREAM)
    virtual const uint8* localRendererBuffer() { return NULL; }
    virtual const int localRendererWidth() { return 0; }
    virtual const int localRendererHeight() { return 0; }

    virtual const uint8* remoteRendererBuffer() { return NULL; }; 
    virtual const int remoteRendererWidth() { return 0; } 
    virtual const int remoteRendererHeight() { return 0; }

    enum PaintType {PaintForNormalVideo, PaintForLocalRenderer, PaintForRemoteRenderer};
    void setPaintType(PaintType type){m_paintType = type;}

    rtc::scoped_refptr<webrtc::AudioTrackInterface> audioTrack_;

    static Mutex m_syncLock; //by chinhin
