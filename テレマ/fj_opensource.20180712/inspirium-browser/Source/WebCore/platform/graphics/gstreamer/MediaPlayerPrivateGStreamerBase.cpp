...
{
#if ENABLE(MEDIA_STREAM)
    MediaPlayerPrivateGStreamerBase *ptr = const_cast<MediaPlayerPrivateGStreamerBase *>(this); 
    if(!ptr)
        return IntSize();
    static int renderWidth = 0, renderHeight = 0;
    if(m_paintType == PaintForLocalRenderer) {
        if(ptr->localRendererWidth() || ptr->localRendererHeight()) {
            renderWidth = ptr->localRendererWidth();
            renderHeight = ptr->localRendererHeight();
        }
        return IntSize(renderWidth, renderHeight); 
    }

    if(m_paintType == PaintForRemoteRenderer) {
        if(ptr->remoteRendererWidth() || ptr->remoteRendererHeight()) {
            renderWidth = ptr->remoteRendererWidth();
            renderHeight = ptr->remoteRendererHeight();
        }
        return IntSize(renderWidth, renderHeight); 
    }
#endif
...
#if ENABLE(MEDIA_STREAM)
    if(audioTrack_)
        audioTrack_->GetSource()->SetVolume(volume);
#endif
...
void MediaPlayerPrivateGStreamerBase::notifyPlayerOfVolumeChange()
{
    ...
    double volume = 0;
#else
    double volume = gst_stream_volume_get_volume(m_volumeElement.get(), GST_STREAM_VOLUME_FORMAT_CUBIC);
#endif
...
//by chinhin
    ...
    if(m_paintType == PaintForLocalRenderer) {
        static rtc::scoped_ptr<uint8[]> lastRestoreLocalFrameData = NULL;
        static int width = 0, height = 0;

        //when click video control stop button ,localRendererBuffer is NULL
        //Then draw the lastRestoreLocalFrameData
        if(localRendererBuffer()) {
            width = localRendererWidth();
            height = localRendererHeight();

            lastRestoreLocalFrameData.reset(new uint8[width * height * 4]);
            memcpy(lastRestoreLocalFrameData.get(), localRendererBuffer(), width * height * 4);
        }
        if(lastRestoreLocalFrameData){ 
            QImage image(lastRestoreLocalFrameData.get(), width, height, QImage::Format_RGB32);
            QPainter *painter = context->platformContext();
            if(painter)
                painter->drawImage(rect, image);
        }     
        return;
    } else if(m_paintType == PaintForRemoteRenderer) {
        static rtc::scoped_ptr<uint8[]> lastRestoreRemoteFrameData = NULL;
        static int width = 0, height = 0;

        //when click video control stop button ,remoteRendererBuffer is NULL
        //Then draw the lastRestoreRemoteFrameData
        if(remoteRendererBuffer()) {
            width = remoteRendererWidth();
            height = remoteRendererHeight();

            lastRestoreRemoteFrameData.reset(new uint8[width * height * 4]);
            memcpy(lastRestoreRemoteFrameData.get(), remoteRendererBuffer(), width * height * 4);
        }
        if(lastRestoreRemoteFrameData){ 
            QImage image(lastRestoreRemoteFrameData.get(), width, height, QImage::Format_RGB32);
            QPainter *painter = context->platformContext();
            if(painter)
                painter->drawImage(rect, image);
        } 
