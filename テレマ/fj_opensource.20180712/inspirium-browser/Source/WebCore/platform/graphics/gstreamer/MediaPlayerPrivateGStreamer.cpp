...
//--
static void* gstP[2]={NULL,NULL};

static MediaPlayerPrivateGStreamer::GST_PermissionType sVideoPermission = MediaPlayerPrivateGStreamer::GST_PermissionType::Disallow;
static MediaPlayerPrivateGStreamer::GST_PermissionType sAudioPermission = MediaPlayerPrivateGStreamer::GST_PermissionType::Disallow;
bool MediaPlayerPrivateGStreamer::fjib_set_gstplayer_permission(GST_MediaType type,GST_PermissionType permission){
    bool ret = true;
    //setPermision
    if(type == Video){sVideoPermission = permission;}
    if(type == Audio){sAudioPermission = permission;}

    //stop playing state
    if(permission == Disallow){
        if(type == Video){
            MediaPlayerPrivateGStreamer* player = (MediaPlayerPrivateGStreamer*)fjib_get_gstplayer(Video);
            if(player)
            {
                ret = player->removeVideo();
            }
            else
            {
                ret = false;
            ...
            MediaPlayerPrivateGStreamer* player = (MediaPlayerPrivateGStreamer*)fjib_get_gstplayer(Audio);
            if(player)
            {
                ret = player->removeAudio();
            }
            else
            {
                ret = false;
...
bool MediaPlayerPrivateGStreamer::fjib_set_gstplayer(void* p, GST_MediaType type){
    if(p == NULL){
        gstP[type] = NULL;
        return true;
    }
...
void* MediaPlayerPrivateGStreamer::fjib_get_gstplayer(GST_MediaType type){
    if(type < 0 || type >= 2){
        return NULL;
    }
    return gstP[type];
...
bool MediaPlayerPrivateGStreamer::removeAudio(){
    if(!m_playBin){
        return false;
    }
    if(!m_hasAudio){
...
bool MediaPlayerPrivateGStreamer::removeVideo(){
    if(!m_playBin){
        return false;
    }
    if(!m_hasVideo){
...
{
//--
    if(m_player->mediaPlayerClient() && m_player->mediaPlayerClient()->mediaPlayerIsVideo())
    {
        fjib_set_gstplayer(this , Video);
    }
    else{
        fjib_set_gstplayer(this , Audio);
    }
//--
}
...
{
//--
    if(fjib_get_gstplayer(Video) == this){
        fjib_set_gstplayer(NULL, Video);
    }

    if(fjib_get_gstplayer(Audio) == this){
        fjib_set_gstplayer(NULL, Audio);
    }
//--
...
//--
    //check gst permission
    if(m_player->mediaPlayerClient() && m_player->mediaPlayerClient()->mediaPlayerIsVideo())
    {
        if(sVideoPermission == Disallow){
...
#if ENABLE(MEDIA_STREAM)
void MediaPlayerPrivateGStreamer::load(MediaStreamPrivate* streamPrivate)
{
    notImplemented();
}
#endif
...
#else
    m_autoAudioSink = gst_element_factory_make(fjib_get_audiosink(), 0);
    g_object_set(G_OBJECT(m_autoAudioSink.get()) , "device", fjib_get_device() , NULL);
#endif
//-
...
//--
    ...
    if(m_player->mediaPlayerClient() && !m_player->mediaPlayerClient()->mediaPlayerIsVideo()){
        g_object_set(m_playBin.get(), "audio-sink", createAudioSink(), nullptr);
    }
    if(m_webkitVideoSink){
        GRefPtr<GstPad> videoSinkPad = adoptGRef(gst_element_get_static_pad(m_webkitVideoSink.get(), "sink"));
        if (videoSinkPad)
