...
public:
    ...
    WEBCORE_API void coverWithTilesIfNeeded();

    float contentsScale() { return m_contentsScale; }
    WEBCORE_API void setContentsScale(float);

    bool contentsFrozen() const { return m_contentsFrozen; }
    WEBCORE_API void setContentsFrozen(bool);

    void updateTileBuffers();

    WEBCORE_API void invalidate(const IntRect& dirtyRect);
    WEBCORE_API void paint(GraphicsContext*, const IntRect&);
