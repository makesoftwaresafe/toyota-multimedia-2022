                                 ...
                                 QFontEngine::ShaperFlags flags) const
{
    ...
    if (freetype->symbol_map) {
        ...
        QStringIterator it(str, str + len);
#if 0
		uint alternative_uc = 0x20;//fjib_get_alternative_char();
#endif
        while (it.hasNext()) {
            uint uc = it.next();
			uint o_uc = uc;
            ...
            if (!glyphs->glyphs[glyph_pos]) {
                ...
                redo:
                    ...
                    //printf("uc[%c]-", uc);
                    //}
                    //printf("uc[%x],glyph[%d],glyph_pos[%d],cmapc[%d]\n", uc,glyph,glyph_pos, freetype->cmapCache[uc]);fflush(stdout);
                    if (!glyph && (uc == 0xa0 || uc == 0x9)) {
                        uc = 0x20;
                        goto redo;
                    }
#if 0
					else if(!glyph && (uc == 0x20)) {
						qWarning("No glyph : code %X.", uc);
					}
					else if(!glyph && (uc == alternative_uc)) {
						qWarning("Specified alternative code %X for %X was not found. Use 0x20.", alternative_uc, o_uc);
						uc = 0x20;
						goto redo;
					}
					else if(!glyph
							&& !(uc == 0x200b || uc == 0x200c || uc == 0x200d || uc == 0xfeff)
							&& !(uc < QFreetypeFace::cmapCacheSize)
							)
					{
						uc = alternative_uc;
						goto redo;
