...
{
#ifdef USE_FJIB_FONT_INFO
    QString fontpath = QString::fromUtf8(fjib_font_path());
	bool useFixedFontFile = fjib_use_fixed_font_file();
	if(useFixedFontFile) {
		// only one font file will be added.
		QString fixedAbsFileName = fontpath + QString::fromUtf8("/") + QString::fromUtf8(fjib_font_file());
		QFile f(fixedAbsFileName);
		if(!f.exists()) {
			qFatal("Invalid font file specified: %s.", qPrintable(fixedAbsFileName));
		}
		else {
			const QByteArray file = QFile::encodeName(fixedAbsFileName);
...
QString QBasicFontDatabase::fontNameFromTTFile(const QString &filename)
{
	...
	bool useFixedFontFile = fjib_use_fixed_font_file();
	if(useFixedFontFile) {
		QString fixedAbsFileName = QString::fromUtf8(fjib_font_path()) + QString::fromUtf8("/") + QString::fromUtf8(fjib_font_file());
		f.setFileName(fixedAbsFileName);
		if(!f.exists()) {
			qFatal("Invalid font file specified: %s.", qPrintable(fixedAbsFileName));
		}
		else {
