...
static QByteArray ucalDefaultTimeZoneId()
{
    ...
    size = insp_ucal_getDefaultTimeZone(reinterpret_cast<UChar *>(result.data()), size, &status);
                                       ...
                                       const QString &localeCode)
{
    ...
    size = insp_ucal_getTimeZoneDisplayName(ucal,
                              ...
                              int *utcOffset, int *dstOffset)
{
    ...
    *dstOffset = 0;
    ...
    UErrorCode status = U_ZERO_ERROR;
    UCalendar *ucal = insp_ucal_clone(m_ucal, &status);
    if (!U_SUCCESS(status))
        return false;

    // Set the date to find the offset for
    status = U_ZERO_ERROR;
    insp_ucal_setMillis(ucal, atMSecsSinceEpoch, &status);

    int32_t utc = 0;
    if (U_SUCCESS(status)) {
        status = U_ZERO_ERROR;
        // Returns msecs
        utc = insp_ucal_get(ucal, UCAL_ZONE_OFFSET, &status) / 1000;
    }
    ...
    if (U_SUCCESS(status)) {
        status = U_ZERO_ERROR;
        // Returns msecs
        dst = insp_ucal_get(ucal, UCAL_DST_OFFSET, &status) / 1000;
    }

    insp_ucal_close(ucal);
    if (U_SUCCESS(status)) {
        *utcOffset = utc;
        *dstOffset = dst;
        return true;
                                                     ...
                                                     qint64 atMSecsSinceEpoch)
{
    ...
    if (U_SUCCESS(status) && ok) {
        status = U_ZERO_ERROR;
        insp_ucal_setMillis(ucal, tranMSecs, &status);
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    const int32_t dstMSecs = insp_ucal_getDSTSavings(reinterpret_cast<const UChar *>(id.data()), &status);
    if (U_SUCCESS(status))
    ...
    : QTimeZonePrivate(other), m_ucal(0)
{
    ...
    UErrorCode status = U_ZERO_ERROR;
    m_ucal = insp_ucal_clone(other.m_ucal, &status);
    if (!U_SUCCESS(status)) {
        m_id.clear();
        m_ucal = 0;
    }
...
void QIcuTimeZonePrivate::init(const QByteArray &ianaId)
{
    ...
    m_ucal = insp_ucal_open(reinterpret_cast<const UChar *>(id.data()), id.size(),
...
bool QIcuTimeZonePrivate::isDaylightTime(qint64 atMSecsSinceEpoch) const
{
    ...
    if (U_SUCCESS(status)) {
        status = U_ZERO_ERROR;
        result = insp_ucal_inDaylightTime(ucal, &status);
    }
...
{
    UErrorCode status = U_ZERO_ERROR;
    UEnumeration *uenum = insp_ucal_openTimeZones(&status);
    QSet<QByteArray> set;
    if (U_SUCCESS(status))
...
QSet<QByteArray> QIcuTimeZonePrivate::availableTimeZoneIds(QLocale::Country country) const
{
    ...
    UErrorCode status = U_ZERO_ERROR;
    UEnumeration *uenum = insp_ucal_openCountryTimeZones(regionCode, &status);
    QSet<QByteArray> set;
    if (U_SUCCESS(status))
        set = uenumToIdSet(uenum);
    uenum_close(uenum);
...
// TODO Available directly in C++ api but not C api, from 4.8 onwards new filter method works
    ...
    UErrorCode status = U_ZERO_ERROR;
    UEnumeration *uenum = insp_ucal_openTimeZoneIDEnumeration(UCAL_ZONE_TYPE_ANY, 0,
