...
QHostInfo QHostInfoAgent::fromName(const QString &hostName)
{
	...
	DEBUG_SOCKET_QDEBUG("QHostInfoAgent::fromName(%s) looking up...",
...
# endif
    ...
    if (result == 0) {
        ...
        while (node) {
#ifdef FJ_INSP_NETWORKLOG
			DEBUG_SOCKET_QDEBUG("getaddrinfo node: flags: %d, family: %d, ai_socktype: %d ,ai_protocol: %d, ai_addrlen: %d", 
				node->ai_flags, node->ai_family, node->ai_socktype, node->ai_protocol, node->ai_addrlen);
#endif
    ...
    if (results.error() != QHostInfo::NoError) {
		DEBUG_SOCKET_QDEBUG("QHostInfoAgent::fromName(): error #%d %s",
		...
		DEBUG_SOCKET_QDEBUG("QHostInfoAgent::fromName(): found %i entries for \"%s\": {%s}",
