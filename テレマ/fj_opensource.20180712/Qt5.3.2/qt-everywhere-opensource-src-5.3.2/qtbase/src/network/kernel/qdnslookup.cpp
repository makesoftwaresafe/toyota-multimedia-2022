...
static void qt_qdnsservicerecord_sort(QList<QDnsServiceRecord> &records)
{
    ...
    while (i < records.size()) {
        ...
        DEBUG_SOCKET_QDEBUG2("qt_qdnsservicerecord_sort() : priority %i (size: %i, total weight: %i)",
...
#ifdef FJ_INSP_NETWORKLOG
                    DEBUG_SOCKET_QDEBUG2("qt_qdnsservicerecord_sort() : adding %s %i (weight: %i)",
                    	qPrintable(slice[j].target()), slice[j].port(), slice[j].weight());
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("DNS reply for %s: %i (%s)", qPrintable(name), _reply.error, qPrintable(_reply.errorString));
#endif
        reply = _reply;
        runnable = 0;
        isFinished = true;
...
void QDnsLookupThreadPool::start(QRunnable *runnable)
{
    ...
    if (!signalsConnected) {
        ...
        if (!signalsConnected) {
            ...
            if (!app) {
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("QDnsLookup requires a QCoreApplication");
#endif
                delete runnable;
                return;
            }
