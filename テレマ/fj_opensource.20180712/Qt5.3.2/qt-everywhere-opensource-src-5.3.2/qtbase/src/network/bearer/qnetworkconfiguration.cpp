...
static QNetworkConfiguration::BearerType cellularStatus()
{
    ...
    if ((cellularStatusFD = qt_safe_open(cellularStatusFile, O_RDONLY)) == -1) {
        ...
        DEBUG_SOCKET_QDEBUG2("%s failed to open %s", Q_FUNC_INFO, cellularStatusFile);
#endif
        return ret;
    }
    char buf[2048];
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s read from PPS file failed: %s", Q_FUNC_INFO, strerror(errno));
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s failed to initialize PPS decoder", Q_FUNC_INFO);
#endif
        qt_safe_close(cellularStatusFD);
        return ret;
    }
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s pps_decoder_push failed %d", Q_FUNC_INFO, err);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s field has not the expected data type", Q_FUNC_INFO);
#endif
        pps_decoder_cleanup(&ppsDecoder);
        qt_safe_close(cellularStatusFD);
        return ret;
    ...
    if (!pps_decoder_get_int(&ppsDecoder, "network_technology", &type)
            == PPS_DECODER_OK) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s could not read bearer type from PPS", Q_FUNC_INFO);
#endif
        pps_decoder_cleanup(&ppsDecoder);
        qt_safe_close(cellularStatusFD);
        return ret;
    ...
    default:
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s unhandled bearer type %d", Q_FUNC_INFO, type);
#endif
        break;
    }
    pps_decoder_cleanup(&ppsDecoder);
...
QNetworkConfiguration::BearerType QNetworkConfiguration::bearerTypeFamily() const
{
    ...
    default:
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("unknown bearer type %d", type);
#endif
