...
void QTcpServerPrivate::readNotification()
{
    ...
    for (;;) {
        if (pendingConnections.count() >= maxConnections) {
            ...
            DEBUG_SOCKET_QDEBUG2("QTcpServerPrivate::_q_processIncomingConnection() too many connections");
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QTcpServerPrivate::_q_processIncomingConnection() accepted socket %i", descriptor);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG3("QTcpServer::listen() called when already listening");
#endif
        return false;
    }
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QTcpServer::listen(%i, \"%s\") == true (listening on port %i)", port,
...
bool QTcpServer::setSocketDescriptor(qintptr socketDescriptor)
{
    ...
    if (isListening()) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG3("QTcpServer::setSocketDescriptor() called when already listening");
#endif
        return false;
    }
    ...
    if (!d->socketEngine->initialize(socketDescriptor, QAbstractSocket::ListeningState)) {
        ...
        d->serverSocketErrorString = d->socketEngine->errorString();
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QTcpServer::setSocketDescriptor(%i) failed (%s)", socketDescriptor,
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QTcpServer::setSocketDescriptor(%i) succeeded.", socketDescriptor);
#endif
    return true;
}
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QTcpServer::incomingConnection(%i)", socketDescriptor);
#endif
