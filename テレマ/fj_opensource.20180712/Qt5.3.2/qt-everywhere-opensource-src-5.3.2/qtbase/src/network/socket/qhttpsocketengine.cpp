...
bool QHttpSocketEngine::connectInternal()
{
    ...
    if (d->state == Connected) {
        ...
        DEBUG_SOCKET_QDEBUG2("QHttpSocketEngine::connectToHost: called when already connected");
#endif
...
qint64 QHttpSocketEngine::read(char *data, qint64 maxlen)
{
    ...
    if (bytesRead == -1) {
        ...
        close();
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG( "QHttpSocketEngine::read-error bytesRead==-1- RemoteHostClosedError :socketEngine=%x" , this);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QHttpSocketEngine::slotSocketError: got weird error = %d", error);
#endif
	}
