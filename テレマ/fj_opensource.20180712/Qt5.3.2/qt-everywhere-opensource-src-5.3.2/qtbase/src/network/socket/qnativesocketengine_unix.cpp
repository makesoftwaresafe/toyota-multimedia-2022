...
#ifdef FJ_INSP_NETWORKLOG
   	DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::createNewSocket():dscr=%d :socketEngine=%x",socket,this);
#endif
...
bool QNativeSocketEnginePrivate::setOption(QNativeSocketEngine::SocketOption opt, int v)
{
    ...
    case QNativeSocketEngine::NonBlockingSocketOption: {
        ...
        if (flags == -1) {
#ifdef FJ_INSP_NETWORKLOG
			DEBUG_SOCKET_PERROR("QNativeSocketEnginePrivate::setOption(): fcntl(F_GETFL) failed");
#endif
            return false;
        }
        if (::fcntl(socketDescriptor, F_SETFL, flags | O_NONBLOCK) == -1) {
#ifdef FJ_INSP_NETWORKLOG
			DEBUG_SOCKET_PERROR("QNativeSocketEnginePrivate::setOption(): fcntl(F_SETFL) failed");
#endif
            return false;
        }
#else // Q_OS_VXWORKS
...
#ifdef FJ_INSP_NETWORKLOG
			DEBUG_SOCKET_PERROR("QNativeSocketEnginePrivate::setOption(): ioctl(FIONBIO, 1) failed");
#endif
            return false;
        }
#endif // Q_OS_VXWORKS
...
{
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeConnect() : dscr=%d ", socketDescriptor);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeConnect(%s, %i) == false (%s) :dscr=%d",
            	addr.toString().toLatin1().constData(), port,
            	socketState == QAbstractSocket::ConnectingState 
				? "Connection in progress" : socketErrorString.toLatin1().constData(), socketDescriptor);
#endif
            return false;
        }
    }
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeConnect(%s, %i) == true : dscr=%d",
    	addr.toString().toLatin1().constData(), port, socketDescriptor);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeBind(%s, %i) == false (%s):dscr=%d",
        	address.toString().toLatin1().constData(), port, socketErrorString.toLatin1().constData(),socketDescriptor);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeBind(%s, %i) == true :dscr=%d",
    	address.toString().toLatin1().constData(), port,socketDescriptor);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeListen(%i) == false (%s):dscr=%d",
        	backlog, socketErrorString.toLatin1().constData(), socketDescriptor);
#endif
        return false;
    }

#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeListen(%i) == true:dscr=%d", backlog, socketDescriptor);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeBytesAvailable() == %lli:dscr=%d", available, socketDescriptor);
#endif
    return available;
}
...
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeHasPendingDatagrams() == %s:dscr=%d",
    	result ? "true" : "false", socketDescriptor);
#endif
    return result;
}
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativePendingDatagramSize() == %i:dscr=%d", recvResult, socketDescriptor);
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeReceiveDatagram(%p \"%s\", %lli, %s, %i) == %lli:dscr=%d",
    	data, qt_prettyDebug(data, qMin(recvFromResult, ssize_t(16)), recvFromResult).data(), maxSize,
    	address ? address->toString().toLatin1().constData() : "(nil)",
    	port ? *port : 0, (qint64) recvFromResult, socketDescriptor);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG("QNativeSocketEngine::sendDatagram(%p \"%s\", %lli, \"%s\", %i) == %lli:dscr=%d", data,
    	qt_prettyDebug(data, qMin<int>(len, 16), len).data(), len, host.toString().toLatin1().constData(),
    	port, (qint64) sentBytes, socketDescriptor);
#endif
...
bool QNativeSocketEnginePrivate::fetchConnectionParameters()
{
...
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("unexpected size of IPV6_V6ONLY socket option");
#endif
			}
            if (!ipv6only) {
                socketProtocol = QAbstractSocket::AnyIPProtocol;
    ...
    DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::fetchConnectionParameters() local == %s:%i,"
    	" peer == %s:%i, socket == %s - %s:dscr=%d",
    	...
    	socketProtocolStr.toLatin1().constData(), socketDescriptor);
#endif
    return true;
}
...
{
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG("QNativeSocketEngine::nativeClose():dscr=%d",socketDescriptor);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeWrite(%p \"%s\", %llu) == %i:dscr=%d",
    	data, qt_prettyDebug(data, qMin((int) len, 16),
    	(int) len).data(), len, (int) writtenBytes, socketDescriptor);
#endif
...
qint64 QNativeSocketEnginePrivate::nativeRead(char *data, qint64 maxSize)
{
    ...
    if (!q->isValid()) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNativeSocketEngine::nativeRead: Invalid socket");
#endif
        return -1;
    }
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG("QNativeSocketEnginePrivate::nativeRead(%p \"%s\", %llu) == %i:dscr=%d",
    	data, qt_prettyDebug(data, qMin(r, ssize_t(16)), r).data(),
    	maxSize, r, socketDescriptor);
