...
#define QT_CHECK_BOUND(function, a) do { \
    if (!isValid()) { \
        DEBUG_SOCKET_QDEBUG2(function" called on a QUdpSocket when not in QUdpSocket::BoundState"); \
        return (a); \
    } } while (0)
#endif
...
void QUdpSocket::setMulticastInterface(const QNetworkInterface &iface)
{
    ...
    if (!isValid()) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QUdpSocket::setMulticastInterface() called on a QUdpSocket when not in QUdpSocket::BoundState");
#endif
        return;
    }
                                  ...
                                  quint16 port)
{
    Q_D(QUdpSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QUdpSocket::writeDatagram(%p, %llu, \"%s\", %i)", data, size,
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QUdpSocket::readDatagram(%p, %llu, %p, %p)", data, maxSize, address, port);
#endif
#ifdef FJ_INSP_NETWORKLOG
