...
#define Q_CHECK_VALID_SOCKETLAYER(function, returnValue) do { \
    if (!isValid()) { \
        DEBUG_SOCKET_QDEBUG3(""#function" was called on an uninitialized socket device"); \
        return returnValue; \
    } } while (0)
#define Q_CHECK_INVALID_SOCKETLAYER(function, returnValue) do { \
    if (isValid()) { \
        DEBUG_SOCKET_QDEBUG3(""#function" was called on an already initialized socket device"); \
        return returnValue; \
    } } while (0)
#define Q_CHECK_STATE(function, checkState, returnValue) do { \
    if (d->socketState != (checkState)) { \
        DEBUG_SOCKET_QDEBUG2(""#function" was not called in "#checkState); \
        return (returnValue); \
    } } while (0)
#define Q_CHECK_NOT_STATE(function, checkState, returnValue) do { \
    if (d->socketState == (checkState)) { \
        DEBUG_SOCKET_QDEBUG2(""#function" was called in "#checkState); \
        return (returnValue); \
    } } while (0)
#define Q_CHECK_STATES(function, state1, state2, returnValue) do { \
    if (d->socketState != (state1) && d->socketState != (state2)) { \
        DEBUG_SOCKET_QDEBUG2(""#function" was called" \
                 " not in "#state1" or "#state2); \
        return (returnValue); \
    } } while (0)
#define Q_CHECK_TYPE(function, type, returnValue) do { \
    if (d->socketType != (type)) { \
        DEBUG_SOCKET_QDEBUG2(#function" was called by a" \
...
bool QNativeSocketEngine::initialize(QAbstractSocket::SocketType socketType, QAbstractSocket::NetworkLayerProtocol protocol)
{
    ...
    if (!d->createNewSocket(socketType, protocol)) {
		...
		DEBUG_SOCKET_QDEBUG("QNativeSocketEngine::initialize(type == %s, protocol == %s) failed: %s :socketEngine=%x",
               typeStr.toLatin1().constData(), protocolStr.toLatin1().constData(), d->socketErrorString.toLatin1().constData(),this);
#endif
        return false;
    }
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNativeSocketEngine::initialize unable to inline out-of-band data");
#endif
    }
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG("QNativeSocketEngine::initialize(socketDescriptor == %i) failed: %s :socketEngine=%x",
        	socketDescriptor, d->socketErrorString.toLatin1().constData(),this);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QAbstractSocket: cannot bind to QHostAddress::Any (or an IPv6 address) and join an IPv4 multicast group");
#endif
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QAbstractSocket: bind to QHostAddress::AnyIPv4 instead if you want to do this");
#endif
        return false;
    }
...
#ifdef FJ_INSP_NETWORKLOG
		DEBUG_SOCKET_QDEBUG( "QNativeSocketEngine::read-error - RemoteHostClosedError :socketEngine=%x",this);
#endif
