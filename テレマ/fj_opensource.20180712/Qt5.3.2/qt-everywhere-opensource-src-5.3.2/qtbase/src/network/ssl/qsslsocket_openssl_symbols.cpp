...
void qsslSocketUnresolvedSymbolWarning(const char *functionName)
{
    ...
    DEBUG_SOCKET_QDEBUG2("QSslSocket: cannot call unresolved function %s", functionName);
#endif
}
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket: cannot resolve %s", functionName);
#endif
}
}
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket: unable to resolve symbols. "
             "QT_NO_LIBRARY is defined which means runtime resolving of "
             "libraries won't work.");
    DEBUG_SOCKET_QDEBUG2("Either compile Qt statically or with support for runtime resolving "
             "of libraries.");
#endif
    return false;
}
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("unsupported date format detected");
#endif
        return QDateTime();
    }
