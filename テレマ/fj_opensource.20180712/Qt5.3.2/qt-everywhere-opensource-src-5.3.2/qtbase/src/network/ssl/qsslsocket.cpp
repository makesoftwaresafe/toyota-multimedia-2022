    ...
    : QTcpSocket(*new QSslSocketBackendPrivate, parent)
{
    ...
    DEBUG_SOCKET_QDEBUG2("QSslSocket::QSslSocket(%p), this = %x", parent, (void *)this);
#endif
    d->q_ptr = this;
    d->init();
}
...
QSslSocket::~QSslSocket()
{
    Q_D(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::~QSslSocket(), this = %x", (void *)this);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG3("QSslSocket::connectToHostEncrypted() called when already connecting/connected");
#endif
        return;
    }
...
bool QSslSocket::setSocketDescriptor(qintptr socketDescriptor, SocketState state, OpenMode openMode)
{
    Q_D(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::setSocketDescriptor(%i, %d, %d)", socketDescriptor, (int)state, (int)openMode);
#endif
    if (!d->plainSocket)
...
void QSslSocket::setPeerVerifyDepth(int depth)
{
    ...
    if (depth < 0) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QSslSocket::setPeerVerifyDepth: cannot set negative depth of %d", depth);
#endif
        return;
    }
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::close()");
#endif
    Q_D(QSslSocket);
...
bool QSslSocket::flush()
{
    Q_D(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::flush()");
#endif
    if (d->mode != UnencryptedMode)
...
void QSslSocket::abort()
{
    Q_D(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::abort()");
#endif
    if (d->plainSocket)
        d->plainSocket->abort();
    close();
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QSslSocket::waitForDisconnected() is not allowed in UnconnectedState");
#endif
        return false;
    }
...
void QSslSocket::startClientEncryption()
{
    ...
    if (d->mode != UnencryptedMode) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QSslSocket::startClientEncryption: cannot start handshake on non-plain connection");
#endif
        return;
    }
    if (state() != ConnectedState) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QSslSocket::startClientEncryption: cannot start handshake when not connected");
#endif
        return;
    }
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG3("QSslSocket::startClientEncryption()");
#endif
...
void QSslSocket::startServerEncryption()
{
    ...
    if (d->mode != UnencryptedMode) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QSslSocket::startServerEncryption: cannot start handshake on non-plain connection");
#endif
        return;
    }
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG3("QSslSocket::startServerEncryption()");
#endif
...
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG2("QSslSocket::connectToHost(%s, %i, %d)", hostName.toUtf8().constData(), port, (int)openMode);
#endif
    if (!d->plainSocket) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("\tcreating internal plain socket");
#endif
        d->createPlainSocket(openMode);
    }
...
void QSslSocket::disconnectFromHost()
{
    Q_D(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::disconnectFromHost()");
#endif
    if (!d->plainSocket)
        return;
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::readData(%p,%d) == %lld", (void *)data, maxlen, readBytes);
#endif
...
qint64 QSslSocket::writeData(const char *data, qint64 len)
{
    Q_D(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::writeData(%p,%lld)", (void *)data, len);
#endif
    if (d->mode == UnencryptedMode && !d->autoStartHandshake)
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::_q_connectedSlot()");
    DEBUG_SOCKET_QDEBUG2("\tstate = %d", (int)(q->state()));
    DEBUG_SOCKET_QDEBUG2("\tpeer = %s %s %i", q->peerName().toUtf8().constData(), q->peerAddress().toString().toUtf8().constData(), q->peerPort());
    DEBUG_SOCKET_QDEBUG2("\tlocal = %s %s %i", QHostInfo::fromName(q->localAddress().toString()).hostName().toUtf8().constData(), q->localAddress().toString().toUtf8().constData(), q->localPort());
#endif
...
void QSslSocketPrivate::_q_hostFoundSlot()
{
    Q_Q(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::_q_hostFoundSlot()");
    DEBUG_SOCKET_QDEBUG2("\tstate = %d", (int)(q->state()));
#endif
    emit q->hostFound();
}
...
void QSslSocketPrivate::_q_disconnectedSlot()
{
    Q_Q(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::_q_disconnectedSlot()");
    DEBUG_SOCKET_QDEBUG2("\tstate = %d", (int)(q->state()));
#endif
    disconnected();
    emit q->disconnected();
...
void QSslSocketPrivate::_q_stateChangedSlot(QAbstractSocket::SocketState state)
{
    Q_Q(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::_q_stateChangedSlot(%d)", (int)state);
#endif
...
void QSslSocketPrivate::_q_errorSlot(QAbstractSocket::SocketError error)
{
    Q_Q(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::_q_errorSlot(%d)", (int)error);
    DEBUG_SOCKET_QDEBUG2("\tstate = %d", (int)(q->state()));
    DEBUG_SOCKET_QDEBUG2("\terrorString = %s", q->errorString().toUtf8().constData());
#endif
...
void QSslSocketPrivate::_q_readyReadSlot()
{
    Q_Q(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::_q_readyReadSlot() -%llu bytes available", plainSocket->bytesAvailable());
#endif
...
void QSslSocketPrivate::_q_bytesWrittenSlot(qint64 written)
{
    Q_Q(QSslSocket);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QSslSocket::_q_bytesWrittenSlot(%lld)", written);
#endif
