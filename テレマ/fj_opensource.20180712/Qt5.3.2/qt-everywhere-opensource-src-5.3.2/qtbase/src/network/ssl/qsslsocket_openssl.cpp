...
int q_X509Callback(int ok, X509_STORE_CTX *ctx)
{
    if (!ok) {
        ...
        DEBUG_SOCKET_QDEBUG2("verification error: dumping bad certificate");
        DEBUG_SOCKET_QDEBUG2("%s", QSslCertificatePrivate::QSslCertificate_from_X509(q_X509_STORE_CTX_get_current_cert(ctx)).toPem().constData());
        DEBUG_SOCKET_QDEBUG2("dumping chain");
        foreach (QSslCertificate cert, QSslSocketBackendPrivate::STACKOFX509_to_QSslCertificates(q_X509_STORE_CTX_get_chain(ctx))) {
            ...
            qDebug() << "Issuer:" << "O=" << cert.issuerInfo(QSslCertificate::Organization)
                ...
                << "ST=" << cert.issuerInfo(QSslCertificate::StateOrProvinceName); // return QStringList
            qDebug() << "Subject:" << "O=" << cert.subjectInfo(QSslCertificate::Organization)
                ...
                << "ST=" << cert.subjectInfo(QSslCertificate::StateOrProvinceName); // return QStringList
...
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("could not set SSL_CTRL_SET_TLSEXT_HOSTNAME, Server Name Indication disabled");
#endif
			}
        }
    }
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("could not resolve symbols in security library"); // should never happen
#endif
		}
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("could not load security library");
#endif
    }
#elif defined(Q_OS_WIN)
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("could not resolve symbols in crypt32 library"); // should never happen
#endif
		}
    } else {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("could not load crypt32 library"); // should never happen
#endif
    }
    ...
    if (ptrSecCertificateCopyData) {
        ...
        if (!status) {
            ...
            for (CFIndex i = 0; i < size; ++i) {
                ...
                if (data == NULL) {
#ifdef FJ_INSP_NETWORKLOG
                    DEBUG_SOCKET_QDEBUG2("error retrieving a CA certificate from the system store");
#endif
                } else {
           ...
           DEBUG_SOCKET_QDEBUG2("could not retrieve system CA certificates");
#endif
        }
    }
#elif defined(Q_OS_WIN)
...
#endif
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("systemCaCertificates retrieval time %dms", timer.elapsed());
    DEBUG_SOCKET_QDEBUG2("imported %d certificates", systemCerts.count());
#endif
...
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("QSslSocketBackendPrivate::transmit: encrypted %dbytes", writtenBytes);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("QSslSocketBackendPrivate::transmit: wrote %d encrypted bytes to the socket %lld actual.", encryptedBytesRead, actualWritten);
#endif
            if (actualWritten < 0) {
...
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("QSslSocketBackendPrivate::transmit: read %d encrypted bytes from the socket", encryptedBytesRead);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG3("QSslSocketBackendPrivate::transmit: testing encryption");
#endif
            if (startHandshake()) {
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG3("QSslSocketBackendPrivate::transmit: encryption established");
#endif
                connectionEncrypted = true;
                transmitting = true;
            } else if (plainSocket->state() != QAbstractSocket::ConnectedState) {
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("QSslSocketBackendPrivate::transmit: connection lost");
#endif
                break;
            } else if (paused) {
...
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("QSslSocketBackendPrivate::transmit: encryption not done yet");
#endif
            }
        }
...
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("QSslSocketBackendPrivate::transmit: decrypted %d bytes", readBytes);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("QSslSocketBackendPrivate::transmit: remote disconnect");
#endif
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("QSslSocketBackendPrivate::startHandshake: error! %s", q->errorString().toUtf8().constData());
#endif
    ...
    if (!errors.isEmpty()) {
        ...
        if (doVerifyPeer
            ...
            for (int i=0; i< sslErrors.count(); i++) {
                ...
                default:
#ifdef FJ_INSP_NETWORKLOG
                    DEBUG_SOCKET_QDEBUG2("%s", sslErrors.at(i).errorString().toUtf8().constData());
#endif
                    break;
                }
            }
...
void QWindowsCaRootFetcher::start()
{
    ...
    if (!wincert) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QWindowsCaRootFetcher failed to convert certificate to windows form");
#endif
    ...
    BOOL result = CertGetCertificateChain(
        ...
        &chain);
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QWindowsCaRootFetcher %lld ms to get chain", stopwatch.elapsed());
#endif

    QSslCertificate trustedRoot;
    if (result) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG3("QWindowsCaRootFetcher - examining windows chains");
#endif
#ifdef FJ_INSP_NETWORKLOG
        if (chain->TrustStatus.dwErrorStatus == CERT_TRUST_NO_ERROR)
            DEBUG_SOCKET_QDEBUG2(" - TRUSTED");
        else
            DEBUG_SOCKET_QDEBUG2(" - NOT TRUSTED %d", chain->TrustStatus.dwErrorStatus);
        if (chain->TrustStatus.dwInfoStatus & CERT_TRUST_IS_SELF_SIGNED)
            DEBUG_SOCKET_QDEBUG2(" - SELF SIGNED");
        DEBUG_SOCKET_QDEBUG2("QSslSocketBackendPrivate::fetchCaRootForCert - dumping simple chains");
        for (unsigned int i = 0; i < chain->cChain; i++) {
            if (chain->rgpChain[i]->TrustStatus.dwErrorStatus == CERT_TRUST_NO_ERROR)
                DEBUG_SOCKET_QDEBUG2(" - TRUSTED SIMPLE CHAIN %d", i);
            else
                DEBUG_SOCKET_QDEBUG2(" - UNTRUSTED SIMPLE CHAIN %d reason: %d", i, chain->rgpChain[i]->TrustStatus.dwErrorStatus);
            for (unsigned int j = 0; j < chain->rgpChain[i]->cElement; j++) {
                ...
                DEBUG_SOCKET_QDEBUG2("   - %p ", foundCert); // 161007 TODO fix for Win
            }
        }
        DEBUG_SOCKET_QDEBUG2(" - and %d lowquality chains", chain->cLowerQualityChainContext); //expect 0, we haven't asked for them // 161007 TODO fix for Win
#endif
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("could not open file %s for appending", sslKeyFile.toUtf8().constData());
#endif
		}
        if (!file.write(debugLineClientRandom)) {
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("could not write to file %s", sslKeyFile.toUtf8().constData());
#endif
		}
        file.close();
    } else {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("could not decrypt SSL traffic");
#endif
    }
#endif
...
QList<QSslError> QSslSocketBackendPrivate::verify(QList<QSslCertificate> certificateChain, const QString &hostName)
{
    ...
    if (!certStore) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("Unable to create certificate store");
#endif
        errors << QSslError(QSslError::UnspecifiedError);
        return errors;
    }
