...
//ASynchronously request's time out
static unsigned long s_requestTimeoutSec = 0;
//Synchronously request's time out
static unsigned long s_syncRequestTimeoutSec = 30;
...
void QNetworkAccessManager::setTimeOut(unsigned long second)
{
    s_requestTimeoutSec = second;
}

unsigned long QNetworkAccessManager::timeOut()
{
    return s_requestTimeoutSec;
}

void QNetworkAccessManager::setSynchronousTimeout(unsigned long second)
{
    s_syncRequestTimeoutSec = second;
}

unsigned long QNetworkAccessManager::synchronousTimeout()
{
    return s_syncRequestTimeoutSec;
}
...
QList<QNetworkProxy> QNetworkAccessManagerPrivate::queryProxy(const QNetworkProxyQuery &query)
{
    ...
    if (proxyFactory) {
        ...
        if (proxies.isEmpty()) {
            ...
            DEBUG_SOCKET_QDEBUG2("QNetworkAccessManager: factory %p has returned an empty result set", proxyFactory);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("could not open device for reading");
#endif
			}
        } else {
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("device is not readable");
#endif
        }
    }
...
void QNetworkAccessManager::showOutputHeaderRequestResponseLog(QNetworkReply *reply)
{
//#ifdef FJ_INSP_NETWORKLOG
	QUrl url = reply->request().url();

	//CFjInspiriumLogger* logger = CFjInspiriumLogger::getInstance();

	qInspiriumNetworkLogger(" *** Header information for %s", url.toString().toUtf8().data());
	qInspiriumNetworkLogger("=== [REQUEST] ===");
	QNetworkRequest req = reply->request();
	QList<QByteArray> requestHeaders = req.rawHeaderList();
	for(int i = 0; i < requestHeaders.size(); i++) {
		const char* headerValue = req.rawHeader(requestHeaders[i]).constData();
		qInspiriumNetworkLogger("%s: %s", requestHeaders[i].data(), headerValue);
	}

	qInspiriumNetworkLogger("=================");

	qInspiriumNetworkLogger("=== [RESPONSE] ===");
	...
	qInspiriumNetworkLogger("Status: %d", v.toInt());
	for(int i = 0; i < list.size(); i++) {
		QNetworkReply::RawHeaderPair p = list[i];
		const char* aHeader = p.first.constData();
		const char* aValue = p.second.constData();
		qInspiriumNetworkLogger("%s: %s", aHeader, aValue);
	}
	qInspiriumNetworkLogger("=================");
//#endif
}
