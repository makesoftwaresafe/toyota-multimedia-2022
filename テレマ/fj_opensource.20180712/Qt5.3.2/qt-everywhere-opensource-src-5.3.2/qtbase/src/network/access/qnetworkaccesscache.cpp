...
void QNetworkAccessCache::addEntry(const QByteArray &key, CacheableObject *entry)
{
    ...
    if (node.useCount) {
        ...
        DEBUG_SOCKET_QDEBUG2("QNetworkAccessCache::addEntry: overriding active cache entry '%s'",
        	key.constData());
#endif
	}
    if (node.object)
...
void QNetworkAccessCache::releaseEntry(const QByteArray &key)
{
    ...
    if (it == hash.end()) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkAccessCache::releaseEntry: trying to release key '%s' that is not in cache",
        	key.constData());
#endif
        return;
    }
...
void QNetworkAccessCache::removeEntry(const QByteArray &key)
{
    ...
    if (it == hash.end()) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkAccessCache::removeEntry: trying to remove key '%s' that is not in cache",
        	key.constData());
#endif
        return;
    }
    ...
    if (node->useCount > 1) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkAccessCache::removeEntry: removing active cache entry '%s'",
        	key.constData());
#endif
	}
