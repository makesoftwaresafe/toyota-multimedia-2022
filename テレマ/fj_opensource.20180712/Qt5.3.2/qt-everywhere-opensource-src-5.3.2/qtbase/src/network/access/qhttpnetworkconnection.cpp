    ...
    if (channelCount == 1) {
        ...
        if (networkLayerState == HostLookupPending || networkLayerState == IPv4or6) {
            ...
            if (networkLayerState == QHttpNetworkConnectionPrivate::Unknown) {
                ...
                DEBUG_SOCKET_QDEBUG2("We got a connection error when networkLayerState is Unknown");
#endif
			}
        }
    }
...
#ifdef FJ_INSP_NETWORKLOG
        	DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionPrivate::queueRequest-high:%s" , request.url().toString().toUtf8().data()  );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        	DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionPrivate::queueRequest-low:%s" , request.url().toString().toUtf8().data()  );
#endif
...
void QHttpNetworkConnectionPrivate::requeueRequest(const HttpMessagePair &pair)
{
    ...
    case QHttpNetworkRequest::HighPriority:
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionPrivate::requeueRequest-high:%s" , pair.first.url().toString().toUtf8().data()  );
#endif
        highPriorityQueue.prepend(pair);
        break;
    case QHttpNetworkRequest::NormalPriority:
    case QHttpNetworkRequest::LowPriority:
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionPrivate::requeueRequest-low:%s" , pair.first.url().toString().toUtf8().data()  );
#endif
        lowPriorityQueue.prepend(pair);
        break;
    }
...
bool QHttpNetworkConnectionPrivate::dequeueRequest(QAbstractSocket *socket)
{
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionPrivate::dequeueRequest-high:%s" , channels[i].request.url().toString().toUtf8().data()  );
#endif
        return true;
    }
    ...
    if (!lowPriorityQueue.isEmpty()) {
        ...
        channels[i].reply = messagePair.second;
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionPrivate::dequeueRequest-low:%s" , channels[i].request.url().toString().toUtf8().data()  );
#endif
        return true;
    }
    return false;
...
void QHttpNetworkConnectionPrivate::_q_hostLookupFinished(QHostInfo info)
{
    ...
    if (bIpv4 && bIpv6)
        ...
        if (dequeueRequest(channels[0].socket)) {
            ...
            // Should not happen
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("QHttpNetworkConnectionPrivate::_q_hostLookupFinished could not dequeu request");
#endif
            networkLayerState = QHttpNetworkConnectionPrivate::Unknown;
        }
    }
