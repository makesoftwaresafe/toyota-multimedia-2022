                                             ...
                                             const QVariant &value)
{
    ...
    if (name.isEmpty()) {
        ...
        DEBUG_SOCKET_QDEBUG2("QNetworkRequest::setHeader: invalid header value KnownHeader(%d) received", header);
#endif
        return;
    }
    ...
    if (value.isNull()) {
        ...
        if (rawValue.isEmpty()) {
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("QNetworkRequest::setHeader: QVariant of type %s cannot be used with header %s",
