...
bool QNetworkReplyHttpImplPrivate::loadFromCacheIfAllowed(QHttpNetworkRequest &httpRequest)
{
    ...
    DEBUG_SOCKET_QDEBUG2("response_is_fresh %d", CacheLoadControlAttribute);
#endif
         ...
         qint64 contentLength, bool spdyWasUsed)
{
    ...
    if (statusCode == 304) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("Received a 304 from %s", request.url().toString().toUtf8().constData());
#endif
        QAbstractNetworkCache *nc = managerPrivate->networkCache;
        if (nc) {
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("http error! %d %s", errorCode, errorString.toUtf8().constData());
#endif
...
bool QNetworkReplyHttpImplPrivate::sendCacheContents(const QNetworkCacheMetaData &metaData)
{
    ...
    if (!contents) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("Can not send cache, the contents are 0 %s", url.toString().toUtf8().constData());
#endif
        return false;
    }
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("Successfully sent cache: %s %d bytes", url.toString().toUtf8().constData(), contents->size());
#endif
...
QNetworkCacheMetaData QNetworkReplyHttpImplPrivate::fetchCacheMetaData(const QNetworkCacheMetaData &oldMetaData) const
{
    ...
    foreach (QByteArray header, newHeaders) {
        ...
        if (n != o && header != "date") {
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("replacing %s", header.constData());
            DEBUG_SOCKET_QDEBUG2("new %s", n.constData());
            DEBUG_SOCKET_QDEBUG2("old %s", o.constData());
#endif
        }
...
void QNetworkReplyHttpImplPrivate::_q_startOperation()
{
    ...
    if (state == Working) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkReplyImpl::_q_startOperation was called more than once");
#endif
        return;
    }
    state = Working;
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("Backend is waiting for QNetworkSession to connect, but there is none!");
#endif
            QMetaObject::invokeMethod(q, "_q_error", synchronous ? Qt::DirectConnection : Qt::QueuedConnection,
...
#else
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("Backend start failed");
#endif
        QMetaObject::invokeMethod(q, "_q_error", synchronous ? Qt::DirectConnection : Qt::QueuedConnection,
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkReplyImplPrivate::error: Internal problem, this method must only be called once.");
#endif
        return;
    }
...
void QNetworkReplyHttpImplPrivate::setCachingEnabled(bool enable)
{
    ...
    if (enable) {
        if (bytesDownloaded) {
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("setCachingEnabled: %lld bytesDownloaded", bytesDownloaded);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkReplyImpl: setCachingEnabled(true) called after setCachingEnabled(false)");
#endif
        managerPrivate->networkCache->remove(url);
        cacheSaveDevice = 0;
        cacheEnabled = false;
