...
static QDateTime parseDateString(const QByteArray &dateString)
{
    ...
    while (at < dateString.length()) {
        ...
        DEBUG_SOCKET_QDEBUG2("%s", dateString.mid(at).constData());
#endif
        ...
        if (!isNum
            ...
            ++month;
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("Month: %d", month);
#endif
            at += 3;
            continue;
        }
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("Zone: %d", month);
#endif
            at += 3;
            continue;
        }
...
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("Zone offset: %d %d %d", zoneOffset, hours, minutes);
#endif
                at += end;
                continue;
            }
        ...
        if (isNum && time.isNull()
            ...
            if (pos != -1) {
                ...
                time = QTime(h, m, s, ms);
#ifdef FJ_INSP_NETWORKLOG
				QString listStr = QString::fromUtf8("(");
				bool checkFirst = false;
				for(QStringList::const_iterator it = list.constBegin(); it != list.constEnd(); ++it) {
					if(!checkFirst) {
						listStr = listStr + QString::fromUtf8("\"%s\"");
						checkFirst = true;
					}
					else {
						listStr = listStr + QString::fromUtf8(", \"%s\"");
				...
				listStr = listStr + QString::fromUtf8(")");
                DEBUG_SOCKET_QDEBUG2("Time: %s %d", listStr.toUtf8().constData(), timeRx.matchedLength());
#endif
        ...
        if (isNum) {
            ...
            at += length;
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("Saving %d", x);
#endif
            continue;
        }
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("Final set %d %d %d", year, month, day);
#endif

    if (year == -1 || month == -1 || day == -1) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("Parser failure %d %d %d", year, month, day);
#endif
        return QDateTime();
    }
            ...
            case ';':
                ...
                if (field.first == "expires") {
                    ...
                    if (field.second.startsWith('/')) {
						...
						cookie.setPath(QUrl::fromPercentEncoding(field.second));
                    } else {
...
{
    DEBUG_SOCKET_QDEBUG2("QNetworkCookie(%s)", cookie.toRawForm(QNetworkCookie::Full).constData());
    return s.space();
}
#endif
