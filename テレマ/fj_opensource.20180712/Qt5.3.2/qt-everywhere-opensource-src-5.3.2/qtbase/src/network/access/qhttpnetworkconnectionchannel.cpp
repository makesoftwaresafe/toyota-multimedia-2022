...
void QHttpNetworkConnectionChannel::init()
{
    ...
    DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionChannel init() channel=%x, socket=%x" , this , socket );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
    	DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionChannel state<-Closing:channel=%x" , this );
#endif
...
void QHttpNetworkConnectionChannel::handleUnexpectedEOF()
{
    ...
    if (reconnectAttempts <= 0) {
        ...
        close();
#ifdef FJ_INSP_NETWORKLOG
		DEBUG_SOCKET_QDEBUG("failed in all reconnectAttempts : channel=%x, url=%s" , this , request.url().toString().toUtf8().data() );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
    	DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionChannel state<-Connecting:channel=%x" , this );
#endif
...
void QHttpNetworkConnectionChannel::allDone()
{
    ...
    if (!reply) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QHttpNetworkConnectionChannel::allDone() called without reply. Please report at http://bugreports.qt-project.org/");
#endif
        return;
    }
