...
void QHttpProtocolHandler::_q_receiveReply()
{
    ...
    if (!m_reply) {
        if (m_socket->bytesAvailable() > 0) {
            ...
            DEBUG_SOCKET_QDEBUG2("QAbstractProtocolHandler::_q_receiveReply() called without QHttpNetworkReply, %llu bytes on socket.", m_socket->bytesAvailable());
#endif
		}
        m_channel->close();
        return;
...
#ifdef FJ_INSP_NETWORKLOG
    	DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionChannel state<-Reading:channel=%x" , this );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG3("QAbstractProtocolHandler::sendRequest() called without QHttpNetworkReply");
#endif
#ifdef FJ_INSP_NETWORKLOG
    	DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionChannel state<-Idle:channel=%x" , this );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG( "QHttpProtocolHandler::sendRequest(): channel=%x, url=%s" , this , m_channel->request.url().toString().toUtf8().data() );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
    		DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionChannel state<-Writing:channel=%x" , this );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
    		DEBUG_SOCKET_QDEBUG( "QHttpNetworkConnectionChannel state<-Waiting:channel=%x" , this );
#endif
