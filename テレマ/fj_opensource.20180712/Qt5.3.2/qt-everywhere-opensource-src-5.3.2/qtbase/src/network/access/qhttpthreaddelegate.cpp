...
static QNetworkReply::NetworkError statusCodeFromHttp(int httpStatusCode, const QUrl &url)
{
    ...
    default:
        if (httpStatusCode > 500) {
            ...
            DEBUG_SOCKET_QDEBUG2("QNetworkAccess: got HTTP status code %d which is not expected from url: \"%s\"",
...
{
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::startRequestSynchronously()-start:thread=%x" , QThread::currentThreadId() );
#endif
    synchronous = true;
	...
	qWarning() << "startRequestSynchronously()";
    if (0 < QNetworkAccessManager::synchronousTimeout())
        QTimer::singleShot(QNetworkAccessManager::synchronousTimeout()*1000, this, SLOT(abortRequest()));
    else
        // Worst case timeout
...
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::startRequestSynchronously()-end:thread=%x" , QThread::currentThreadId() );
#endif
}
...
{
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::startRequest()-start:thread=%x" , QThread::currentThreadId() );
#endif
	...
	if (!synchronous && 0 < QNetworkAccessManager::timeOut())
    {
        connect(httpReply, SIGNAL(waitTimerTimeOut()), this, SLOT(requestTimeout()));
        httpReply->startWaitTimer();
    }
    ...
    if (synchronous) {
		...
		//In the asynchronous HTTP case, when recieve or send data, restart waitTimer.
        if (0 < QNetworkAccessManager::timeOut())
        {
            connect(httpReply,SIGNAL(dataReadProgress(qint64,qint64)), httpReply, SLOT(restartWaitTimer()));
            connect(httpReply,SIGNAL(dataSendProgress(qint64,qint64)), httpReply, SLOT(restartWaitTimer()));
        }
    }
...
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::startRequest()-end:thread=%x" , QThread::currentThreadId() );
#endif
}
...
{
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::abortRequest()thread=%x, sync=%d" , QThread::currentThreadId() , synchronous);
#endif
    if (httpReply) {
        delete httpReply;
        httpReply = 0;
...
{
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::readBufferSizeChanged() size=%d" , size );
#endif
    if (httpReply) {
...
void QHttpThreadDelegate::finishedSlot()
{
	...
	httpReply->stopWaitTimer();
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::finishedSlot() thread=%x result=%d" ,QThread::currentThreadId() , httpReply->statusCode() );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::synchronousFinishedSlot() thread=%x result=%d" , QThread::currentThreadId() , httpReply->statusCode() );
#endif
    if (httpReply->statusCode() >= 400) {
            // it's an error reply
            QString msg = QLatin1String(QT_TRANSLATE_NOOP("QNetworkReply",
...
void QHttpThreadDelegate::finishedWithErrorSlot(QNetworkReply::NetworkError errorCode, const QString &detail)
{
	...
	httpReply->stopWaitTimer();
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::finishedWithErrorSlot() thread=%x error=%d:%s" ,QThread::currentThreadId() , errorCode , detail.toUtf8().data() );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::synchronousFinishedWithErrorSlot() thread=%x error=%d:%s" ,QThread::currentThreadId() , errorCode , detail.toUtf8().data() );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::headerChangedSlot() thread=%x" , QThread::currentThreadId() );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::synchronousHeaderChangedSlot() thread=%x" , QThread::currentThreadId() );
#endif
...
void QHttpThreadDelegate::synchronousAuthenticationRequiredSlot(const QHttpNetworkRequest &request, QAuthenticator *a)
{
    ...
    Q_UNUSED(request);
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::synchronousAuthenticationRequiredSlot() thread=%x" , QThread::currentThreadId() );
#endif
...
#ifdef FJ_INSP_NETWORKLOG
	DEBUG_SOCKET_QDEBUG( "QHttpThreadDelegate::synchronousProxyAuthenticationRequiredSlot() thread=%x" , QThread::currentThreadId() );
#endif
    // Ask the credential cache
...
void QHttpThreadDelegate::requestTimeout()
{
    //finishedWithErrorSlot(QNetworkReply::TimeoutError, QString::fromAscii("Request Timeout")); // fromAscii is not supported for 5.3
	finishedWithErrorSlot(QNetworkReply::TimeoutError, QString::fromUtf8("Request Timeout"));
}
