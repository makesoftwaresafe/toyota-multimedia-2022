...
void QNetworkDiskCache::setCacheDirectory(const QString &cacheDir)
{
    ...
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::setCacheDirectory() %s", cacheDir.toUtf8().constData());
#endif
    Q_D(QNetworkDiskCache);
    if (cacheDir.isEmpty())
        return;
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::cacheSize()");
#endif
    Q_D(const QNetworkDiskCache);
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::prepare() %s", metaData.url().toString().toUtf8().constData());
#endif
    Q_D(QNetworkDiskCache);
    ...
    if (d->cacheDirectory.isEmpty()) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::prepare() The cache directory is not set");
#endif
        return 0;
    }
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::prepare() unable to open temporary file");
#endif
            cacheItem.reset();
            return 0;
        }
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::insert() %p", device);
#endif
    Q_D(QNetworkDiskCache);
    QHash<QIODevice*, QCacheItem*>::iterator it = d->inserting.find(device);
    if (it == d->inserting.end()) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::insert() called on a device we don't know about %p", device);
#endif
        return;
    }
...
// reimplementable
void QNetworkDiskCache::cacheSaved(const QString& filename) {
	Q_UNUSED(filename);
}

// reimplementable
void QNetworkDiskCache::cacheRemoved(const QString& filename) {
	Q_UNUSED(filename);
}
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache: couldn't remove the cache file %s", fileName.toUtf8().constData());
#endif
            return;
        }
    }
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::remove() %s", url.toString().toUtf8().constData());
#endif
    Q_D(QNetworkDiskCache);
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::removFile() %s", file.toUtf8().constData());
#endif
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::metaData() %s", url.toString().toUtf8().constData());
#endif
    Q_D(QNetworkDiskCache);
    if (d->lastItem.metaData.url() == url)
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::fileMetaData() %s", fileName.toUtf8().constData());
#endif
    Q_D(const QNetworkDiskCache);
    QFile file(fileName);
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::data() %s", url.toString().toUtf8().constData());
#endif
    Q_D(QNetworkDiskCache);
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::updateMetaData() %s", metaData.url().toString().toUtf8().constData());
#endif
    QUrl url = metaData.url();
    QIODevice *oldDevice = data(url);
    if (!oldDevice) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::updateMetaData(), no device!");
#endif
        return;
    }
    ...
    if (!newDevice) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::updateMetaData(), no new device! %s", url.toString().toUtf8().constData());
#endif
        return;
    }
    char data[1024];
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::expire() The cache directory is not set");
#endif
        return 0;
    }
...
#ifdef FJ_INSP_NETWORKLOG
    if (removedFiles > 0) {
        DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::expire() Removed:%d Kept:%d", removedFiles, cacheItems.count() - removedFiles);
    }
#endif
    return totalSize;
}
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QNetworkDiskCache::clear()");
#endif
    Q_D(QNetworkDiskCache);
