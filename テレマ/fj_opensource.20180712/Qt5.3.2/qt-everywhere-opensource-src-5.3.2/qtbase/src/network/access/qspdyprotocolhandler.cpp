    ...
    do {
        ...
        switch (zlibRet) {
		    ...
		    default: {
		        ...
		        DEBUG_SOCKET_QDEBUG2("%s got unexpected zlib return value: %d", Q_FUNC_INFO, zlibRet);
#endif
		        return false;
		    }
        }
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("sending SPDY CREDENTIAL frame is not yet implemented"); // QTBUG-36188
#endif
        return;
    }
    default:
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s Trying to upload to closed stream", Q_FUNC_INFO);
#endif
        return false;
    }
...
void QSpdyProtocolHandler::handleControlFrame(const QByteArray &frameHeaders) // ### make it char *
{
    ...
    switch (type) {
		...
		default: {
#ifdef FJ_INSP_NETWORKLOG
		    DEBUG_SOCKET_QDEBUG2("%s cannot handle frame of type %i", Q_FUNC_INFO, type);
#endif
		}
    }
}
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s error reading header from SYN_REPLY message", Q_FUNC_INFO);
#endif
        return;
    }
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s error parsing header from SYN_REPLY message", Q_FUNC_INFO);
#endif
                                            ...
                                            const QByteArray &frameData)
{
    ...
    default:
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s could not understand servers RST_STREAM status code", Q_FUNC_INFO);
#endif
        errorCode = QNetworkReply::ProtocolFailure;
...
void QSpdyProtocolHandler::handleSETTINGS(char flags, quint32 /*length*/, const QByteArray &frameData)
{
    ...
    for (int a = 0, frameDataIndex = 4; a < numberOfEntries; ++a, frameDataIndex += 8) {
        ...
        switch (uniqueID) {
		    ...
		    default: {
#ifdef FJ_INSP_NETWORKLOG
		        DEBUG_SOCKET_QDEBUG2("%s found unknown settings value %d", Q_FUNC_INFO, value);
#endif
			}
        }
    }
                                        ...
                                        const QByteArray &frameData)
{
    ...
    switch (statusCode) {
		...
		default:
#ifdef FJ_INSP_NETWORKLOG
		    DEBUG_SOCKET_QDEBUG2("%s unexpected status code %d", Q_FUNC_INFO, statusCode);
#endif
		    errorCode = QNetworkReply::ProtocolUnknownError;
    }
...
void QSpdyProtocolHandler::handleDataFrame(const QByteArray &frameHeaders)
{
    ...
    if (flag_compress) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("%s SPDY level compression is not supported", Q_FUNC_INFO);
#endif
    }
