...
static const char* QCOOKIE_FILE_EXTENSION = ".cookie";
/* end : 161017 add FJ */
static QString s_cookiePath = QString();
static QString s_cookieFileName = QString();
static qint64 s_cookieCapacity = 0;
...
QDataStream &operator>>(QDataStream &stream, QNetworkCookie &cookie)
{
    ...
    if (value.size() <= 0) {
        ...
        DEBUG_SOCKET_QDEBUG2("CookieJar: Unable to deserialize cookie:");
#endif
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("CookieJar: Unable to parse cookie:%s", value.constData());
#endif
        cookie = QNetworkCookie();
        return stream;
    }
                                          ...
                                          const QUrl &url)
{
	...
	/* start : 161017 add FJ */
	if(mModified && !d->cookieFileName.isEmpty()) {
		d->save();
	}
	/* end : 161017 add FJ */
    return added;
...
bool QNetworkCookieJar::insertCookie(const QNetworkCookie &cookie)
{
    ...
    if (!isDeletion) {
		...
		for (int i = d->allCookies.size() - 1; i >= 0; --i) {
			...
			if (isParentDomain(cookie.domain(), current.domain()) || isParentDomain(current.domain(), cookie.domain())) {
...
bool QNetworkCookieJar::deleteCookie(const QNetworkCookie &cookie)
{
	...
	int i = 0; // 161017 add FJ
    for (it = d->allCookies.begin(); it != d->allCookies.end(); it++) {
		const QNetworkCookie &current = d->allCookies.at(i); // 161017 add FJ
        if (it->hasSameIdentifier(cookie)) {
			d->remove(current); // 161017 add FJ
            d->allCookies.erase(it);
			mModified = true; // 161017 add FJ
            return true;
        }
		i++; // 161017 add FJ
	}
...
void QNetworkCookieJar::setCookiePath(const QString& path)
{
    if (path.isEmpty()) {
        s_cookiePath.clear();
        s_cookieFileName.clear();
        return;
    }
    QFileInfo fileInfo(path);
    s_cookiePath = fileInfo.path() + QLatin1Char('/');
    s_cookieFileName = fileInfo.fileName();
}

const QString& QNetworkCookieJar::cookiePath()
{
    static QString cookieFilePath;
    cookieFilePath = s_cookiePath + s_cookieFileName;
    return cookieFilePath;
}

void QNetworkCookieJar::setCookieSize(unsigned long size)
{
    s_cookieCapacity = size;
}

unsigned long QNetworkCookieJar::cookieSize()
{
    return (unsigned long)s_cookieCapacity;
}

void QNetworkCookieJar::clearAllCookies()
{
    Q_D(QNetworkCookieJar);
    d->remove();
    d->allCookies.clear();
...
QNetworkCookieJarPrivate::QNetworkCookieJarPrivate()
 : cookieCapacity(s_cookieCapacity),
   cookieSavedSize(0),
   cookiePath(s_cookiePath),
   cookieFileName(s_cookieFileName)
{
    load();
}

void QNetworkCookieJarPrivate::load()
{
    cookieSavedSize = 0;
    allCookies.clear();

    if (!cookieFileName.isEmpty()) {
        QFile file(cookiePath + cookieFileName);
        if (!file.exists())
            return;
        if (!file.open(QFile::ReadOnly)) {
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("CookieJar: Unable to open cookie file:%s", file.fileName().toUtf8().constData());
#endif
            return;
        }
        ...
        if (0 < size) {
            ...
            stream >> allCookies;
            if (0 < allCookies.count())
                cookieSavedSize = size;
        }
        file.close();
    } else if (!cookiePath.isEmpty()) {
        QDir dir(cookiePath, QString(), QDir::Time | QDir::Reversed, QDir::Files | QDir::NoDotAndDotDot);
        if (!dir.exists()) {
            if (!dir.mkpath(dir.absolutePath())) {
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("CookieJar: Unable to make cookie path:%s", cookiePath.toUtf8().constData());
#endif
			}
            return;
        } else {
            uint length = dir.count();
            for (uint i=0; i<length; i++) {
                QFile file(cookiePath + dir[i]);
                if (!file.fileName().endsWith(QLatin1String(QCOOKIE_FILE_EXTENSION)))
                    continue;
                if (!file.open(QFile::ReadOnly)) {
#ifdef FJ_INSP_NETWORKLOG
                    DEBUG_SOCKET_QDEBUG2("CookieJar: Unable to open cookie file:%s", file.fileName().toUtf8().constData());
                ...
                if (0 < size) {
                    ...
                    if (cookie.name().isEmpty()) {
                        ...
                        if (!file.remove()) {
#ifdef FJ_INSP_NETWORKLOG
                            DEBUG_SOCKET_QDEBUG2("CookieJar: Unable to remove damaged cookie file:%s", file.fileName().toUtf8().constData());
#endif
						}
                        continue;
                    }
...
void QNetworkCookieJarPrivate::save()
{
    if (!cookieFileName.isEmpty()) {
        ...
        if (0 < cookieCapacity) {
            while(cookieCapacity < byteArray.size()) {
                int num = cookies.count();
                num = num * 0.2 ;
                for (int i = 0; i <= num; i++) {
                    allCookies.removeOne(cookies.at(0));
                    cookies.removeAt(0);
                }
                byteArray.clear();
                if(cookies.count() == 0)
        ...
        if (byteArray.size() != file.write(byteArray)) {
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("CookieJar: Unable to write cookie file:%s", file.fileName().toUtf8().constData());
#endif
        }
		else
            cookieSavedSize = byteArray.size();
        file.close();
    } else if (!cookiePath.isEmpty()) {
        for (int i=0; i<allCookies.count(); ++i)
            save(allCookies.at(i));
    }
}

void QNetworkCookieJarPrivate::save(const QNetworkCookie& cookie)
{
    if (cookiePath.isEmpty() || !cookieFileName.isEmpty())
        return;
    ...
    if (0 < cookieCapacity) {
        while(cookieCapacity < (cookieSavedSize + size)) {
            int num = allCookies.count();
            num = num * 0.2 ;
            for (int i = 0, removed = 0; i < allCookies.count();) {
                if (allCookies.at(i).isSessionCookie()) {
                    i++;
                    continue;
                }
                remove(allCookies.at(i));
                allCookies.removeAt(i);
                removed++;
                if (removed >= num)
                    break;
            }
    ...
    QFile file(cookiePath + fileName(cookie));
    if(!file.open(QFile::WriteOnly)) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("CookieJar: Unable to open cookie file:%s", file.fileName().toUtf8().constData());
#endif
...
void QNetworkCookieJarPrivate::remove()
{
    if (!cookieFileName.isEmpty()) {
        QFile file(cookiePath + cookieFileName);
        if (!file.remove()) {
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("CookieJar: Unable to remove cookie file:%s", file.fileName().toUtf8().constData());
#endif
		}
    } else if (!cookiePath.isEmpty()) {
        QDirIterator dirIte(cookiePath, QDir::Files | QDir::NoDotAndDotDot);
        while (dirIte.hasNext()) {
            QFile file(dirIte.next());
            if (!file.fileName().endsWith(QLatin1String(QCOOKIE_FILE_EXTENSION)))
                continue;
            if (!file.remove()) {
#ifdef FJ_INSP_NETWORKLOG
...
void QNetworkCookieJarPrivate::remove(const QNetworkCookie& cookie)
{
    if (cookiePath.isEmpty() || !cookieFileName.isEmpty())
        return;
...
void QNetworkCookieJarPrivate::purgeOldCookies()
{
    ...
    bool modified = false;
    QDateTime now = QDateTime::currentDateTime();
    for (int i = allCookies.count() - 1; i >= 0; --i) {
        if (allCookies.at(i).isSessionCookie() == false &&
            allCookies.at(i).expirationDate() < now)
        {
            remove(allCookies.at(i));
            allCookies.removeAt(i);
            modified = true;
    ...
    if (modified && !cookieFileName.isEmpty())
        save();
}

QString QNetworkCookieJarPrivate::fileName(const QNetworkCookie& cookie)
{
    QCryptographicHash hash(QCryptographicHash::Sha1);
    hash.addData(cookie.name());
    QUrl url;
    url.setHost(cookie.domain());
    url.setPath(cookie.path());
    //hash.addData(url.encodedHost());
	hash.addData(url.toAce(url.host()));
    //hash.addData(url.encodedPath());
	hash.addData(url.path(QUrl::FullyEncoded).toLatin1());
    return QLatin1String(hash.result().toHex()) + QLatin1String(QCOOKIE_FILE_EXTENSION);
}
