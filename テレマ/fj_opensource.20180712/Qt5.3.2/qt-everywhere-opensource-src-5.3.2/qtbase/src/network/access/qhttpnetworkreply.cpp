    ...
    : QObject(*new QHttpNetworkReplyPrivate(url), parent)
{
	...
	d->waitTimer.setSingleShot(true);
    connect(&(d->waitTimer), SIGNAL(timeout()), this, SIGNAL(waitTimerTimeOut()));
}
...
void QHttpNetworkReply::stopWaitTimer()
{
    Q_D(QHttpNetworkReply);
    d->waitTimer.stop();
}

void QHttpNetworkReply::startWaitTimer()
{
    Q_D(QHttpNetworkReply);
    if (0 < QNetworkAccessManager::timeOut()) {
        d->waitTimer.start(QNetworkAccessManager::timeOut()*1000);
	}
}

void QHttpNetworkReply::restartWaitTimer()
{
    Q_D(QHttpNetworkReply);
    if (d->waitTimer.isActive() && 0<QNetworkAccessManager::timeOut())
    {
        d->waitTimer.stop();
        d->waitTimer.start(QNetworkAccessManager::timeOut()*1000);
    }
