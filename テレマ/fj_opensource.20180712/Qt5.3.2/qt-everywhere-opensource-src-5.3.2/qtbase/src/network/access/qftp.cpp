...
void QFtpDTP::writeData()
{
    ...
    if (is_ba) {
        ...
        DEBUG_SOCKET_QDEBUG2("QFtpDTP::writeData: write %d bytes", data.ba->size());
#endif
        if (data.ba->size() == 0)
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QFtpDTP::writeData: write() of size %lli bytes", read);
#endif
        if (read > 0) {
...
{
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QFtpDTP::abortConnection, bytesAvailable == %lli",
...
void QFtpDTP::socketConnected()
{
    bytesDone = 0;
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QFtpDTP::connectState(CsConnected)");
#endif
...
void QFtpDTP::socketReadyRead()
{
    ...
    if (pi->currentCommand().isEmpty()) {
        socket->close();
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QFtpDTP::connectState(CsClosed)");
#endif
    ...
    if (pi->currentCommand().startsWith(QLatin1String("LIST"))) {
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("QFtpDTP read (list): '%s'", line.constData());
#endif
        ...
        if (!is_ba && data.dev) {
            do {
                ...
                bytesDone += bytesRead;
#ifdef FJ_INSP_NETWORKLOG
                DEBUG_SOCKET_QDEBUG2("QFtpDTP read: %lli bytes (total %lli bytes)", bytesRead, bytesDone);
#endif
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("QFtpDTP readyRead: %lli bytes available (total %lli bytes read)",
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QFtpDTP::connectState(CsHostNotFound)");
#endif
        emit connectState(QFtpDTP::CsHostNotFound);
    } else if (e == QTcpSocket::ConnectionRefusedError) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QFtpDTP::connectState(CsConnectionRefused)");
#endif
...
void QFtpDTP::socketBytesWritten(qint64 bytes)
{
    bytesDone += bytes;
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QFtpDTP::bytesWritten(%lli)", bytesDone);
#endif
    emit dataTransferProgress(bytesDone, bytesTotal);
    if (callWriteData)
        writeData();
...
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QFtpPI send: ABOR");
#endif
...
//    qDebug("QFtpPI state: %d [processReply() begin]", state);
    if (replyText.length() < 400) {
		DEBUG_SOCKET_QDEBUG2("QFtpPI recv: %d %s", 100*replyCode[0]+10*replyCode[1]+replyCode[2], replyText.toLatin1().constData());
	}
	else {
		DEBUG_SOCKET_QDEBUG2("QFtpPI recv: %d (text skipped)", 100*replyCode[0]+10*replyCode[1]+replyCode[2]);
	}
#endif
...
//    qDebug("QFtpPI state: %d [processReply() intermediate]", state);
    ...
    if (rawCommand) {
...
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("QFtp: bad 227 response -- address and port information missing");
#endif
        ...
        if (portPos == -1) {
#ifdef FJ_INSP_NETWORKLOG
            DEBUG_SOCKET_QDEBUG2("QFtp: bad 229 response -- port information missing");
#endif
            // this error should be reported
        } else {
            ++portPos;
...
bool QFtpPI::startNextCmd()
{
...
#ifdef FJ_INSP_NETWORKLOG
    if (state != Idle)
        DEBUG_SOCKET_QDEBUG2("QFtpPI startNextCmd: Internal error! QFtpPI called in non-Idle state %d", state);
#endif
    if (pendingCommands.isEmpty()) {
        currentCmd.clear();
    ...
    pendingCommands.pop_front();
#ifdef FJ_INSP_NETWORKLOG
    DEBUG_SOCKET_QDEBUG2("QFtpPI send: %s", currentCmd.left(currentCmd.length()-2).toLatin1().constData());
#endif
    state = Waiting;
...
void QFtpPrivate::_q_piError(int errorCode, const QString &text)
{
    ...
    if (pending.isEmpty()) {
#ifdef FJ_INSP_NETWORKLOG
        DEBUG_SOCKET_QDEBUG2("QFtpPrivate::_q_piError was called without pending command!");
#endif
        return;
    }
