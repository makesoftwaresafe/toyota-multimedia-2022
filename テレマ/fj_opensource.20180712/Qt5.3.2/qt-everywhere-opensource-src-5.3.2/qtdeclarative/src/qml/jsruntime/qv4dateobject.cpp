...
static inline double DaylightSavingTA(double t)
{
    ...
    if (!insp_localtime_r((const time_t*) &tt, &tmtm))
#else
    // Returns shared static data which may be overwritten at any time
    // (for MinGW/Windows localtime is luckily thread safe)
    long int tt = (long int)(t / msPerSecond);
    if (struct tm *tmtm_p = insp_localtime((const time_t*) &tt))
        tmtm = *tmtm_p;
    else
#endif
        return 0;
...
static double getLocalTZA()
{
    ...
    insp_tzset();
    insp_time(&curr);
    insp_localtime_r(&curr, &t);
    time_t locl = insp_mktime(&t);
    insp_gmtime_r(&curr, &t);
    time_t globl = insp_mktime(&t);
