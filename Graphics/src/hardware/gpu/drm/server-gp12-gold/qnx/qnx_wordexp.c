...
static void* qnx_mempcpy(void* dst, const void* src, size_t len)
{
    return (char *)memcpy(dst, src, len) + len;
}

static char* qnx_stpcpy(char* dst, const char* src)
{
    const size_t len = strlen (src);
    return (char *) memcpy (dst, src, len + 1) + len;
}

static char* qnx_strndup(const char *s, size_t n)
{
    char* result;
    size_t len = strlen(s);
					  ...
					  const char *str, size_t len)
{
	...
	if (buffer != NULL) {
		*((char *) qnx_mempcpy(&buffer[*actlen], str, len)) = '\0';
		*actlen += len;
	}
	return buffer;
}
			...
			const char *words, size_t * offset, size_t wordc)
{
	...
	if (i == 1 + *offset) {
		...
		char *user = qnx_strndup(&words[1 + *offset], i - (1 + *offset));
		struct passwd pwd, *tpwd;
		int buflen = 1000;
...
exec_comm_child(char *comm, int *fildes, int showerr, int noexec)
{
	...
	/* Bad.  What now?  */
	qnx_error("cannot spawn a shell");
	abort();
}
		...
		/* Is it `$*' or `$@' (unquoted) ? */
			...
			*end = 0;
			for (p = 1; __libc_argv[p]; ++p) {
				if (p > 1)
					*end++ = ' ';
				end = qnx_stpcpy(end, __libc_argv[p]);
			}
			...
			case ACT_RP_SHORT_RIGHT:
				for (p = end; p >= value; --p) {
					if (fnmatch(pattern, p, 0) != FNM_NOMATCH) {
						...
						*(char *) qnx_mempcpy(newval, value, p - value) = '\0';
						if (free_value)
							free(value);
						value = newval;
						free_value = 1;
...
int qnx_wordexp(const char *words, wordexp_t * we, int flags)
{
	size_t words_offset;
	size_t word_length;
	size_t max_length;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/denso-jp-hv-ivi/hardware/gpu/drm/server-gp12-gold/qnx/qnx_wordexp.c $ $Rev: 845340 $")
#endif
