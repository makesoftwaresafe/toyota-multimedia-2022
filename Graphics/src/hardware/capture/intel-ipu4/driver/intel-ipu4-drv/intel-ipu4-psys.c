...
#else
static void* intel_ipu4_psys_isr_run(void *data);
#endif
...
#else
	// Assumes this is not IO memory
	page = (struct page *)malloc(sizeof(struct page)*npages);
	if (!page) {
		kfree(pages);
		return -ENOMEM;
	}
...
static int intel_ipu4_psys_open(struct inode *inode, struct file *file)
{
	...
	if (start_thread) {
		...
		rval = pthread_cond_init(&fh->cond, NULL);
		if (rval != EOK) {
			mutex_unlock(&psys->mutex);
			return -EIO;
		}
		...
		psys->isr_thread_exit = false;
		rval = pthread_create(&psys->isr_thread, &thread_attr,
			intel_ipu4_psys_isr_run, (void*) psys);
...
static int intel_ipu4_psys_release(struct inode *inode, struct file *file)
{
	...
	if (list_empty(&psys->fhs)) {
		psys->isr_thread_exit = true;
		pthread_join(psys->isr_thread, NULL);
	}
...
#else
		uint64_t cur_time;
		qnxw_get_current_clk_time(&cur_time);
		kcmd->watchdog.expires = (int64_t)cur_time + (psys->timeout * NSEC_PER_MSEC) ;
#endif
...
#else
	pthread_cond_signal(&kcmd->fh->cond);
#endif
}
...
#else
					"kcmd:%d[0x%" PRIx64 "] taking too long\n",
#endif
...
#else
	pthread_cond_signal(&psys->sched_cmd_cond);
#endif
	mutex_unlock(&psys->mutex);
	return;
...
#else
	// No work queue - call work function directly from this timer thread
	intel_ipu4_psys_watchdog_work(&psys->watchdog_work);
#endif
}
...
#else
	dev_dbg(&psys->adev->dev, "IOC_QCMD: id:%d issue_id:0x%" PRIx64 " pri:%d\n",
#endif
				  ...
				  unsigned int f_flags)
{
	...
	if (!(f_flags & O_NONBLOCK)) {
		...
		while (wait_for_cmd) {
		    pthread_mutex_lock(&fh->mutex.mux);
		    pthread_cond_wait(&fh->cond, &fh->mutex.mux);
		    pthread_mutex_unlock(&fh->mutex.mux);
		    kcmd = intel_ipu4_get_completed_kcmd(psys, fh);
		    if (kcmd) {
		        wait_for_cmd = false;
		    }
...
#ifdef __QNXNTO__
			psys->pdata->ipdata->hw_variant.mmu_hw[0].val.offset +
#else
...
#else
static void* intel_ipu4_psys_sched_cmd(void *ptr)
#endif
{
	...
	while (1) {
		...
		while (wait_for_event) {
	        mutex_lock(&psys->mutex);
		    pthread_cond_wait(&psys->sched_cmd_cond, &psys->mutex.mux);
		    if (psys->sched_cmd_thread_run == false) {
		        wait_for_event = false;
		    }
		    pending = atomic_read(&psys->wakeup_sched_thread_count);
		    if (pending != 0) {
		        wait_for_event = false;
		    }
	        mutex_unlock(&psys->mutex);
...
static int intel_ipu4_psys_probe(struct intel_ipu4_bus_device *adev)
{
...
#else
	rval = pthread_cond_init(&psys->sched_cmd_cond, NULL);
	if (rval != EOK) {
		mutex_destroy(&psys->mutex);
		goto out_unlock;
	}
	...
	pthread_attr_setschedpolicy(&thread_attr, SCHED_RR);
	pthread_attr_setschedparam(&thread_attr, &sched_param);
	psys->sched_cmd_thread_run = true;
	rval = pthread_create(&psys->sched_cmd_thread, &thread_attr,
		intel_ipu4_psys_sched_cmd, (void*) psys);
	pthread_attr_destroy(&thread_attr);
	if (rval != EOK) {
		mutex_destroy(&psys->mutex);
		pthread_cond_destroy(&psys->sched_cmd_cond);
		psys->sched_cmd_thread_run = false;
		goto out_unlock;
	}
#endif
	...
	strlcpy((char*)caps.dev_model, intel_ipu4_media_ctl_dev_model(isp),
#else
...
#else
	if (psys->sched_cmd_thread_run) {
	    psys->sched_cmd_thread_run = false;
	    pthread_cond_signal(&psys->sched_cmd_cond);
	    pthread_join(psys->sched_cmd_thread, NULL);
	}
	pthread_cond_destroy(&psys->sched_cmd_cond);
	spin_destroy(&psys->power_lock);
	spin_destroy(&psys->pgs_lock);
#endif
out_unlock:
...
#else
	while (psys->isr_thread_exit == false) {
#endif
...
#else
struct intel_ipu4_bus_driver intel_ipu4_psys_driver = {
#endif
...
#else
void intel_ipu4_psys_exit(void)
#endif
{
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/denso-jp-hv-ivi/hardware/capture/intel-ipu4/driver/intel-ipu4-drv/intel-ipu4-psys.c $ $Rev: 871034 $")
#endif
