...
#ifdef __QNXNTO__
			       mmu_hw->val.base + MMUV2_AT_REG_L1_ZLW_INSERTION(0));
#else
...
#ifdef __QNXNTO__
		ret = readl(mmu_hw->val.base + MMUV2_AT_REG_L1_FW_ZLW_FIFO);
#else
...
#ifdef __QNXNTO__
			readl(mmu->mmu_hw[i].val.base+REG_L1_PHYS);
#else
...
#ifdef __QNXNTO__
			writel(inv, mmu->mmu_hw[i].val.base + REG_TLB_INVALIDATE);
#else
...
#ifdef __QNXNTO__
		// store current initAddrTableFlag
		pthread_mutex_lock(&addrTableMutex);
		currentInitAddrTableFlag = initAddrTableFlag;
		// set flag to true since we need to keep l2_virt address in address table till completely destroy mmu domain
		initAddrTableFlag = true;
		pthread_mutex_unlock(&addrTableMutex);

		l1_entry = virt_to_phys(l2_virt) >> ISP_PADDR_SHIFT;

		// restore current initAddrTableFlag
		pthread_mutex_lock(&addrTableMutex);
		initAddrTableFlag = currentInitAddrTableFlag;
		pthread_mutex_unlock(&addrTableMutex);
#else
		l1_entry = virt_to_phys(l2_virt) >> ISP_PADDR_SHIFT;
#endif
...
static int intel_ipu4_mmu_hw_init(struct device *dev)
{
	...
	for (i = 0; i < mmu->nr_mmus; i++) {
		...
		writel((phys_addr_t)virt_to_phys(adom->pgtbl)
		       >> ISP_PADDR_SHIFT,
#ifdef __QNXNTO__
		       mmu->mmu_hw[i].val.base + REG_L1_PHYS);
#else
...
#ifdef __QNXNTO__
			       mmu->mmu_hw[i].val.base + REG_INFO);
#else
		...
		for (j = 0, block_addr = 0; j < mmu_hw->nr_l1streams;
			...
			writel(block_addr,
#ifdef __QNXNTO__
			       mmu_hw->val.base + MMUV2_REG_L1_STREAMID(j));
#else
...
#ifdef __QNXNTO__
			       mmu_hw->val.base + MMUV2_AT_REG_L1_ZLW_EN_SID(j));
#else
		...
		if (zlw_invalidate)
#ifdef __QNXNTO__
            		writel(1, mmu_hw->val.base + MMUV2_AT_REG_L1_FW_ZLW_PRIO);
#else
		...
		for (j = 0, block_addr = 0; j <  mmu_hw->nr_l2streams;
			...
			writel(block_addr,
#ifdef __QNXNTO__
			       mmu_hw->val.base + MMUV2_REG_L2_STREAMID(j));
#else
...
struct intel_ipu4_bus_driver intel_ipu4_mmu_driver = {
#else
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/denso-jp-hv-ivi/hardware/capture/intel-ipu4/driver/intel-ipu4-drv/intel-ipu4-mmu.c $ $Rev: 871034 $")
#endif
