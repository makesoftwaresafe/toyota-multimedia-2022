...
#ifdef __QNXNTO__
	// This condition prevents deadlock when switch streaming between camera and HDMI input
	// TODO: find the real root cause of mutex deadlock
	if (pipe_av != av) {
		mutex_unlock(&av->mutex);
...
#ifdef __QNXNTO__
	// Code below is trying to obtain the format from I2c driver; obtain this
	// from our wrapper instead
	int err;
	uint32_t port;

	err = qnxw_get_csi2_port(av->pad.entity, &port);
	if (err != EOK) {
		dev_dbg(&av->isys->adev->dev, "Failed to get csi2 port");
		return -EIO;
	}
	err = qnxw_get_source_format(&fmt, port, av->ip.vc);
	if (err != EOK) {
	    dev_dbg(&av->isys->adev->dev, "Failed to get source format");
	    return -EIO;
	}
#else
...
#ifdef __QNXNTO__
void intel_ipu4_isys_queue_flush(struct vb2_queue *q)
{
...
#ifdef __QNXNTO__
			dev_dbg(&isys->adev->dev, "sof: using sequence number %u for timestamp 0x%" PRIx64,
...
#else
	dev_dbg(&isys->adev->dev, "SOF: looking for 0x%" PRIx64, time);
#endif
	for (i = 0; i < INTEL_IPU4_ISYS_MAX_PARALLEL_SOF; i++)
		dev_dbg(&isys->adev->dev,
#ifndef __QNXNTO__
...
#else
			"SOF: sequence %u, timestamp value 0x%" PRIx64,
#endif
...
#ifdef __QNXNTO__
	// These are not used directly by any of the driver code, so should be safe to remove
	aq->vbq.mem_ops = NULL;
#else
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/denso-jp-hv-ivi/hardware/capture/intel-ipu4/driver/intel-ipu4-drv/intel-ipu4-isys-queue.c $ $Rev: 838597 $")
#endif
