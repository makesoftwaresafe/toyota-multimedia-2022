...
#else
// No CSI2 port optimization for QNX
static bool csi2_port_optimized = false;
#endif
...
#ifndef __QNXNTO__
// No I2C code needed for QNX - we do not need to talk to camera I2c driver
...
{
    int                                     i;
    struct intel_ipu4_isys_csi2_config*     csi2_info;
    const struct intel_ipu4_isys_internal_csi2_pdata *csi2 =
        &isys->pdata->ipdata->csi2;

    // The above code is finding I2c driver, creating a subdev with it and then
    // creating a media link to connect it to our CSI2 subdev.  We do not need
    // to do this on our side, all we need to do is assign to appropriate CSI2
    // port how many lanes are required; information we get from our wrapper
    for (i = 0; i < csi2->nports; i++) {
        csi2_info = qnxw_get_csi2_config(i);
        if (csi2_info && (csi2_info->port < csi2->nports)) {
            isys->csi2[csi2_info->port].nlanes = csi2_info->nlanes;
        }
    }
}
#endif
...
#ifdef __QNXNTO__
	// Free these that were allocated with devm_kcalloc()
	free(isys->tpg);
	isys->tpg = NULL;
	free(isys->csi2);
	isys->csi2 = NULL;
...
#endif
#ifdef __QNXNTO__
	spin_destroy(&isys->lock);
	spin_destroy(&isys->power_lock);
#endif
}
...
#ifdef __QNXNTO__
	    // In original code they set isp->pkg_dir_dma_addr when initialize psys module (see cpd_fw_reload() from intel-ipu4-psys.c).
	    // In our code we set isp->pkg_dir_dma_addr later on in this function after execute intel_ipu4_buttress_map_fw_image()
	    if (!mmu) {
#else
...
#ifdef __QNXNTO__
	// Shared ISYS structure with our wrapper
	qnxw_set_isys_instance(isys);
#endif
...
#ifdef __QNXNTO__
            isp->pkg_dir_dma_addr = sg_dma_address(isys->fw_sgt.sgl);
#endif
...
static int isys_isr_one(struct intel_ipu4_bus_device *adev)
{
...
#else
			"hostlib: error resp %02d %s, stream %u, error SUSPENSION, details %d, timestamp 0x%" PRIx64 ", pin %d",
#endif
			resp->type,
...
#else
			"hostlib: error resp %02d %s, stream %u, error %d, details %d, timestamp 0x%" PRIx64 ", pin %d",
#endif
			resp->type,
			fw_msg[resp_type_to_index(resp->type)].msg,
			resp->stream_handle,
...
#else
			"hostlib: resp %02d %s, stream %u, timestamp 0x%" PRIx64 ", pin %d",
#endif
			resp->type,
			fw_msg[resp_type_to_index(resp->type)].msg,
			resp->stream_handle,
	...
	case IPU_FW_ISYS_RESP_TYPE_FRAME_SOF:
		...
		dev_dbg(&adev->dev,
#ifdef __QNXNTO__
			"sof: handle %d: (index %u), timestamp 0x%16.16" PRIx64,
#else
...
#ifdef __QNXNTO__
void* intel_ipu4_isys_isr_run(void *ptr)
#else
...
#ifdef __QNXNTO__
	while (isys->exit_isr_thread == false) {
#else
...
#ifdef __QNXNTO__
// Need to make this public for QNX case
struct intel_ipu4_bus_driver isys_driver = {
#else
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/denso-jp-hv-ivi/hardware/capture/intel-ipu4/driver/intel-ipu4-drv/intel-ipu4-isys.c $ $Rev: 871034 $")
#endif
