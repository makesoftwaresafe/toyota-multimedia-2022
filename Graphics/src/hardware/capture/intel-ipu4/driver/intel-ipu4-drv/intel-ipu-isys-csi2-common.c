...
{
#ifdef __QNXNTO__
	// Get link frequency from our wrapper code as we cannot get it from camera
	// driver like they are doing below
	struct v4l2_querymenu qm;
	qm.value = qnxw_get_csi2_link_frequency(csi2->index);
	dev_dbg(&csi2->isys->adev->dev, "%s: link frequency %" PRIx64, __func__, qm.value);
#else
...
{
#ifdef __QNXNTO__
	// This code was getting this from I2C driver which is not possible for QNX
	uint32_t        index;
	if (sd == NULL) {
		return -EINVAL;
	}
	...
	index = csi2->index;
	int err = qnxw_get_frame_desc_entry_by_dt(entry, data_type, index);
	if (err == EOK) {
		return 0;
	}
	return -EINVAL;
	...
	struct ipu_fw_isys_stream_cfg_data_abi *cfg)
{
	...
	rval = qnxw_get_csi2_port(av->pad.entity, &port);
	if (rval != EOK) {
		dev_dbg(&av->isys->adev->dev, "Failed to get csi2 port");
		return;
	}
	rval = qnxw_get_frame_desc_entry_by_dt(&entry, INTEL_IPU4_ISYS_MIPI_CSI2_TYPE_EMBEDDED8, port);
	if (rval != EOK) {
		dev_dbg(&av->isys->adev->dev, "Failed to get frame_desc_entry by dt");
		return;
	}
#else
...
static int set_stream(struct v4l2_subdev *sd, int enable)
{
	...
	struct intel_ipu4_isys_pipeline *ip =
		container_of(sd->entity.pipe,
			     struct intel_ipu4_isys_pipeline, pipe);
#ifdef __QNXNTO__
	struct intel_ipu4_isys_csi2_config *cfg = NULL;
#else
...
#ifdef __QNXNTO__
	cfg = qnxw_get_csi2_config(csi2->index);
	if (cfg == NULL) {
		return -EINVAL;
	}
	dev_dbg(&csi2->isys->adev->dev, "lane nr %d,legacy lane cfg %d,combo lane cfg %d.\n",
		cfg->nlanes, csi2->isys->legacy_port_cfg,
...
#ifdef __QNXNTO__
// csi2_link_validate below is not called in QNX; need some of its init
void intel_ipu_isys_csi2_init_pipe(struct intel_ipu4_isys_csi2 *csi2, struct intel_ipu4_isys_pipeline* ip)
{
	csi2->receiver_errors = 0;
	ip->csi2 = csi2;
	intel_ipu4_isys_video_add_capture_done(ip, csi2_capture_done);
}
#endif
...
#else
        // Use our local entity instead as no remote entity
        sd,
#endif
		&entry,
		INTEL_IPU4_ISYS_MIPI_CSI2_TYPE_EMBEDDED8);
...
#ifdef __QNXNTO__
		// Use local entity as we do not have any remote pad
		struct v4l2_mbus_frame_desc_entry entry;
		int rval = get_frame_desc_entry_by_dt(sd, &entry, INTEL_IPU4_ISYS_MIPI_CSI2_TYPE_EMBEDDED8);
#else
		struct media_pad *pad = media_entity_remote_pad(
			&sd->entity.pads[CSI2_PAD_SINK]);
		struct v4l2_mbus_frame_desc_entry entry;
...
#ifdef __QNXNTO__
	destroy_completion(&csi2->eof_completion);
	if (csi2->store_csi2_header) {
	    free(csi2->store_csi2_header);
	    csi2->store_csi2_header = NULL;
	}
#endif
	csi2->isys = NULL;
}
...
#ifdef __QNXNTO__
	// Function is not defined - assuming this was for debug only and not required
#else
	trace_ipu4_sof_seqid(ev.u.frame_sync.frame_sequence, csi2->index, vc);
#endif
#ifdef __QNXNTO__
	qnxw_csi2_sof_event(vdev, &ev);
#else
					   ...
					   unsigned int vc)
{
...
#ifdef __QNXNTO__
	// Setup event information for calling wrapper below
	struct intel_ipu4_isys_pipeline *ip = NULL;
	struct v4l2_event ev = {
		.type = V4L2_EVENT_FRAME_SYNC,
	};
	...
	ev.u.frame_sync.frame_sequence = atomic_read(&ip->sequence) - 1;
	ev.id = ip->stream_id;
#endif
	spin_unlock_irqrestore(&csi2->isys->lock, flags);

#ifdef __QNXNTO__
	// Notify QNX wrapper of EOF event
	qnxw_csi2_eof_event(vdev, &ev);
#endif
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/denso-jp-hv-ivi/hardware/capture/intel-ipu4/driver/intel-ipu4-drv/intel-ipu-isys-csi2-common.c $ $Rev: 871034 $")
#endif
